"""Genera JSON completo (tokens, temps i probabilitats) per als clips A."""
...
def one(row: dict):
    ...
    if not json_path.exists():
        cmd = [
            ...
            "-bs", "5", "-bo", "5", "-nf", "-oj", "-ojf", "-otxt", "-of",
            str(prefix), str(PROV / row["clip"]),
        ]
        with log_path.open("w", encoding="utf-8") as handle:
            result = subprocess.run(cmd, stdout=handle, stderr=subprocess.STDOUT)
        if result.returncode:
            raise RuntimeError(f"fallada JSON full {row['id_persona']} {row['forma']}")
    return {
        "id_persona": row["id_persona"],
        "forma": row["forma"],
        "clip": row["clip"],
        "json_full": f"qa-clips-json-full/{json_path.name}",
...
def main() -> None:
    ...
    output = PROV / "qa-clips-json-full.tsv"
    fields = ["id_persona", "forma", "clip", "json_full", "estat"]

"""Resumeix i prepara l'audició del candidat RTVA Carine Montaner."""
...
SMALL = CAND / "asr" / "carine-montaner.json"
BASE = CAND / "asr" / "carine-montaner-base.json"
...
def main() -> None:
    ...
    (CAND / "comparacio-asr.md").write_text(
        f"""# Comparació ASR — Carine Montaner
...
""", encoding="utf-8")
    ...
    for row in chosen:
        ...
        filename = f"carine-montaner__{slug(str(row['forma']))}__{float(row['inici_s']):.2f}.wav"

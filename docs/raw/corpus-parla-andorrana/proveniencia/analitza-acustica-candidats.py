"""Calcula descriptors acústics orientatius per als clips dels candidats."""
from __future__ import annotations
import csv, wave
from pathlib import Path
import numpy as np
ROOT=Path(__file__).parents[1]; RATE=16000; FRAME=640; HOP=320; FFT=2048; THRESH=10**(-35/20)
SPECS=[('lead-rtva-001','Joan Verdú','formes-joan-clips.tsv'),('lead-rtva-002-ian-moya','Ian Moya','formes-clips.tsv'),('lead-rtva-003-dj-neura','DJ Neura','formes-clips.tsv'),('lead-rtva-004-joan-mico','Joan Micó','formes-clips.tsv'),('lead-cg-001-xavier-espot','Xavier Espot','formes-clips.tsv')]
...
def process(folder,label,manifest):
 cand=ROOT/'proveniencia'/'candidats'/folder; rows=[]
 with (cand/manifest).open(encoding='utf-8',newline='') as h:
  rows=[measure(cand,r) for r in csv.DictReader(h,delimiter='\t')]
 ...
 lines=[f'# Perfil acústic — {label}','', 'Les mesures són descriptors instrumentals dels clips seleccionats. No substitueixen l’escolta ni permeten atribuir una variant dialectal.','', '| forma | durada | F0 mediana | F0 IQR | veu | pausa mediana |','|---|---:|---:|---:|---:|---:|']
 ...
 if '## Perfil acústic instrumental' not in s:
  marker='## Revisió preparada\n'; block=f"## Perfil acústic instrumental\n\n`analisi-acustica.tsv` i `analisi-acustica.md` conserven durada, proporció de veu, F0, energia, centroid espectral i pauses dels {len(rows)} clips. Són descriptors exploratoris; no substitueixen l’audició ni confirmen trets dialectals.\n\n";report.write_text(s.replace(marker,block+marker,1),encoding='utf-8')
 return len(rows)
def main():
 for spec in SPECS:print(spec[1],process(*spec),'clips')

"""Ordena provisionalment els torns d'DJ Neura i l'entrevistador."""
...
CAND = ROOT / "proveniencia" / "candidats" / "lead-rtva-003-dj-neura"
JSON = CAND / "asr" / "dj-neura.json"
...
def classify(text: str) -> tuple[str, str, str]:
    ...
    if first and len(words) >= 8:
        return "dj-neura-probable", "baixa", "primera persona i resposta desenvolupada"
    if len(words) >= 18:
        return "dj-neura-probable", "baixa", "segment llarg compatible amb resposta"
...
def main() -> None:
    ...
    form_counts = Counter()
    for row in rows:
        if row["rol_provisional"] != "dj-neura-probable":
            continue
        for category, form in forms:
            if re.search(rf"(?<![\wà-ÿ]){re.escape(form)}(?![\wà-ÿ])", row["text_asr"], re.I):
                form_counts[form] += 1
                form_rows.append({"categoria": category, "forma": form, "segment": row["segment"], "inici_s": row["inici_s"], "final_s": row["final_s"], "text_asr": row["text_asr"], "estat": "rol DJ Neura provisional; pendent d'audició"})
    with (CAND / "formes-dj-neura-provisional.tsv").open("w", encoding="utf-8", newline="") as handle:
    ...
    (CAND / "segmentacio-veus.md").write_text(
        """# Segmentació provisional de veus — DJ Neura
...
|---|---:|
""" + "\n".join(f"| {role} | {count} |" for role, count in sorted(counts.items())) + f"\n\nLa columna `motiu` conserva la raó de cada hipòtesi i les etiquetes no modifiquen el recompte canònic. La hipòtesi `dj-neura-probable` conté **{len(form_rows)} ocurrències** en **{len(form_counts)} formes**; el detall és a `formes-dj-neura-provisional.tsv`.\n", encoding="utf-8")

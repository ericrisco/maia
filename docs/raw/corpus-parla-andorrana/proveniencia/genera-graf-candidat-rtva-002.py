"""Genera la vista provisional de Ian Moya contra el graf canònic complet."""
...
CAND = PROV / "candidats" / "lead-rtva-002-ian-moya"
OUT = CAND / "graf"
...
def main() -> None:
    ...
    with (OUT / "nodes.tsv").open("w", encoding="utf-8", newline="") as handle:
        ...
        writer.writerow(["lead-rtva-002-ian-moya", "Ian Moya", len(candidate_forms), ",".join(sorted(candidate_forms)), "candidat separat; pendent de veu i audició"])
        for node in canonical:
    ...
    with (OUT / "trets.tsv").open("w", encoding="utf-8", newline="") as handle:
        ...
        writer.writerows([[form, "Ian Moya + graf canònic complet", "consensual ASR; pendent d'audició"] for form in sorted(candidate_forms)])
    names = {node["id_parlant"]: node["nom_public"].replace('"', '\\"') for node in canonical}
    lines = ['graph TD', '  C["Ian Moya — candidat"]']
    ...
    (OUT / "README.md").write_text(
        f"""# Graf provisional — Ian Moya

Aquesta vista manté el candidat RTVA separat del graf canònic. Usa **{len(candidate_forms)} formes** presents en small i base i connecta Ian amb **{len(edges)} persones** que en comparteixen almenys dues segons la matriu ASR completa.

Les arestes són semblances textuals; no indiquen dialecte, identitat lingüística ni veu confirmada. Ian no entra encara al recompte canònic.
""", encoding="utf-8")

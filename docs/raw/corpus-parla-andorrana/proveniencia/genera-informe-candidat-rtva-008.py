"""Resumeix i prepara l'audició del candidat RTVA Mireia Pedescoll."""
...
SMALL = CAND / "asr" / "mireia-pedescoll.json"
BASE = CAND / "asr" / "mireia-pedescoll-base.json"
...
def main() -> None:
    ...
    (CAND / "comparacio-asr.md").write_text(
        f"""# Comparació ASR — Mireia Pedescoll
...
""", encoding="utf-8")
    ...
    for row in chosen:
        ...
        filename = f"mireia-pedescoll__{slug(str(row['forma']))}__{float(row['inici_s']):.2f}.wav"

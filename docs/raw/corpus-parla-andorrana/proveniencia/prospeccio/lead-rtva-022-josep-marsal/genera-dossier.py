"""Prepara el dossier de prospecció de Josep Marsal fora del cànon."""
...
SMALL=json.loads((ASR/'josep-marsal-small-nocontext.json').read_text())
BASE=json.loads((ASR/'josep-marsal-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[
 ('política','institucions'),('president','institucions'),('justícia','institucions'),('tribunal','institucions'),('consell','institucions'),('conseller','institucions'),('democràcia','institucions'),('Constitució','institucions'),('Govern','institucions'),('història','memòria'),('transformació','memòria'),('segle','memòria'),('economia','economia'),('comerç','economia'),('Andorra','toponim'),('cultura','cultura'),('joventut','societat'),('esports','esport'),
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-rtva-022__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-rtva-022-josep-marsal','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})

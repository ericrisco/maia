"""Prepara el dossier de prospecció de Núria Pablos fora del cànon."""
...
SMALL=json.loads((ASR/'nuria-pablos-small-nocontext.json').read_text())
BASE=json.loads((ASR/'nuria-pablos-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[('Núria', 'persona'), ('celíac', 'salut'), ('celiaquia', 'salut'), ('gluten', 'salut'), ('dieta', 'salut'), ('alimentació', 'salut'), ('escoles', 'educació'), ('Ministeri', 'institucions'), ('associació', 'societat'), ('Andorra', 'toponim'), ('andorrà', 'gentilici'), ('pa', 'alimentació'), ('farina', 'alimentació'), ('restaurants', 'societat'), ('família', 'societat'), ('salut', 'salut'), ('diagnòstic', 'salut')]
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if clip: queue.append({'candidate':'lead-yt-058-nuria-pablos','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})

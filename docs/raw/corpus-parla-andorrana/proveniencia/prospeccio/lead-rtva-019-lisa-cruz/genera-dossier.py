"""Prepara el dossier de prospecció de Lisa Cruz fora del cànon."""
...
SMALL=json.loads((ASR/'lisa-cruz-small-nocontext.json').read_text())
BASE=json.loads((ASR/'lisa-cruz-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[
 ('joventut','societat'),('jove','societat'),('joves','societat'),('Fòrum de la Joventut','institucions'),
 ('habitatge','societat'),('lloguer','economia'),('comprar','economia'),('sou','economia'),('feina','economia'),
 ('estudis','educació'),('estudiant','educació'),('universitat','educació'),('participació','institucions'),
 ('política','institucions'),('eleccions','institucions'),('militància','institucions'),('institucions','institucions'),
 ('Andorra','toponim'),('andorrà','gentilici'),('andorrans','gentilici'),('joventut andorrana','societat'),
 ('col·lectiu','societat'),('drets','institucions'),('present','temps'),('futur','temps'),('amic','societat'),
 ('company','societat'),('comunicació','societat'),('xarxes socials','societat'),('català','sociolingüística'),
 ('castellà','contacte'),('espanyol','contacte'),('francès','contacte'),('habitatge assequible','economia'),
 ('desafecció','societat'),('responsable','institucions'),('activa','societat'),('acollit','societat'),
 ('parròquia','institucions'),('Constitució','institucions'),
]
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-rtva-019__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-rtva-019-lisa-cruz','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})

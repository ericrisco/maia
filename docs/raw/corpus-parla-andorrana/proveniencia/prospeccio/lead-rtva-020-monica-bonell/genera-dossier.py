"""Prepara el dossier de prospecció de Mònica Bonell fora del cànon."""
...
SMALL=json.loads((ASR/'monica-bonell-small-nocontext.json').read_text())
BASE=json.loads((ASR/'monica-bonell-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[
 ('ministra','institucions'),('ministeri','institucions'),('cultura','cultura'),('joventut','societat'),('esports','esport'),
 ('patrimoni','cultura'),('museu','cultura'),('biblioteca','cultura'),('arxiu','cultura'),('federació','esport'),
 ('futbol','esport'),('bàsquet','esport'),('pavelló','esport'),('pavelló Toni Martí','esport'),('Comitè Olímpic','esport'),
 ('Govern','institucions'),('Comú d’Andorra la Vella','institucions'),('Andorra la Vella','toponim'),
 ('La Massana','toponim'),('Santa Coloma','toponim'),('Andorra','toponim'),('andorrà','gentilici'),('andorrans','gentilici'),
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-rtva-020__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-rtva-020-monica-bonell','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})

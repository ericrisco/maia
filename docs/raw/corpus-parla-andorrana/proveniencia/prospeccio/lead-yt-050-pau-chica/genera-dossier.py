"""Prepara el dossier de prospecció de Pau Chica fora del cànon."""
...
SMALL=json.loads((ASR/'pau-chica-small-nocontext.json').read_text())
BASE=json.loads((ASR/'pau-chica-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[('historiador', 'professió'), ('història', 'cultura'), ('Andorra', 'toponim'), ('andorrà', 'gentilici'), ('Guerra Civil', 'història'), ('Segona Guerra Mundial', 'història'), ('arxiu', 'cultura'), ('document', 'cultura'), ('memòria', 'cultura'), ('testimoni', 'història'), ('població', 'societat'), ('habitants', 'societat'), ('exili', 'història'), ('frontera', 'territori'), ('francès', 'sociolingüística'), ('català', 'sociolingüística'), ('recerca', 'educació'), ('llibre', 'cultura')]
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-yt-050__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-yt-050-pau-chica','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})

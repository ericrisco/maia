"""Prepara el dossier de prospecció de David Montané fora del cànon."""
...
SMALL=json.loads((ASR/'david-montane-small-nocontext.json').read_text())
BASE=json.loads((ASR/'david-montane-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[
 ('pressupost','institucions'),('finances públiques','economia'),('sostenibilitat','economia'),('estabilitat','economia'),('fiscal','economia'),('govern','institucions'),('comuns','institucions'),('deute','economia'),('inversió','economia'),('administració','institucions'),('ciutadania','societat'),('comunitat internacional','societat'),('grup parlamentari','institucions'),('llei','institucions'),('cambra','institucions'),('Tribunal de Comptes','institucions'),('demòcrates','política'),('Tercera Via','política'),('Unió Lauredià','política'),('Andorra','toponim'),('andorrà','gentilici'),('català','sociolingüística'),('serveis','institucions'),('infraestructures','institucions'),('llavissada','territori'),('interventors','institucions'),('despesa','economia'),('tributària','economia'),('endeutament','economia'),('pressupostària','economia'),
]
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-yt-033__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-yt-033-david-montane','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})
    ...
    (ROOT/'informe.md').write_text(f'''# Expedient de prospecció — David Montané\n\n## Estat\n\n`lead-yt-033-david-montane` és una font candidata fora del recompte i dels grafs canònics. El vídeo del canal DEMOCRATESANDORRA presenta el primer discurs de David Montané al ple del Consell General; la fitxa es manté candidata fins a la revisió de veu i context. L'enregistrament conté principalment el discurs parlamentari i una breu intervenció posterior; cap clip s'atribueix automàticament a David Montané.\n\n## Procedència i fitxers\n\n- Font: [YouTube — Primer discurs de David Montané al ple del Consell General](https://www.youtube.com/watch?v=-Nn6jatvEAY).\n- Pàgina conservada: `source-page.html`; termes: `legal-page.html` i [`terms.md`](terms.md).\n- Flux original local: `source-original.mp4` (derivat local de recerca).\n- WAV de treball: `audio.wav`, mono, 16 kHz, {info['audio_wav']['duration_seconds']:.2f} s.\n- Hashes i metadades: `source.info.json`.\n\n## Transcripcions\n\nLa passada small sense context acumulat té {ss} segments fins a {sd/1000:.2f} s; la passada base independent té {bs} segments fins a {bd/1000:.2f} s. La small conté {len(low)} tokens per sota de 0,55 i {rep} repeticions consecutives exactes. Les dues sortides són ASR de treball i s'han de contrastar amb l'àudio.\n\n## Inventari i anàlisi provisional\n\n`formes.tsv` conserva {len(form_rows)} formes: les 35 formes comparables del paquet i {len(forms)-35} candidats institucionals, econòmics, polítics i sociolingüístics detectats en aquesta font. `clips/` conté {len(queue)} clips locals; `cua-audicio.tsv` manté veu, variant, fonètica, prosòdia i nota en `pendent`.\n\nEls candidats locals inclouen `pressupost`, `finances públiques`, `sostenibilitat`, `estabilitat`, `deute`, `inversió`, `administració`, `ciutadania`, `comuns`, `Tribunal de Comptes`, `llavissada` i `endeutament`. Són formes localitzades textualment per ASR, no variants dialectals confirmades.\n\nSeqüències lèxiques més repetides en la passada small (no són trets dialectals): {top}\n\nNo es publica cap tret fonètic, prosòdic ni dialectal com a confirmat. Qualsevol incorporació al cànon exigeix escolta clip per clip, atribució de veu i revisió dels termes d'ús.\n''',encoding='utf-8')
    print(f'OK dossier David Montané: {len(form_rows)} formes · {len(queue)} clips · {ss}/{bs} segments small/base')

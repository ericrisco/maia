"""Prepara el dossier de prospecció de Josep Dallarès fora del cànon."""
...
SMALL=json.loads((ASR/'josep-dalleres-small-nocontext.json').read_text())
BASE=json.loads((ASR/'josep-dalleres-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[
 ('síndic general','institucions'),('conseller','institucions'),('ambaixador','institucions'),
 ('ambaixada','institucions'),('ministre','institucions'),('cònsol','institucions'),
 ('Consell General','institucions'),('Constitució','institucions'),('Andorra','toponim'),
 ('andorrà','gentilici'),('andorrans','gentilici'),('parròquia','institucions'),
 ('Encamp','toponim'),('poesia','literatura'),('poeta','literatura'),
 ('novel·la','literatura'),('Frontera endins','literatura'),('llengua','sociolingüística'),
 ('català','sociolingüística'),('francès','contacte'),('castellà','contacte'),
 ('Pirineus','territori'),('principat','territori'),
]
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-rtva-011__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-rtva-011-josep-dalleres','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})

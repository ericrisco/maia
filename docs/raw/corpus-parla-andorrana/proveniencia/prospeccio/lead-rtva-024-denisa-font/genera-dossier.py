"""Prepara el dossier de prospecció de Denisa Font fora del cànon."""
...
SMALL=json.loads((ASR/'denisa-font-small-nocontext.json').read_text())
BASE=json.loads((ASR/'denisa-font-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[
 ('tradició','cultura'),('tradicions','cultura'),('història','memòria'),('parròquia','toponim'),('Canillo','toponim'),('casa','societat'),('família','societat'),('poble','societat'),('costum','cultura'),('costums','cultura'),('article','cultura'),('llibre','cultura'),('escriptor','cultura'),('Andorra','toponim'),('cultura','cultura'),('joventut','societat'),('esports','esport'),
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-rtva-024__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-rtva-024-denisa-font','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})

"""Prepara el dossier de prospecció de Guillem Forné fora del cànon."""
...
SMALL=json.loads((ASR/'guillem-forne-small-nocontext.json').read_text())
BASE=json.loads((ASR/'guillem-forne-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[
 ('Guillem Forné','persona'),('La Massana','toponim'),('Massana','toponim'),('conseller','institucions'),('conseller de Cultura','institucions'),('cultura','cultura'),('parròquia','institucions'),('comú','institucions'),('Andorra','toponim'),('andorrà','gentilici'),('Andorra la Vella','toponim'),('Pirineus TV','mitjans'),('La Vida','mitjans'),('AndoFlora','cultura'),('flors','medi ambient'),('floral','cultura'),('fira','cultura'),('mercat','economia'),('tradició','cultura'),('viles florides','cultura'),('Catalunya','toponim'),('canalla','societat'),('infants','societat'),('nens','societat'),('teatre','cultura'),('literatura','cultura'),('llibres','cultura'),('lectura','educació'),('dansa','cultura'),('escena nacional','cultura'),('restauradors','economia'),('gastronomia','economia'),('producte','economia'),('calçots','gastronomia'),('bolets','gastronomia'),('boleros','cultura'),('Sant Julià','toponim'),('climatologia','medi ambient'),('hotels','economia'),
]
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-yt-040__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-yt-040-guillem-forne','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})
    ...
    (ROOT/'informe.md').write_text(f'''# Expedient de prospecció — Guillem Forné\n\n## Estat\n\n`lead-yt-040-guillem-forne` és una font candidata fora del recompte i dels grafs canònics. El vídeo del canal Sergi Mas conserva una entrevista amb Guillem Forné, conseller de Cultura de La Massana, dins el programa La Vida de Pirineus TV; la fitxa es manté candidata fins a la revisió de veu i context. L'enregistrament conté preguntes dels entrevistadors i la veu principal de Guillem Forné; cap clip s'atribueix automàticament a Guillem Forné.\n\n## Procedència i fitxers\n\n- Font: [YouTube — Entrevista a GUILLEM FORNÉ. Conseller de Cultura de La Massana](https://www.youtube.com/watch?v=I1QZRB8A5Y4).\n- Pàgina conservada: `source-page.html`; termes: `legal-page.html` i [`terms.md`](terms.md).\n- Flux original local: `source-original.mp4` (derivat local de recerca).\n- WAV de treball: `audio.wav`, mono, 16 kHz, {info['audio_wav']['duration_seconds']:.2f} s.\n- Hashes i metadades: `source.info.json`.\n\n## Transcripcions\n\nLa passada small sense context acumulat té {ss} segments fins a {sd/1000:.2f} s; la passada base independent té {bs} segments fins a {bd/1000:.2f} s. La small conté {len(low)} tokens per sota de 0,55 i {rep} repeticions consecutives exactes. Les dues sortides són ASR de treball i s'han de contrastar amb l'àudio.\n\n## Inventari i anàlisi provisional\n\n`formes.tsv` conserva {len(form_rows)} formes: les 35 formes comparables del paquet i {len(forms)-35} candidats històrics, institucionals, territorials, ambientals i sociolingüístics detectats en aquesta font. `clips/` conté {len(queue)} clips locals; `cua-audicio.tsv` manté veu, variant, fonètica, prosòdia i nota en `pendent`.\n\nEls candidats locals inclouen `Eurovisió`, `Guillem Forné`, `cantant`, `cançó`, `català`, `llengua`, `representant`, `festival`, `TV3`, `televisió`, `Istanbul`, `Turquia`, `microestat`, `emigració`, `família` i `pandèmia`. Són formes localitzades textualment per ASR, no variants dialectals confirmades.\n\nSeqüències lèxiques més repetides en la passada small (no són trets dialectals): {top}\n\nNo es publica cap tret fonètic, prosòdic ni dialectal com a confirmat. Qualsevol incorporació al cànon exigeix escolta clip per clip, atribució de veu i revisió dels termes d'ús.\n''',encoding='utf-8')
    print(f'OK dossier Guillem Forné: {len(form_rows)} formes · {len(queue)} clips · {ss}/{bs} segments small/base')

"""Prepara el dossier de prospecció de Guillem Areny fora del cànon."""
...
SMALL=json.loads((ASR/'guillem-areny-small-nocontext.json').read_text())
BASE=json.loads((ASR/'guillem-areny-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[
 ('Guillem Areny','persona'),('La Massana','toponim'),('Massana','toponim'),('aiguat','medi ambient'),('aiguats','medi ambient'),('inundacions','medi ambient'),('1982','història'),('Armeria Andorrana','economia'),('armes','economia'),('Avinguda Meritxell','toponim'),('Hiper Andorra','economia'),('pont','infraestructures'),('Prisunic','economia'),('explosió','emergència'),('gas','emergència'),('evacuació','emergència'),('víctimes','societat'),('agent de circulació','institucions'),('vehicles','transport'),('carreteres','infraestructures'),('Hotel La Massana','economia'),('turistes','turisme'),('Manel Ribas','persona'),('Andorra','toponim'),('andorrà','gentilici'),('solidaritat','societat'),('emergència','institucions'),('botiga','economia'),('incendi','emergència'),('pont del Prisunic','toponim'),
]
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-yt-041__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-yt-041-guillem-areny','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})

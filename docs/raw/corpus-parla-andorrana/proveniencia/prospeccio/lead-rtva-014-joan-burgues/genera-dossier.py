"""Prepara el dossier de prospecció de Joan Burgués fora del cànon."""
...
SMALL=json.loads((ASR/'joan-burgues-small-nocontext.json').read_text())
BASE=json.loads((ASR/'joan-burgues-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[
 ('fotografia','cultura'),('fotògraf','cultura'),('cineasta','cultura'),('cinema','cultura'),
 ('arts','cultura'),('lletres','cultura'),('cultura','cultura'),('Cercle de les Arts','institucions'),
 ('Andorra','toponim'),('andorrà','gentilici'),('andorrans','gentilici'),('parròquia','institucions'),
 ('Encamp','toponim'),('Ordino','toponim'),('muntanya','territori'),('Pirineus','territori'),
 ('català','sociolingüística'),('francès','contacte'),('castellà','contacte'),
 ('exposició','cultura'),('patrimoni','cultura'),('tradició','cultura'),
]
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-rtva-014__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-rtva-014-joan-burgues','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})

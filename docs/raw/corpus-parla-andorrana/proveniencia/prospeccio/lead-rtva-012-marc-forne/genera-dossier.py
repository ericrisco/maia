"""Prepara el dossier de prospecció de Marc Forné fora del cànon."""
...
SMALL=json.loads((ASR/'marc-forne-small-nocontext.json').read_text())
BASE=json.loads((ASR/'marc-forne-base-nocontext.json').read_text())
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-rtva-012__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-rtva-012-marc-forne','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})

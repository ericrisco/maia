"""Prepara el dossier de prospecció de Xavier Espot fora del cànon."""
...
SMALL=json.loads((ASR/'xavier-espot-small-nocontext.json').read_text())
BASE=json.loads((ASR/'xavier-espot-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[
 ('Demòcrates','política'),('habitatge','societat'),('habitatges','societat'),('lloguer','economia'),('poder adquisitiu','economia'),('creixement urbanístic','territori'),('Museu Nacional','cultura'),('cultura','cultura'),('justícia','institucions'),('medi ambient','territori'),('política exterior','institucions'),('Unió Europea','institucions'),("acord d'associació",'institucions'),('França','toponim'),('Espanya','toponim'),('pandèmia','societat'),('inversions','economia'),('ciutadans','societat'),('Andorra','toponim'),('andorrà','gentilici'),('català','sociolingüística'),('llengua','sociolingüística'),('territori','territori'),('immigració','societat'),('joventut','societat'),('educació','societat'),('turisme','economia'),('construcció','economia'),('sostenibilitat','economia'),('pressupost','economia'),('fiscal','economia'),('comuns','institucions'),('Constitució','institucions'),('relat nacional','cultura'),('serveis','institucions'),('sanitat','societat'),
]
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-yt-036__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-yt-036-xavier-espot','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})
    ...
    (ROOT/'informe.md').write_text(f'''# Expedient de prospecció — Xavier Espot\n\n## Estat\n\n`lead-yt-036-xavier-espot` és una font candidata fora del recompte i dels grafs canònics. El vídeo del canal Dona Secret presenta una entrevista amb Xavier Espot, cap de Govern en funcions i cap de llista de Demòcrates per Andorra; la fitxa es manté candidata fins a la revisió de veu i context. L'enregistrament conté preguntes dels entrevistadors i la veu principal de Xavier Espot; cap clip s'atribueix automàticament a Xavier Espot.\n\n## Procedència i fitxers\n\n- Font: [YouTube — Especial eleccions 2023: entrevista a Xavier Espot](https://www.youtube.com/watch?v=MhGYlCSdtQ4).\n- Pàgina conservada: `source-page.html`; termes: `legal-page.html` i [`terms.md`](terms.md).\n- Flux original local: `source-original.mp4` (derivat local de recerca).\n- WAV de treball: `audio.wav`, mono, 16 kHz, {info['audio_wav']['duration_seconds']:.2f} s.\n- Hashes i metadades: `source.info.json`.\n\n## Transcripcions\n\nLa passada small sense context acumulat té {ss} segments fins a {sd/1000:.2f} s; la passada base independent té {bs} segments fins a {bd/1000:.2f} s. La small conté {len(low)} tokens per sota de 0,55 i {rep} repeticions consecutives exactes. Les dues sortides són ASR de treball i s'han de contrastar amb l'àudio.\n\n## Inventari i anàlisi provisional\n\n`formes.tsv` conserva {len(form_rows)} formes: les 35 formes comparables del paquet i {len(forms)-35} candidats polítics, institucionals, econòmics, territorials i sociolingüístics detectats en aquesta font. `clips/` conté {len(queue)} clips locals; `cua-audicio.tsv` manté veu, variant, fonètica, prosòdia i nota en `pendent`.\n\nEls candidats locals inclouen `Demòcrates`, `habitatge`, `lloguer`, `poder adquisitiu`, `creixement urbanístic`, `Museu Nacional`, `Unió Europea`, `acord d'associació`, `política exterior`, `medi ambient`, `cultura` i `justícia`. Són formes localitzades textualment per ASR, no variants dialectals confirmades.\n\nSeqüències lèxiques més repetides en la passada small (no són trets dialectals): {top}\n\nNo es publica cap tret fonètic, prosòdic ni dialectal com a confirmat. Qualsevol incorporació al cànon exigeix escolta clip per clip, atribució de veu i revisió dels termes d'ús.\n''',encoding='utf-8')
    print(f'OK dossier Xavier Espot: {len(form_rows)} formes · {len(queue)} clips · {ss}/{bs} segments small/base')

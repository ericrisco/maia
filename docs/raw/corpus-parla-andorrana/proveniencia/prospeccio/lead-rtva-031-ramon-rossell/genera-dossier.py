"""Prepara el dossier de prospecció de Mossèn Ramon Rossell fora del cànon."""
...
SMALL=json.loads((ASR/'ramon-rossell-small-nocontext.json').read_text())
BASE=json.loads((ASR/'ramon-rossell-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[
 ('educació','societat'),('escola','societat'),('història','memòria'),('cultura','cultura'),('patrimoni','cultura'),('tradició','cultura'),('parròquia','toponim'),('poble','societat'),('família','societat'),('muntanya','territori'),('paisatge','territori'),('memòria','memòria'),('Andorra','toponim'),('església','religió'),('capellà','religió'),('parròquia','religió'),('fe','religió'),('cultura','cultura'),('joventut','societat'),('esports','esport'),
 ('patrimoni','cultura'),('museu','cultura'),('biblioteca','cultura'),('arxiu','cultura'),('federació','esport'),('ministres','institucions'),('Govern','institucions'),('conseller','institucions'),('consell','institucions'),
 ('futbol','esport'),('bàsquet','esport'),('pavelló','esport'),('pavelló Toni Martí','esport'),('Comitè Olímpic','esport'),
 ('Comú d’Andorra la Vella','institucions'),('Andorra la Vella','toponim'),
 ('La Massana','toponim'),('Santa Coloma','toponim'),('Andorra','toponim'),('parròquia','toponim'),('família','societat'),('andorrà','gentilici'),('andorrans','gentilici'),
 ('projecte','institucions'),('concessió','institucions'),('estadi','esport'),('esdeveniment','cultura'),
 ('premis','cultura'),('Toni Martí','memòria'),('jocs','esport'),('festa','cultura'),('equip','esport'),
 ('català','sociolingüística'),('francès','contacte'),('castellà','contacte'),('hipermotion','esport'),
 ('lliga','esport'),('comissió legislativa','institucions'),('pavelló','institucions'),
]
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-rtva-031__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-rtva-031-ramon-rossell','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})
    write_tsv(ROOT/'formes.tsv',form_rows,list(form_rows[0]))
    write_tsv(ROOT/'cua-audicio.tsv',queue,list(queue[0]))
    def sha(p):
        h=hashlib.sha256();
        with p.open('rb') as f:
            for b in iter(lambda:f.read(1031*1031),b''):h.update(b)
        return h.hexdigest()
    info=json.loads((ROOT/'source.info.json').read_text()); info['data_consulta']=str(date.today());info['asr_small']='asr/ramon-rossell-small-nocontext.json';info['asr_base']='asr/ramon-rossell-base-nocontext.json';info['forms_count']=len(form_rows);info['clips_count']=len(queue);info['hashes']={x:sha(ROOT/x) for x in ['source-page.html','legal-page.html','source-original.mp4','audio.wav']};(ROOT/'source.info.json').write_text(json.dumps(info,ensure_ascii=False,indent=2)+'\n')
    ...
    (ROOT/'informe.md').write_text(f'''# Expedient de prospecció — Mossèn Ramon Rossell\n\n## Estat\n\n`lead-rtva-031-ramon-rossell` és una font candidata fora del recompte i dels grafs canònics. La pàgina de RTVA presenta Mossèn Ramon Rossell com a veu de la sèrie *Memòria d’un país*; la fitxa es manté candidata fins a la revisió de veu i context. El programa conserva com a mínim la veu de la presentadora i la del convidat; cap clip s'atribueix automàticament a Mossèn Ramon Rossell.\n\n## Procedència i fitxers\n\n- Font: [RTVA — Mossèn Ramon Rossell, entrevista d'actualitat](https://www.rtva.ad/programes/mossen-ramon-rossell-080120249).\n- Pàgina conservada: `source-page.html`; termes: `legal-page.html` i [`terms.md`](terms.md).\n- Flux original local: `source-original.mp4` (derivat local de recerca).\n- WAV de treball: `audio.wav`, mono, 16 kHz, {info['audio_wav']['duration_seconds']:.2f} s.\n- Hashes i metadades: `source.info.json`.\n\n## Transcripcions\n\nLa passada small sense context acumulat té {ss} segments fins a {sd/1000:.2f} s; la passada base independent té {bs} segments fins a {bd/1000:.2f} s. La small conté {len(low)} tokens per sota de 0,55 i {rep} repeticions consecutives exactes. Les dues sortides són ASR de treball i s'han de contrastar amb l'àudio.\n\n## Inventari i anàlisi provisional\n\n`formes.tsv` conserva {len(form_rows)} formes: les 35 formes comparables del paquet i {len(forms)-35} candidats lèxics, sociolingüístics, toponímics i de pagesia detectats en aquesta font. `clips/` conté {len(queue)} clips locals; `cua-audicio.tsv` manté veu, variant, fonètica, prosòdia i nota en `pendent`.\n\nEls candidats locals inclouen `educació`, `escola`, `història`, `cultura`, `patrimoni`, `tradició`, `parròquia`, `poble`, `família`, `muntanya`, `paisatge`, `església`, `capellà` i `fe`. Són formes localitzades textualment per ASR, no variants dialectals confirmades.\n\nSeqüències lèxiques més repetides en la passada small (no són trets dialectals): {top}\n\nNo es publica cap tret fonètic, prosòdic ni dialectal com a confirmat. Qualsevol incorporació al cànon exigeix escolta clip per clip, atribució de veu i revisió dels termes d'ús.\n''',encoding='utf-8')
    print(f'OK dossier Mossèn Ramon Rossell: {len(form_rows)} formes · {len(queue)} clips · {ss}/{bs} segments small/base')

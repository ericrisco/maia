"""Prepara el dossier de prospecció de Arnau Fortuny fora del cànon."""
...
SMALL=json.loads((ASR/'arnau-fortuny-small-nocontext.json').read_text())
BASE=json.loads((ASR/'arnau-fortuny-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[('Arnau', 'persona'), ('emprenedor', 'economia'), ('emprenedoria', 'economia'), ('empresa', 'economia'), ('negoci', 'economia'), ('projecte', 'economia'), ('inversió', 'economia'), ('tecnologia', 'tecnologia'), ('digital', 'tecnologia'), ('jocs', 'tecnologia'), ('videojocs', 'tecnologia'), ('Andorra', 'toponim'), ('andorrà', 'gentilici'), ('Barcelona', 'toponim'), ('Catalunya', 'toponim'), ('treball', 'societat'), ('futur', 'societat')]
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if clip: queue.append({'candidate':'lead-yt-056-arnau-fortuny','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})

"""Prepara el dossier de prospecció de Pere Vilanova fora del cànon."""
...
SMALL=json.loads((ASR/'pere-vilanova-small-nocontext.json').read_text())
BASE=json.loads((ASR/'pere-vilanova-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[
 ('magistrat','institucions'),('Tribunal Constitucional','institucions'),('Constitució','institucions'),
 ('Consell General','institucions'),('coprincipat','institucions'),('principat','territori'),
 ('Andorra','toponim'),('andorrà','gentilici'),('andorrans','gentilici'),('parròquia','institucions'),
 ('jurista','institucions'),('dret','institucions'),('ciència política','acadèmia'),
 ('administració','institucions'),('universitat','acadèmia'),('magistratura','institucions'),
 ('català','sociolingüística'),('francès','contacte'),('castellà','contacte'),
 ('Pirineus','territori'),('Constitució andorrana','institucions'),('sobirania','institucions'),
]
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-rtva-013__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-rtva-013-pere-vilanova','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})

"""Prepara el dossier de prospecció de Conxita Marsol fora del cànon."""
...
SMALL=json.loads((ASR/'conxita-marsol-small-nocontext.json').read_text())
BASE=json.loads((ASR/'conxita-marsol-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[
 ('Conxita Marsol','persona'),('cònsol','institucions'),('cònsol major','institucions'),('Andorra la Vella','toponim'),('parròquia','institucions'),('comú','institucions'),('comuns','institucions'),('consellera general','institucions'),('presidenta','institucions'),('grup parlamentari','institucions'),('ministra','institucions'),('govern','institucions'),('política','institucions'),('liberal','institucions'),('turisme','economia'),('comerç','economia'),('comercial','economia'),('Europa','toponim'),('referent','cultura'),('orgull','societat'),('família','societat'),('cultura','cultura'),('educació','societat'),('escola','societat'),('infants','societat'),('nens','societat'),('Andorra','toponim'),('andorrà','gentilici'),('català','sociolingüística'),('llengua','sociolingüística'),('ciutadans','societat'),('serveis','institucions'),('dret','institucions'),('Lleida','toponim'),('Noguera','toponim'),('Artesa de Segre','toponim'),
]
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-yt-038__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-yt-038-conxita-marsol','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})

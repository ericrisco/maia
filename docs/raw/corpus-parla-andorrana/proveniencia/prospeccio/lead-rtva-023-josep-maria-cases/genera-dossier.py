"""Prepara el dossier de prospecció de Josep Maria Cases fora del cànon."""
...
SMALL=json.loads((ASR/'josep-maria-cases-small-nocontext.json').read_text())
BASE=json.loads((ASR/'josep-maria-cases-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[
 ('empresari','economia'),('empresa','economia'),('empresarial','economia'),('economia','economia'),('comerç','economia'),('negoci','economia'),('inversió','economia'),('família','societat'),('successió','societat'),('president','institucions'),('Govern','institucions'),('Andorra','toponim'),('cultura','cultura'),('joventut','societat'),('esports','esport'),
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-rtva-023__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-rtva-023-josep-maria-cases','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})

"""Prepara el dossier de prospecció de Pere Besolí fora del cànon."""
...
SMALL=json.loads((ASR/'pere-besoli-small-nocontext.json').read_text())
BASE=json.loads((ASR/'pere-besoli-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[
 ('educació','societat'),('escola','societat'),('història','memòria'),('cultura','cultura'),('patrimoni','cultura'),('empresa','economia'),('empresari','economia'),('comerç','economia'),('muntanya','territori'),('poble','societat'),('família','societat'),('tradició','cultura'),('parròquia','toponim'),('Andorra','toponim'),('cultura','cultura'),('joventut','societat'),('esports','esport'),
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-rtva-028__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-rtva-028-pere-besoli','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})
    write_tsv(ROOT/'formes.tsv',form_rows,list(form_rows[0]))
    write_tsv(ROOT/'cua-audicio.tsv',queue,list(queue[0]))
    def sha(p):
        h=hashlib.sha256();
        with p.open('rb') as f:
            for b in iter(lambda:f.read(1028*1028),b''):h.update(b)
        return h.hexdigest()
    info=json.loads((ROOT/'source.info.json').read_text()); info['data_consulta']=str(date.today());info['asr_small']='asr/pere-besoli-small-nocontext.json';info['asr_base']='asr/pere-besoli-base-nocontext.json';info['forms_count']=len(form_rows);info['clips_count']=len(queue);info['hashes']={x:sha(ROOT/x) for x in ['source-page.html','legal-page.html','source-original.mp4','audio.wav']};(ROOT/'source.info.json').write_text(json.dumps(info,ensure_ascii=False,indent=2)+'\n')

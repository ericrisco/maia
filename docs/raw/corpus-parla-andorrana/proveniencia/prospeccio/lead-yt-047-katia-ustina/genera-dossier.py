"""Prepara el dossier de prospecció de Katia Ustina fora del cànon."""
...
SMALL=json.loads((ASR/'katia-ustina-small-nocontext.json').read_text())
BASE=json.loads((ASR/'katia-ustina-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[('diners', 'economia'), ('finances', 'economia'), ('economia', 'economia'), ('mercat', 'economia'), ('banca', 'economia'), ('inversió', 'economia'), ('empresa', 'economia'), ('emprenedoria', 'economia'), ('dones', 'societat'), ('femenina', 'societat'), ('Andorra', 'toponim'), ('andorrà', 'gentilici'), ('treball', 'societat'), ('educació', 'educació'), ('família', 'societat'), ('futur', 'societat')]
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if clip: queue.append({'candidate':'lead-yt-047-katia-ustina','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})

"""Prepara el dossier de prospecció de Arnau Rius fora del cànon."""
...
SMALL=json.loads((ASR/'arnau-rius-small-nocontext.json').read_text())
BASE=json.loads((ASR/'arnau-rius-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[('arquitecte', 'professió'), ('arquitectura', 'professió'), ('edifici', 'infraestructures'), ('edificis', 'infraestructures'), ('fusta', 'materials'), ('obra', 'infraestructures'), ('paisatge', 'territori'), ('muntanya', 'territori'), ('habitatge', 'societat'), ('casa', 'infraestructures'), ('Andorra', 'toponim'), ('andorrà', 'gentilici'), ('La Massana', 'toponim'), ('materials', 'infraestructures'), ('disseny', 'cultura'), ('natura', 'medi ambient'), ('sostenibilitat', 'medi ambient'), ('turisme', 'economia')]
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-yt-045__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-yt-045-arnau-rius','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})

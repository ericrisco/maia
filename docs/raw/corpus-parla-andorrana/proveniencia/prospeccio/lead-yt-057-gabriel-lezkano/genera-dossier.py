"""Prepara el dossier de prospecció de Gabriel Lezkano fora del cànon."""
...
SMALL=json.loads((ASR/'gabriel-lezkano-small-nocontext.json').read_text())
BASE=json.loads((ASR/'gabriel-lezkano-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[('Gabriel', 'persona'), ('músic', 'cultura'), ('música', 'cultura'), ('cor', 'cultura'), ('concert', 'cultura'), ('cantant', 'cultura'), ('Galícia', 'toponim'), ('gallec', 'gentilici'), ('Barcelona', 'toponim'), ('Suècia', 'toponim'), ('Andorra', 'toponim'), ('andorrà', 'gentilici'), ('cultura', 'cultura'), ('lleure', 'societat'), ('família', 'societat'), ('treball', 'societat'), ('oportunitat', 'societat')]
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if clip: queue.append({'candidate':'lead-yt-057-gabriel-lezkano','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})

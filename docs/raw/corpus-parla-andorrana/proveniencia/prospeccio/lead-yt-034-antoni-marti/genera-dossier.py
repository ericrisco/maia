"""Prepara el dossier de prospecció de Antoni Martí Petit fora del cànon."""
...
SMALL=json.loads((ASR/'antoni-marti-small-nocontext.json').read_text())
BASE=json.loads((ASR/'antoni-marti-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[
 ('transparència','institucions'),('obertura econòmica','economia'),('sistema fiscal','economia'),('impostos','economia'),('copríncep','institucions'),('coprincipat','institucions'),('comunitat internacional','societat'),('blanqueig de diners','economia'),('terrorisme','societat'),('intercanvi d’informació','institucions'),('banca','economia'),('solvència','economia'),('Unió Europea','institucions'),('lliure circulació','institucions'),('joventut','societat'),('aeroport','infraestructura'),('la Seu d’Urgell','toponim'),('Catalunya','toponim'),('Espanya','toponim'),('Andorra','toponim'),('andorrà','gentilici'),('català','sociolingüística'),('govern','institucions'),('comuns','institucions'),('empreses','economia'),('inversions','economia'),('constitució','institucions'),('unió civil','societat'),('avortament','societat'),('territori','territori'),('pirineu','territori'),('serveis','institucions'),('reforma','institucions'),('deute','economia'),('pressupost','economia'),
]
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-yt-034__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-yt-034-antoni-marti','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})
    ...
    (ROOT/'informe.md').write_text(f'''# Expedient de prospecció — Antoni Martí Petit\n\n## Estat\n\n`lead-yt-034-antoni-marti` és una font candidata fora del recompte i dels grafs canònics. El vídeo del canal 3Cat presenta una entrevista d'Els matins amb Antoni Martí Petit, cap de Govern d'Andorra; la fitxa es manté candidata fins a la revisió de veu i context. L'enregistrament conté preguntes de la presentadora i la veu principal d'Antoni Martí; cap clip s'atribueix automàticament a Antoni Martí Petit.\n\n## Procedència i fitxers\n\n- Font: [YouTube — TV3 - Els Matins - Antoni Martí Petit](https://www.youtube.com/watch?v=WAN5vuqAOhQ).\n- Pàgina conservada: `source-page.html`; termes: `legal-page.html` i [`terms.md`](terms.md).\n- Flux original local: `source-original.mp4` (derivat local de recerca).\n- WAV de treball: `audio.wav`, mono, 16 kHz, {info['audio_wav']['duration_seconds']:.2f} s.\n- Hashes i metadades: `source.info.json`.\n\n## Transcripcions\n\nLa passada small sense context acumulat té {ss} segments fins a {sd/1000:.2f} s; la passada base independent té {bs} segments fins a {bd/1000:.2f} s. La small conté {len(low)} tokens per sota de 0,55 i {rep} repeticions consecutives exactes. Les dues sortides són ASR de treball i s'han de contrastar amb l'àudio.\n\n## Inventari i anàlisi provisional\n\n`formes.tsv` conserva {len(form_rows)} formes: les 35 formes comparables del paquet i {len(forms)-35} candidats institucionals, econòmics, territorials i sociolingüístics detectats en aquesta font. `clips/` conté {len(queue)} clips locals; `cua-audicio.tsv` manté veu, variant, fonètica, prosòdia i nota en `pendent`.\n\nEls candidats locals inclouen `transparència`, `sistema fiscal`, `impostos`, `copríncep`, `comunitat internacional`, `banca`, `Unió Europea`, `lliure circulació`, `aeroport`, `la Seu d'Urgell`, `andorrà` i `Constitució`. Són formes localitzades textualment per ASR, no variants dialectals confirmades.\n\nSeqüències lèxiques més repetides en la passada small (no són trets dialectals): {top}\n\nNo es publica cap tret fonètic, prosòdic ni dialectal com a confirmat. Qualsevol incorporació al cànon exigeix escolta clip per clip, atribució de veu i revisió dels termes d'ús.\n''',encoding='utf-8')
    print(f'OK dossier Antoni Martí Petit: {len(form_rows)} formes · {len(queue)} clips · {ss}/{bs} segments small/base')

"""Prepara el dossier de prospecció de Sonia fora del cànon."""
...
SMALL=json.loads((ASR/'sonia-andorrita-small-nocontext.json').read_text())
BASE=json.loads((ASR/'sonia-andorrita-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[('Sonia', 'persona'), ('Andorrita', 'societat'), ('comerç', 'economia'), ('botiga', 'economia'), ('productes', 'economia'), ('clients', 'economia'), ('serveis', 'societat'), ('Andorra', 'toponim'), ('andorrà', 'gentilici'), ('empresa', 'economia'), ('emprenedoria', 'economia'), ('economia', 'economia'), ('comunitat', 'societat'), ('persones', 'societat'), ('solidària', 'societat'), ('associacions', 'societat'), ('inscripció', 'societat')]
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-yt-053-sonia-andorrita__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-yt-053-sonia-andorrita-sonia-andorrita','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})

"""Prepara el dossier de prospecció de Bonaventura Riberaygua fora del cànon."""
...
SMALL=json.loads((ASR/'bonaventura-riberaygua-small-nocontext.json').read_text())
BASE=json.loads((ASR/'bonaventura-riberaygua-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[
 ('empresari','economia'),('empresa','economia'),('ministre','institucions'),('ministeri','institucions'),('economia','economia'),('comerç','economia'),('negoci','economia'),('inversió','economia'),('construcció','economia'),('turisme','economia'),('hotel','economia'),('cultura','cultura'),('joventut','societat'),('esports','esport'),
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-rtva-021__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-rtva-021-bonaventura-riberaygua','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})

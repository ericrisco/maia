"""Prepara el dossier de prospecció de Isidre Baró fora del cànon."""
...
SMALL=json.loads((ASR/'isidre-baro-small-nocontext.json').read_text())
BASE=json.loads((ASR/'isidre-baro-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[
 ('olimpisme','esport'),('olímpic','esport'),('olímpica','esport'),('esport','esport'),
 ('Comitè Olímpic Andorrà','institucions'),('Comitè Olímpic','institucions'),('jocs olímpics','esport'),
 ...
 ('esquí','esport'),('federació','esport'),('president','institucions'),('honorari','institucions'),
]
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-rtva-015__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-rtva-015-isidre-baro','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})

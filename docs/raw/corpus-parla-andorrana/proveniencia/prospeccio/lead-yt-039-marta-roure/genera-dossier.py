"""Prepara el dossier de prospecció de Marta Roure fora del cànon."""
...
SMALL=json.loads((ASR/'marta-roure-small-nocontext.json').read_text())
BASE=json.loads((ASR/'marta-roure-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[
 ('Eurovisió','cultura'),('Eurovision','cultura'),('Marta Roure','persona'),('cantant','cultura'),('cançó','cultura'),('català','sociolingüística'),('llengua','sociolingüística'),('idioma','sociolingüística'),('Andorra','toponim'),('andorrà','gentilici'),('andorrans','gentilici'),('andorranes','gentilici'),('representant','cultura'),('festival','cultura'),('TV3','mitjans'),('televisió','mitjans'),('Istanbul','toponim'),('Turquia','toponim'),('Catalunya','toponim'),('francès','sociolingüística'),('castellà','sociolingüística'),('anglès','sociolingüística'),('artista','cultura'),('escenari','cultura'),('semifinal','cultura'),('països','toponim'),('microestat','institucions'),('emigració','societat'),('França','toponim'),('Espanya','toponim'),('Portugal','toponim'),('família','societat'),('fills','societat'),('universitat','educació'),('advocats','professió'),('pandèmia','societat'),
]
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-yt-039__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-yt-039-marta-roure','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})

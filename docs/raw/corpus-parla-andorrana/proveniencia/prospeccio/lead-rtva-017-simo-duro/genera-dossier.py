"""Prepara el dossier de prospecció de Simó Duró fora del cànon."""
...
SMALL=json.loads((ASR/'simo-duro-small-nocontext.json').read_text())
BASE=json.loads((ASR/'simo-duro-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[
 ('mestre','educació'),('escola','educació'),('escola francesa','educació'),('Ciutat de Valls','toponim'),
 ('ramader','pagesia'),('ramaderia','pagesia'),('bestiar','pagesia'),('camp','territori'),('casa','parentiu'),
 ('padrí','parentiu'),('Andorra','toponim'),('andorrà','gentilici'),('andorrants','gentilici'),
 ('Ordino','toponim'),('La Massana','toponim'),('Escaldes','toponim'),('Catedral d’Andorra','institucions'),
 ('conseller general','institucions'),('conseller','institucions'),('Consell General','institucions'),
 ('copríncep','institucions'),('coprínceps','institucions'),('coprincipat','institucions'),
 ('nacionalitat','institucions'),('jus sanguinis','dret'),('jus soli','dret'),('delegat permanent','institucions'),
 ('folklòric','cultura'),('cor de cambra','cultura'),('Cercle de les Arts i de les Lletres','cultura'),
 ('català','sociolingüística'),('francès','contacte'),('castellà','contacte'),('mos','morfosintaxi'),
 ('poble','territori'),('parròquia','institucions'),('comú','institucions'),('Constitució','institucions'),
 ('avortament','societat'),('gent gran','societat'),('educació especial','educació'),
]
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-rtva-017__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-rtva-017-simo-duro','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})

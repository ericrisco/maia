"""Prepara el dossier de prospecció de Josep Areny fora del cànon."""
...
SMALL=json.loads((ASR/'josep-areny-small-nocontext.json').read_text())
BASE=json.loads((ASR/'josep-areny-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[
 ('hotel','turisme'),('hoteler','turisme'),('hostal','turisme'),('turisme','turisme'),
 ('Cal Calbó','toponim'),('Calvó','toponim'),('Soldeu','toponim'),('Canillo','toponim'),
 ('borda','territori'),('ramaderia','pagesia'),('pastura','pagesia'),('padrí','parentiu'),('casa pairal','parentiu'),
 ('esquí','esport'),('estació d’esquí','esport'),('neu','territori'),('Comú','institucions'),
 ('cònsol','institucions'),('conseller general','institucions'),('Constitució','institucions'),
 ('Andorra','toponim'),('andorrà','gentilici'),('parròquia','institucions'),('muntanya','territori'),
 ('català','sociolingüística'),('francès','contacte'),('castellà','contacte'),
 ('patrimoni','cultura'),('família','parentiu'),('germans','parentiu'),
]
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-rtva-016__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-rtva-016-josep-areny','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})

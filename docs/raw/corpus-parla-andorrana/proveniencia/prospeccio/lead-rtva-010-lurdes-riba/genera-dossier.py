"""Prepara el dossier de prospecció de Lurdes Riba fora del cànon."""
...
SMALL=json.loads((ASR/'lurdes-riba-small-nocontext.json').read_text())
BASE=json.loads((ASR/'lurdes-riba-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[
 ('macarulla','lexic local'),('resclar','lexic pagesia'),('tronfès','lexic pagesia'),
 ('pagesia','lexic local'),('ramaderia','lexic local'),('xarcuteria','lexic local'),
 ('matança','lexic pagesia'),('padrí','parentiu'),('parròquia','institucions'),
 ('Ordino','toponim'),('Andorra','toponim'),('andorrà','gentilici'),('prats','pagesia'),
 ('filigranes','metàfora'),('a l’estiu','temps'),('a l\'estiu','temps'),
 ('gent gran','sociolingüística'),('parella lingüística','sociolingüística'),
 ('escola andorrana','educació'),('barbarismes','sociolingüística'),
 ('cuc','metàfora'),('cònsul','institucions'),('conseller general','institucions'),
]
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-rtva-010__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-rtva-010-lurdes-riba','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})

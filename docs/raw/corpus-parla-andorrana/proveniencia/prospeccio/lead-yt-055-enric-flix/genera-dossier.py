"""Prepara el dossier de prospecció de Enric Flix fora del cànon."""
...
SMALL=json.loads((ASR/'enric-flix-small-nocontext.json').read_text())
BASE=json.loads((ASR/'enric-flix-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[('empresari', 'economia'), ('empresa', 'economia'), ('negoci', 'economia'), ('emprenedor', 'economia'), ('emprenedoria', 'economia'), ('televisió', 'cultura'), ('tele', 'cultura'), ('TDT', 'cultura'), ('programa', 'cultura'), ('esport', 'esport'), ('Barcelona', 'toponim'), ('Andorra', 'toponim'), ('andorrà', 'gentilici'), ('família', 'societat'), ('treball', 'societat'), ('internet', 'tecnologia'), ('mòbil', 'tecnologia')]
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if clip: queue.append({'candidate':'lead-yt-055-enric-flix','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})

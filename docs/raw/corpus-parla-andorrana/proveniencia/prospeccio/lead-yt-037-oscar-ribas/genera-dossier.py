"""Prepara el dossier de prospecció de Òscar Ribas fora del cànon."""
...
SMALL=json.loads((ASR/'oscar-ribas-small-nocontext.json').read_text())
BASE=json.loads((ASR/'oscar-ribas-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[
 ('aiguat','medi ambient'),('aiguats','medi ambient'),('inundacions','medi ambient'),('1982','història'),('Cap de Govern','institucions'),('govern','institucions'),('Andorra','toponim'),('andorrà','gentilici'),('solidaritat','societat'),('població','societat'),('classes','societat'),('origen','societat'),('comuns','institucions'),('parròquia','institucions'),('pont','infraestructures'),('riu','medi ambient'),('aigua','medi ambient'),('carretera','infraestructures'),('infraestructures','infraestructures'),('emergència','institucions'),('sacrifici','societat'),('memòria','cultura'),('història','cultura'),('protecció civil','institucions'),('territori','territori'),('famílies','societat'),('ajuda','societat'),('reconstrucció','infraestructures'),('Nacions Unides','institucions'),('helicòpter','infraestructures'),('bombers','institucions'),('evacuació','institucions'),('voluntari','societat'),('solidari','societat'),('comunicacions','infraestructures'),('Suïssa','toponim'),('Catalunya','toponim'),('França','toponim'),('120 litres','medi ambient'),
]
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-yt-037__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-yt-037-oscar-ribas','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})

"""Prepara el dossier de prospecció de Andreu González fora del cànon."""
...
SMALL=json.loads((ASR/'andreu-gonzalez-small-nocontext.json').read_text())
BASE=json.loads((ASR/'andreu-gonzalez-base-nocontext.json').read_text())
FORMS_SOURCE=Path(__file__).parents[2]/'candidats/lead-rtva-008-mireia-pedescoll/formes-consens.tsv'
EXTRA=[('naturalesa', 'medi ambient'), ('natura', 'medi ambient'), ('ecologista', 'medi ambient'), ('planeta', 'medi ambient'), ('bosc', 'medi ambient'), ('boscos', 'medi ambient'), ('muntanya', 'toponim'), ('animals', 'medi ambient'), ('fauna', 'medi ambient'), ('flora', 'medi ambient'), ('Andorra', 'toponim'), ('andorrà', 'gentilici'), ('La Massana', 'toponim'), ('Bosc', 'medi ambient'), ('conservació', 'medi ambient'), ('educació', 'educació'), ('canvi climàtic', 'medi ambient'), ('residus', 'medi ambient'), ('reciclatge', 'medi ambient'), ('turisme', 'economia'), ('Pirineus', 'toponim')]
...
def main():
    ...
    for idx,(form,cat) in enumerate(forms,1):
        ...
        if seg:
            ...
            clip=f'lead-yt-042__{idx:02d}__{slug}.wav'; out=CLIPS/clip
            if not out.exists():
        ...
        if clip: queue.append({'candidate':'lead-yt-042-andreu-gonzalez','forma':form,'clip':f'clips/{clip}','veu_confirmada':'pendent','forma_confirmada':'pendent','variant_transcrita':'pendent','trets_fonetica':'pendent','prosodia':'pendent','nota_audicio':'pendent','estat_audicio':'pendent'})

"""Ordena provisionalment els torns d'Joan Micó i l'entrevistador."""
...
CAND = ROOT / "proveniencia" / "candidats" / "lead-rtva-004-joan-mico"
JSON = CAND / "asr" / "joan-mico.json"
...
def classify(text: str) -> tuple[str, str, str]:
    ...
    if first and len(words) >= 8:
        return "joan-mico-probable", "baixa", "primera persona i resposta desenvolupada"
    if len(words) >= 18:
        return "joan-mico-probable", "baixa", "segment llarg compatible amb resposta"
...
def main() -> None:
    ...
    form_counts = Counter()
    for row in rows:
        if row["rol_provisional"] != "joan-mico-probable":
            continue
        for category, form in forms:
            if re.search(rf"(?<![\wà-ÿ]){re.escape(form)}(?![\wà-ÿ])", row["text_asr"], re.I):
                form_counts[form] += 1
                form_rows.append({"categoria": category, "forma": form, "segment": row["segment"], "inici_s": row["inici_s"], "final_s": row["final_s"], "text_asr": row["text_asr"], "estat": "rol Joan Micó provisional; pendent d'audició"})
    with (CAND / "formes-joan-mico-provisional.tsv").open("w", encoding="utf-8", newline="") as handle:
    ...
    (CAND / "segmentacio-veus.md").write_text(
        """# Segmentació provisional de veus — Joan Micó
...
|---|---:|
""" + "\n".join(f"| {role} | {count} |" for role, count in sorted(counts.items())) + f"\n\nLa columna `motiu` conserva la raó de cada hipòtesi i les etiquetes no modifiquen el recompte canònic. La hipòtesi `joan-mico-probable` conté **{len(form_rows)} ocurrències** en **{len(form_counts)} formes**; el detall és a `formes-joan-mico-provisional.tsv`.\n", encoding="utf-8")

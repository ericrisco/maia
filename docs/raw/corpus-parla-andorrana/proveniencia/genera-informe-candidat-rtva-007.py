"""Resumeix i prepara l'audició del candidat RTVA Robert Guirao."""
...
CAND = ROOT / "proveniencia" / "candidats" / "lead-rtva-007-robert-guirao"
SMALL = CAND / "asr" / "robert-guirao.json"
BASE = CAND / "asr" / "robert-guirao-base.json"
...
def main() -> None:
    ...
    (CAND / "comparacio-asr.md").write_text(
        f"""# Comparació ASR — Robert Guirao
...
""", encoding="utf-8")
    ...
    for row in chosen:
        ...
        filename = f"robert-guirao__{slug(str(row['forma']))}__{float(row['inici_s']):.2f}.wav"

"""Ordena provisionalment els torns d'Xavier Espot i altres torns institucionals."""
...
CAND = ROOT / "proveniencia" / "candidats" / "lead-cg-001-xavier-espot"
JSON = CAND / "asr" / "xavier-espot.json"
...
def classify(text: str) -> tuple[str, str, str]:
    ...
    if first and len(words) >= 8:
        return "xavier-espot-probable", "baixa", "primera persona i resposta desenvolupada"
    if len(words) >= 18:
        return "xavier-espot-probable", "baixa", "segment llarg compatible amb resposta"
...
def main() -> None:
    ...
    form_counts = Counter()
    for row in rows:
        if row["rol_provisional"] != "xavier-espot-probable":
            continue
        for category, form in forms:
            if re.search(rf"(?<![\wà-ÿ]){re.escape(form)}(?![\wà-ÿ])", row["text_asr"], re.I):
                form_counts[form] += 1
                form_rows.append({"categoria": category, "forma": form, "segment": row["segment"], "inici_s": row["inici_s"], "final_s": row["final_s"], "text_asr": row["text_asr"], "estat": "rol Xavier Espot provisional; pendent d'audició"})
    with (CAND / "formes-xavier-espot-provisional.tsv").open("w", encoding="utf-8", newline="") as handle:
    ...
    (CAND / "segmentacio-veus.md").write_text(
        """# Segmentació provisional de veus — Xavier Espot
...
|---|---:|
""" + "\n".join(f"| {role} | {count} |" for role, count in sorted(counts.items())) + f"\n\nLa columna `motiu` conserva la raó de cada hipòtesi i les etiquetes no modifiquen el recompte canònic. La hipòtesi `xavier-espot-probable` conté **{len(form_rows)} ocurrències** en **{len(form_counts)} formes**; el detall és a `formes-xavier-espot-provisional.tsv`.\n", encoding="utf-8")

---
type: article
title: La sèrie que no és una sèrie
description: "L'enquesta sociolingüística d'Andorra ha canviat de mode, de marc, d'estació i fins de nivell de confiança. I el seu gràfic de l'indicador contradiu el text del mateix informe."
tema: temes/llengua/politica-linguistica
veu: compilada
epoca: contemporania
apte_llengua: false
font: spl-coneixements-usos-linguistics-2022
timestamp: 2026-09-14T00:40:00Z
tags: [llengua, sociolinguistica, dades, metodologia, serie-historica, discrepancia, prioritari]
---

# La sèrie que no és una sèrie

## La unitat llegida

***Coneixements i usos lingüístics de la població d'Andorra. Situació actual i
evolució (1995-2022)***, 48 pàgines, ISBN 978-99920-0-985-7, dipòsit legal
AND.361-2023. **Setena onada.** La fa **Andorra Recerca i Innovació** per
encàrrec del **Servei de Política Lingüística**
([fitxa](../../../fonts/spl-coneixements-usos-linguistics-2022.md)).

El corpus tenia les seves xifres **des de la pàgina del portal i des de la
premsa**. Ara està llegit el document, i el document **diu més coses que les
xifres**.

## Quatre talls dins d'una sèrie que es presenta com a contínua

L'informe afirma a la introducció que **«la metodologia emprada permet comparar
aquests resultats amb els dels estudis del 1995, el 1999, el 2004, el 2009, el
2014 i el 2018»**. La seva pròpia **fitxa tècnica metodològica** (p. 44-45)
enumera **quatre canvis** enmig de la sèrie.

| | 1995 · 1999 · 2004 | 2009 · 2014 | 2018 · 2022 |
| --- | --- | --- | --- |
| **Com es pregunta** | **Enquesta personal**, a domicili, 20-30 minuts | **Enquesta telefònica** | Enquesta telefònica |
| **D'on surt la mostra** | Domicili per **rutes aleatòries**, després quotes | Telèfons **de la guia d'Andorra Telecom** | **Tots els telèfons possibles** que hi ha a Andorra |
| **Probabilitat declarada** | **95%** | 95% (2009) · **95,5%** (2014) | **95,5%** |
| **Quan es fa el treball de camp** | **Setembre-octubre** | **Maig** (2009) · maig-juny (2014) | **Setembre-octubre** (2018) · **maig-juny** (2022) |

**Cap d'aquests quatre canvis no és menor per a una enquesta sobre llengua.**

> **La taula original, onada a onada, s'ha llegit el 2026-09-15** a l'annex de
> l'edició 1995-2009
> ([quaranta-vuit coma cinc, no quaranta-vuit coma sis](./quaranta-vuit-coma-cinc-no-quaranta-vuit-coma-sis.md)),
> **i diu dues coses que aquesta agrupació de tres columnes no deia**:
> **la mostra més gran de tota la sèrie és la primera** —**1.016 el 1995**,
> contra 671, 677 i 821—, **i amb ella el millor marge d'error, ± 3,1 %**; i
> **l'estratificació canvia a cada onada**, amb la situació laboral usada
> **només el 1999** i el 2009 **sense estratificar gairebé de res**, corregit
> després per ponderació. **Estratificar i ponderar no són la mateixa operació.**

- **Presencial a telefònica.** A una enquesta personal la persona veu
  l'enquestador i el sent parlar. **En una enquesta sobre en quina llengua
  parles, la llengua de l'enquestador és part de l'instrument.** El mode va
  canviar entre el 2004 i el 2009, i és exactament on la sèrie de llengua
  inicial catalana toca fons.
- **Guia telefònica a marcatge de tots els números possibles.** Una guia
  exclou qui no hi surt. **Qui no surt a la guia d'Andorra Telecom no és una
  mostra a l'atzar de la població**: tendeix a ser qui fa menys temps que hi és.
- **95% a 95,5%.** Dos nivells de confiança diferents dins de la mateixa taula.
  El marge d'error de cada onada **no és comparable amb el de l'anterior sense
  recalcular-lo**, i l'informe no el recalcula.
- **Maig contra octubre.** Andorra té una població que es mou amb la temporada.
  **Preguntar al maig i preguntar a l'octubre no és preguntar a la mateixa gent**,
  i la sèrie alterna les dues coses sense ordre: setembre, setembre, setembre,
  maig, maig, setembre, maig.

**Cap d'aquests talls no s'esmenta enlloc més del document.** Són a la fitxa
tècnica, a les dues últimes pàgines, i la conclusió parla de tendències.

## El que la taula de tres columnes del corpus amagava

La fitxa [la sèrie històrica](./la-serie-historica.md) dona la llengua inicial
amb **tres columnes: 1995, 1999 i 2022**, des d'una font secundària. **Les
xifres són exactes.** El que falta és el mig. Aquesta és la sèrie sencera, del
gràfic de la pàgina 10, verificat sobre la pàgina:

| Llengua inicial | 1995 | 1999 | 2004 | 2009 | 2014 | 2018 | **2022** |
| --- | --- | --- | --- | --- | --- | --- | --- |
| **Català** | 42,7 % | 35,1 % | **29,9 %** | **28,9 %** | 39,5 % | 35,7 % | **44,1 %** |
| **Castellà** | 34,6 % | 43,2 % | 43,4 % | 43,9 % | 43,8 % | 43,2 % | 40,3 % |
| **Portuguès** | 10,7 % | 10,9 % | 14,7 % | **17,9 %** | **18,6 %** | 17,1 % | 13,5 % |
| **Francès** | 10,8 % | 9,5 % | 10,7 % | 10,5 % | 9,7 % | 8,9 % | 10,0 % |
| **Anglès** | 1,7 % | 1,3 % | 1,2 % | 1,3 % | 3,5 % | 1,8 % | 3,0 % |
| **Altres** | 4,8 % | 6,2 % | 6,9 % | 5,4 % | 5,5 % | 5,0 % | 6,8 % |

**El mínim del català no és el 35,1 % del 1999: és el 28,9 % del 2009.** I
després del 39,5 % del 2014 **hi ha una recaiguda al 35,7 % el 2018**. La corba
no és una V: és **una V més fonda amb una segona baixada**, i amb tres
columnes no es veu cap de les dues coses.

> **I la recaiguda estava anunciada.** **L'edició del 2014 atribueix la seva
> pròpia pujada a la crisi** —al retorn d'immigrants portuguesos i a l'arribada
> d'espanyols de Catalunya, **el 93 % dels arribats feia cinc anys o menys**— i
> **escriu que «aquests canvis demogràfics semblen més conjunturals que
> estructurals»**, amb la condició explícita: **«en un futur context de
> creixement econòmic potser caldrà reforçar el paper actiu dels organismes
> públics oficials»**
> ([els nouvinguts tenien més català que els nascuts a Andorra](./els-nouvinguts-tenien-mes-catala-que-els-nascuts-a-andorra.md)).
> **Quatre anys abans que passés.**

**I el castellà quasi no es mou.** De 43,2 % a 43,9 % entre el 1999 i el 2018:
**set dècimes en dinou anys**. El que puja i baixa contra el català **no és el
castellà, és el portuguès** —de 10,7 % a 18,6 % i de tornada a 13,5 %— i
«altres».

## Aquests percentatges no sumen 100, i no és un error

Al peu del mateix gràfic, en cos petit:

> «Opció multiresposta: cada enquestat podia donar més d'una resposta i per això
> la suma dels percentatges és superior a 100.»

La columna del 2022 suma **117,7 %**. **No és un repartiment de la població: és
la proporció de gent que declara cada llengua**, i qui té dues llengües inicials
compta a les dues. Llegir «44,1 % català / 40,3 % castellà» com si fossin dos
trossos d'un pastís **és llegir-ho malament**, i és com es llegeix sempre que la
xifra viatja sola.

## El gràfic que contradiu el text del seu propi informe

L'**indicador lingüístic** combina coneixement i ús en una escala de 0 a 100. El
text de la pàgina 38 diu:

> «L'indicador lingüístic del català és de 59 punts, **dos punts i mig** més
> respecte del 2018. L'indicador lingüístic del castellà és de gairebé 55
> (**prop de tres punts menys** en comparació amb l'enquesta anterior).»

I les conclusions (p. 41) diuen que hi ha hagut un canvi de tendència respecte
del 2018, **«quan l'indicador del castellà va passar a ser més elevat que el de
la llengua oficial»**.

**El gràfic de la mateixa pàgina 38 diu una altra cosa.** Verificat a 300 ppp:
la línia verda —**català**— està **per sobre** de la morada —castellà— el 2018,
amb **57,6** contra **56,8**.

Les tres afirmacions del text **només quadren si al 2018 el català és 56,8 i el
castellà 57,6**, és a dir, **al revés del que el gràfic dibuixa**:

| | Text de l'informe | Gràfic de l'informe |
| --- | --- | --- |
| Català 2018 | 56,8 | 57,6 |
| Castellà 2018 | 57,6 | 56,8 |
| Català: 2018 → 2022 | 56,8 → 59,3 = **+2,5**, «dos punts i mig» ✓ | 57,6 → 59,3 = +1,7 ✗ |
| Castellà: 2018 → 2022 | 57,6 → 54,7 = **−2,9**, «prop de tres punts» ✓ | 56,8 → 54,7 = −2,1 ✗ |

**No cal arbitrar-ho amb una hipòtesi: ho arbitra l'onada anterior.** L'edició
**1995-2018**, pàgina 32, ho diu amb lletres:

> «L'indicador lingüístic del **català** el 2018 és de **56,8**, un punt menys
> respecte del 2014. L'indicador lingüístic del **castellà** és de **57,6**, una
> mica més d'un punt en comparació de l'enquesta anterior.»

I hi afegeix què va passar el 2014: **«després que el 2014 el català avancés el
castellà per primera vegada des del 1995, la tendència s'ha tornat a invertir.»**

**Conclusió, amb el càlcul a la vista:** el gràfic de l'indicador **no dibuixa el
creuament del 2014-2018**. I aquell creuament **és l'únic esdeveniment sobre el
qual l'informe del 2022 construeix la seva conclusió principal**. El mateix
gràfic surt igual a l'edició del 2018, de manera que **l'error ve d'abans i s'ha
reimprès**.

La sèrie correcta de l'indicador, doncs, és aquesta, amb les dues últimes
columnes preses del text i no del dibuix, **i amb la casella del 2004 corregida
el 2026-09-15 contra l'edició original de la quarta onada** —vegeu
[quaranta-vuit coma cinc, no quaranta-vuit coma sis](./quaranta-vuit-coma-cinc-no-quaranta-vuit-coma-sis.md):

| Indicador lingüístic | 1995 | 1999 | 2004 | 2009 | 2014 | 2018 | 2022 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| **Català** | 56,3 | 53,9 | **48,5** | 48,2 | **57,7** | **56,8** | **59,3** |
| **Castellà** | 50,0 | 58,2 | 60,8 | 59,1 | 56,4 | **57,6** | 54,7 |

## El rècord més alt i el més baix, el mateix any

Les conclusions de l'informe contenen dues frases que s'han de llegir juntes.

**La primera** (p. 42): l'indicador del català torna a ser el primer i **«la
presència del català és la més alta des de l'any 1995»**.

**La segona**, tres paràgrafs abans, sobre les administracions públiques:

> «l'atenció exclusiva en català ha disminuït lleugerament respecte del 2018 i
> **és la més baixa de tota la sèrie històrica (des de l'any 1995)**»

**El màxim i el mínim de la mateixa sèrie, el mateix any.** El país declara el
millor indicador de català des que es mesura **i, alhora, la pitjor atenció
exclusivament en català a la seva pròpia administració des que es mesura**.

I no és l'únic lloc: l'informe també registra **«una presència inferior del
català en el sector de la salut»** i **«una tendència a la baixa de l'atenció en
català en espais com ara els restaurants, les botigues i els bars, pubs i
discoteques, i també al metge especialista»**.

**El català guanya on es declara i perd on s'atén.**

## La regla de conversa, amb la pregunta literal

La pregunta, tal com la formula l'enquesta (p. 40):

> «Si s'adreça a algú en català i aquesta persona li respon en castellà, vostè
> generalment què fa?»

| Resposta | 2014 | 2018 | **2022** |
| --- | --- | --- | --- |
| **Continua en castellà** | 65,5 % | 77,5 % | **74,8 %** |
| Segueix en català | 17,5 % | 19,4 % | **20,6 %** |
| Mai s'adreça a ningú en català | 13,6 % | 10,8 % | 11,6 % |
| Demana si li poden parlar en català | 0,7 % | 3,5 % | 3,1 % |
| Interromp la conversa | 0,1 % | 1,1 % | 0,3 % |

> **Avís de base, afegit el 2026-09-15 i ampliat el mateix dia.** **Aquesta taula
> té una fila que hi falta i una columna amb una base diferent de les altres
> dues.**
>
> **La fila que hi falta és «No contesta»**, que l'edició del 2022 imprimeix a la
> mateixa figura: **2,6 % (2014) · 3,0 % (2018) · 0,8 % (2022)**.
>
> **Amb ella, la columna del 2014 suma exactament 100** i **les del 2018 i 2022
> sumen 115,3 i 111,2**. `Comprovació del corpus.` **A les dues últimes, les
> quatre opcions d'acció sumen prop de 100 entre elles**, que és el que
> s'esperaria si van **sobre el subgrup que s'adreça en català** —**la base que
> l'edició del 2009 imprimeix en lletres: «persones que inicien una conversa en
> català (429)»**.
>
> **`Hipòtesi del corpus`:** **el 2014 és l'única columna calculada sobre tota la
> mostra.** **Rebasant-la** —65,5 ÷ 83,8 = **78,2 %**— **la sèrie queda 79,4
> (2009) · 78,2 (2014) · 77,5 (2018) · 74,8 (2022) i baixa sense sotracs.**
>
> **El corpus retira, doncs, la lectura que hi hagués un salt de dotze punts
> entre el 2014 i el 2018**: **és molt probablement un canvi de denominador, no
> un canvi de conducta.** Vegeu
> [els nouvinguts tenien més català que els nascuts a Andorra](./els-nouvinguts-tenien-mes-catala-que-els-nascuts-a-andorra.md).

**Tres de cada quatre canvien.** I això passa el mateix any que el 47,5 % diu que
**comença sempre** les converses en català. **Començar en català i mantenir-lo
són dues coses diferents**, i l'enquesta les mesura per separat perquè ho són.

Qui manté el català són **els més grans de 50 anys, els de nacionalitat
andorrana, els nascuts a Andorra i els de llengua inicial catalana**.

## El coneixement declarat: el castellà per davant

L'indicador de coneixement —mitjana d'entendre, llegir, parlar i escriure, de 0
a 10— dona el **2022**:

| | Català | Castellà | Francès | Portuguès | Anglès |
| --- | --- | --- | --- | --- | --- |
| **2022** | **8,3** | **9,3** | 5,3 | 3,1 | 4,8 |
| 1995 | 7,3 | 8,9 | 5,3 | 1,3 | 1,9 |

> **Les cinc xifres del 1995 s'han verificat el 2026-09-15 contra l'edició
> original de la quarta onada, i coincideixen.** Allà hi ha, a més, **la sèrie
> sencera de 1995 a 2009 per a les cinc llengües**, i **l'indicador d'ús**, que
> aquesta fitxa no tenia
> ([quaranta-vuit coma cinc, no quaranta-vuit coma sis](./quaranta-vuit-coma-cinc-no-quaranta-vuit-coma-sis.md)).
> **El que en surt: el coneixement declarat del català no es mou en catorze anys
> —7,3 · 7,4 · 7,3 · 7,4— mentre l'ús cau de 3,4 a 2,9 sobre cinc.**

**A l'únic estat del món on el català és l'única llengua oficial, la població
declara saber més castellà que català**, i ho ha declarat a les set onades. La
distància s'ha escurçat: d'1,6 punts el 1995 a **1,0 el 2022**.

I el detall que no s'endevina: els qui declaren **menys** coneixement de català
**no són els nouvinguts**. El 2018 ho eren els qui feia cinc anys o menys que hi
vivien; el 2022 són **els qui fa entre 11 i 15 anys que resideixen a Andorra**,
amb un 6,6.

## Buits registrats

**Això no és l'estudi.** El document ho diu de si mateix: és **«un recull
sintètic extret d'un informe més exhaustiu i detallat»** que remet a
`www.catala.ad`. **Aquell informe no s'ha localitzat.** Les desagregacions, el
qüestionari i les bases hi són, i el corpus no els té.

**El capítol d'usos lingüístics, p. 17-37, no s'ha destil·lat.** Vint-i-una
pàgines —llar, fills, amics, feina, atenció al públic, administració, salut,
comerç— i és el gruix del document. Les xifres d'ús que aquesta fitxa dona
venen **de les conclusions**, no de les taules.

**La discrepància del gràfic de l'indicador queda registrada, no esmenada al
document.** El corpus la resol per a les seves pròpies taules amb el text de les
dues edicions, que coincideixen entre elles; **no s'ha comunicat a ningú ni s'ha
comprovat si existeix una fe d'errates**. I **no s'ha comprovat si l'error també
afecta les onades del 2014 i anteriors**: només s'han contrastat les edicions del
2018 i del 2022.

~~**Les quatre onades anteriors són al disc i no s'han obert**, llevat de la
pàgina 32 de la del 2018. Les xifres de 1995, 1999, 2004, 2009 i 2014 que
apareixen aquí són **les que reprodueix l'onada del 2022**, no les de les seves
edicions originals. **Un valor reproduït set anys després no és un valor
comprovat.**~~ — **`parcial` des del 2026-09-15, i la regla era bona.**

**S'ha obert l'edició original de la quarta onada**, *Coneixements i usos
lingüístics (1995-2009)*, ISBN 978-99920-0-615-3, DL AND. 908-2011
([fitxa](../../../fonts/spl-coneixements-usos-linguistics-2009.md)), i s'han
comprovat **trenta-vuit valors de sèrie**
([lectura](./quaranta-vuit-coma-cinc-no-quaranta-vuit-coma-sis.md)):

| | |
| --- | ---: |
| **Confirmats exactament** | **37** |
| **Desmentits** | **1** |

**El desmentit és l'indicador lingüístic del català del 2004**: **l'edició
original imprimeix 48,5 al gràfic i 48,5 al text**, i **el text hi afegeix
l'aritmètica** —«ha baixat 0,3 punts, en passar d'una mitjana de 48,5 a 48,2»—,
que **només quadra amb 48,5**. **La taula de dalt s'ha corregit.**

**I la sèrie de llengua inicial queda verificada al seu document original per a
les quatre primeres onades**: vint-i-quatre valors, vint-i-quatre coincidències.

**El que segueix `obert`: les edicions de 2014 i 2018**, que són al disc i
segueixen sense obrir-se. **Sense elles no es pot saber en quina reedició es va
introduir la dècima.**

**El marge d'error és més gran que bona part del que es comenta.** ± 3,65 punts.
Variacions com el −1 punt de l'ús habitual del català, el +3,2 % de l'ús
combinat a l'administració o les tres dècimes de l'indicador de coneixement
**cauen dins del marge**, i l'informe les comenta com a tendències. **El corpus
no les tracta com a canvis acreditats.**

**Cap efecte causal no s'infereix.** El document relaciona el repunt del 2022
amb la pandèmia, amb l'estabilització de la població andorrana i amb el prestigi
de la llengua oficial, i ho fa amb un **«potser expliquen»**. **Aquí es registra
que ho diu, no que sigui així.**

**Els dos articles anteriors del corpus no s'han reescrit.**
[Qui parla què](./qui-parla-que.md) i [la sèrie històrica](./la-serie-historica.md)
segueixen citant el portal i la premsa. **Les seves xifres són correctes** —s'han
comprovat totes contra el document—, però **la forma de sèrie de tres columnes i
la lectura dels percentatges com a repartiment** són el que aquesta fitxa
corregeix.

**Drets tancats.** Cap llicència declarada, ni al document ni al portal. **Res
d'aquesta font no entra en cap dataset.**

## Related

- [La sèrie històrica del català a Andorra](./la-serie-historica.md) — la taula de tres columnes que aquesta fitxa completa.
- [Qui parla què](./qui-parla-que.md) — les xifres del 2022, citades des del portal.
- [La llei de la llengua](./la-llei-de-la-llengua.md)
- [El que la llei de la llengua pot multar](./el-que-la-llei-pot-multar.md) — què és sancionable de l'atenció al públic que aquí cau.
- [Andorra es castellanitza](./andorra-es-castellanitza.md) — la mateixa por, dita el 1989.
- [La marca que no vol dir Andorra](../lexic-andorra/la-marca-que-no-vol-dir-andorra.md)
- [Quaranta-vuit coma cinc, no quaranta-vuit coma sis](./quaranta-vuit-coma-cinc-no-quaranta-vuit-coma-sis.md) — l'edició original de la quarta onada, oberta per comprovar aquestes xifres.
- [Els nouvinguts tenien més català que els nascuts a Andorra](./els-nouvinguts-tenien-mes-catala-que-els-nascuts-a-andorra.md) — la cinquena onada, que anuncia per escrit la recaiguda del 2018.

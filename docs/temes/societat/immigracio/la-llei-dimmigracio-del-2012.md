---
type: article
title: La llei d'immigració del 2012
description: "La llei marc que els residents reclamaven el 1997 va arribar el 2012. La jerarquia per origen hi continua: primer els països amb conveni, després la UE, i finalment la resta del món."
tema: temes/societat/immigracio
veu: compilada
epoca: contemporania
apte_llengua: false
font: jurisprudencia-ad
timestamp: 2026-09-14T02:45:00Z
tags: [immigracio, dret, quotes, treball, prioritat, salut, prioritari]
---

# La llei d'immigració del 2012

El corpus acaba de documentar **la cadena normativa fins al 1997** i **el que els
residents en deien**
([la normativa migratòria](./la-normativa-migratoria.md),
[el rellotge dels vint-i-cinc anys](../../institucions/nacionalitat-i-residencia/el-rellotge-dels-vint-i-cinc-anys.md)),
i hi va deixar obert **quan arriba la llei d'immigració andorrana**.

**Es tanca aquí: la Llei 9/2012, qualificada d'immigració**
([font](../../../fonts/jurisprudencia-ad.md)), amb **234 articles** i **divuit
versions consolidades** al text que el corpus té.

| | |
| --- | --- |
| **Reclamada** | El **1997**, per l'ARA i per Càritas, «com a màxim sis mesos després de la Constitució» |
| **Arriba** | **2012** |
| **Retard sobre el que es demanava** | **més de divuit anys** des de la Constitució |
| **Vegades que s'ha modificat** | **divuit consolidacions** |

**Una llei d'immigració que s'ha refet divuit vegades en catorze anys** diu, ella
sola, que la matèria no ha parat de moure's.

## El que no ha canviat: la jerarquia per origen

Aquesta és la troballa que més importa, perquè **el corpus tenia documentat un
criteri d'entrada desigual per origen el 1984-1991 i havia deixat obert si es va
mantenir**.

**Article 40. Principi de prioritat en la concessió de les autoritzacions:**

> les autoritzacions «es concedeixen **en primer lloc als nacionals dels estats
> que hagin signat i ratificat un conveni amb el Principat d'Andorra**,
> seguidament **als nacionals dels estats membres de la Unió Europea o de l'Espai
> Econòmic Europeu** i finalment **als nacionals d'altres estats**.»

**Tres graons, i el tercer és la resta del món.**

| Època | Com s'expressa la prioritat per origen |
| --- | --- |
| **1984-1987** | Quota **40% francesos, 40% espanyols, 20% altres nacionalitats** |
| **des de 1991** | Les quotes **restringeixen els de fora de la UE i l'EFTA** |
| **des de 2012** | **Principi de prioritat**: conveni → UE/EEE → resta |

**La forma jurídica canvia tres vegades; l'ordre, no.** El corpus ho registra com
a **continuïtat documentada**, i **no com a prova d'intenció**: vegeu
[per què tothom s'hi sentia integrat](./per-que-tothom-shi-sentia-integrat.md),
on s'explica què demostren i què no les fonts que hi convergeixen.

### L'excepció, i qui la pot fer servir

**Article 41**: el Govern pot obrir, **quota per quota**, un nombre
d'autoritzacions per a nacionals d'altres estats — però **només** si la persona

> «acredita **la seva alta especialització i/o experiència professional** per al
> lloc de treball que desitja ocupar **i si no percep un salari en relació amb la
> qualificació acreditada**».

**Qui ve de fora dels dos primers graons hi entra per alta qualificació o no hi
entra.** És la mateixa lògica que el 1997 denunciava **Rachid Raguig**
([les comunitats](./les-comunitats.md)), traduïda a dret.

## El que sí que ha canviat: la salut

**El 1997**, segons l'ADMA, el protocol excloïa **no sols per malalties
infectocontagioses, sinó per diabetis, hepatitis C o sordesa**, i **el pagava
l'immigrant** ([les dones migrants el 1997](./les-dones-migrants-el-1997.md)).

**El 2012 el criteri és un altre:**

> «Els estrangers que desitgen entrar al Principat d'Andorra **no poden
> representar un risc greu per a la salut pública**. El Govern determina
> reglamentàriament **les afeccions i malalties que representen un risc per a la
> salut pública**, atenent **les recomanacions i directrius de l'Organització
> Mundial de la Salut** i d'acord amb la legislació sanitària.»

**De «no estar malalt» a «no ser un risc per a la salut pública», amb l'OMS com a
referència.** **El corpus no ha vist el reglament** que fixa la llista —ni el del
1993 ni el d'ara— i **no pot dir quan va canviar realment la pràctica**. **Buit
registrat.**

## Qui fixa les quotes ara

**El 1997 les fixava el Consell General per llei qualificada.** **El 2012 les fixa
el Govern per reglament**, a partir d'una anàlisi que la llei descriu (**article
23**): necessitats quantitatives i qualitatives de l'economia i la societat, i la
situació nacional del treball, amb dades de l'organisme d'ocupació.

I amb **una obligació de consulta que val la pena llegir al costat del que
demanaven els residents el 1997**:

> El Govern «completa aquesta anàlisi efectuant totes les consultes que considera
> oportunes prop de **la Cambra de Comerç, Indústria i Serveis**, de **les
> organitzacions empresarials i sindicals** i del **Consell de Col·legis
> Professionals**, i prop de **les associacions d'àmbit nacional que tenen un
> interès en matèria d'immigració**.»

**Aquesta última línia és, literalment, la interlocució que reclamaven l'ARA i
l'English Speaking Residents Association**: **«els polítics ens haurien de parlar
i d'escoltar»**
([per què tothom s'hi sentia integrat](./per-que-tothom-shi-sentia-integrat.md)).
**Quinze anys després, hi és a la llei** — i **el corpus no sap si s'exerceix**,
perquè la fórmula és **«les consultes que considera oportunes»**. **Buit
registrat.**

I l'objectiu declarat de la política, que tanca el cercle amb el títol de la Diada
de 1997:

> el Govern defineix la política «**privilegiant l'accés als llocs de treball dels
> andorrans i dels estrangers legalment residents, amb l'objectiu d'afavorir la
> cohesió social**».

## Les quotes noves, i el país que descriuen

| Tipus | Per a què |
| --- | --- |
| **Quota especial de temporada** | Cobrir mà d'obra «que no ha estat satisfeta pel mercat del treball interior» en **activitats que només es poden exercir durant una temporada** |
| **Quota especial per activitat professional** | Quan es constata **un dèficit de mà d'obra** en un sector, per **un augment conjuntural** |
| **Residència sense activitat lucrativa** | **Professionals amb projecció internacional**, i per **raons d'interès científic, cultural i esportiu** |
| **Residència per ingrés en centres geriàtrics o de cures privats** | Amb estada d'**almenys 90 dies per any natural** |
| **Economia digital i emprenedoria** | Per a qui **no requereix una ubicació geogràfica concreta** i treballa amb telecomunicacions, i per a **participants en el programa per a emprenedors estrangers** |

**El país de la quota de 1989 «per sectors econòmics» i el de la quota per a
nòmades digitals són el mateix país amb trenta anys de diferència.** Vegeu
[Andorra 2020](../../economia/transformacio-economica/andorra-2020.md), on el pla
estratègic apostava per «migrar vers activitats econòmiques d'alt valor afegit».

## Les condicions que limiten el treballador

Algunes són restriccions personals fortes i el corpus les llista perquè
**concreten què vol dir una autorització lligada a un lloc**:

- **Durant el primer any** d'una autorització de residència i treball **no es pot
  canviar de sector econòmic ni treballar per compte propi** — amb **excepció**
  si hi ha **necessitat urgent en els sectors sanitari o educatiu**, avalada per
  informe del ministeri corresponent.
- **Estudiants**: l'autorització s'anul·la si treballen **més de 20 hores
  setmanals** (**24** al sector sanitari o sociosanitari amb guàrdies d'atenció
  continuada).
- **Autorització temporal**: s'anul·la si es treballa **més de 52 hores
  setmanals** en conjunt, o **més de 12 hores en un sector diferent** del que va
  motivar l'autorització.
- **Reagrupament familiar**: si s'anul·la l'autorització de qui va reagrupar,
  **s'anul·len les de les persones reagrupades**.

**El vincle entre permís i contracte que Càritas denunciava el 1997**
([on falla la integració](./on-falla-la-integracio.md)) **hi continua**, ara
tipificat amb tot detall: **no residir o no treballar «de forma permanent i
efectiva» és causa d'anul·lació**.

### I una regla de còmput que respon a una pregunta oberta

El corpus tenia registrat, per l'ARA, que **els anys de permís de sojorn no
comptaven** per a la residència
([el rellotge dels vint-i-cinc anys](../../institucions/nacionalitat-i-residencia/el-rellotge-dels-vint-i-cinc-anys.md)).
La llei del 2012 **no resol aquella qüestió**, però **sí que diu què es compta
com a residència permanent i efectiva**: s'hi **computen les absències** per
vacances, per **tractament mèdic a l'estranger**, per **un cicle de formació a
l'estranger** i per **desplaçament laboral acordat pel patró** de fins a dotze
mesos.

**Buit registrat:** **si els anys de sojorn compten avui**, i des de quan.

## Quinze anys de quota, i el pic del 2023

**El BOPA publica 105 reglaments de quota entre el 2012 i el 2026**, deduplicats
per contingut
([evidència](../../../raw/lleis/bopa/quotes-immigracio-2012-2026/reglaments-de-quota-d-immigracio-2012-2026.md)). **No són
quotes anuals: se'n publiquen dues o més l'any**, i les posteriors poden
descomptar les autoritzacions ja concedides per les anteriors —així ho diu
expressament el decret del 5 de maig del 2026. **Sumar-les donaria una xifra
falsa.**

**Són quatre sèries paral·leles**: la **quota general de treball** (residència i
treball més treball fronterer), la de **compte propi**, la de **residència sense
treball**, i les **quotes especials** de temporers, cuidadors a domicili i
ampliacions puntuals.

**La quota general de treball, decret a decret**, llegida de l'article 2 de cada
reglament. **Dues columnes de data perquè no es confonguin**: la sessió de Govern
que l'aprova i la publicació al BOPA.

| Sessió de Govern | BOPA | Residència i treball | Fronterer | **Global** | |
| --- | --- | ---: | ---: | ---: | --- |
| 22 d’octubre del 2014 | 2014-10-28 | 170 | 50 | **220** |  |
| 15 d’abril del 2015 | 2015-04-21 | 268 | 52 | **320** |  |
| 14 d’octubre del 2015 | 2015-10-20 | 345 | 68 | **413** |  |
| 20 d’abril del 2016 | 2016-04-26 | 409 | 89 | **498** |  |
| 19 d’octubre del 2016 | 2016-10-25 | 450 | 146 | **596** |  |
| — | 2017-01-17 | 500 | 160 | **660** | modificació |
| 26 d’abril del 2017 | 2017-05-02 | 623 | 168 | **791** |  |
| 18 d’octubre del 2017 | 2017-10-24 | 702 | 198 | **900** |  |
| 25 d’abril del 2018 | 2018-05-01 | 702 | 198 | **900** |  |
| 17 d’octubre del 2018 | 2018-10-23 | 663 | 187 | **850** |  |
| 24 d’abril del 2019 | 2019-05-01 | 700 | 200 | **900** |  |
| 16 d’octubre del 2019 | 2019-10-22 | 663 | 187 | **850** |  |
| 29 d’abril del 2020 | 2020-04-28 | 351 | 99 | **450** |  |
| 21 d’octubre del 2020 | 2020-10-27 | 351 | 99 | **450** |  |
| 28 d’abril del 2021 | 2021-05-04 | 413 | 117 | **530** |  |
| 18 d’agost del 2021 | 2021-08-24 | 288 | 82 | **370** | ampliació |
| 27 d’octubre del 2021 | 2021-11-02 | 702 | 198 | **900** |  |
| 27 d’abril del 2022 | 2022-05-03 | 936 | 264 | **1.200** |  |
| 12 d’octubre del 2022 | 2022-10-18 | 936 | 264 | **1.200** |  |
| 19 d’abril del 2023 | 2023-04-25 | 2.399 | 310 | **2.709** |  |
| 25 d’octubre del 2023 | 2023-11-01 | 2.080 | 220 | **2.300** |  |
| 24 d’abril del 2024 | 2024-05-01 | 702 | 198 | **900** |  |
| 23 d’octubre del 2024 | 2024-10-28 | 702 | 198 | **900** |  |
| 23 d’abril del 2025 | 2025-04-23 | 702 | 198 | **900** |  |
| — | 2025-09-10 | 90 | 10 | **100** | ampliació |
| 24 de setembre del 2025 | 2025-09-24 | 65 | 5 | **70** |  |
| 24 de setembre del 2025 | 2025-09-30 | 702 | 198 | **900** |  |
| — | 2026-03-12 | 135 | 15 | **150** | ampliació |
| 29 d’abril del 2026 | 2026-05-05 | 135 | 15 | **150** |  |
| 29 d’abril del 2026 | 2026-05-05 | 624 | 176 | **800** |  |

**Tres coses que la sèrie diu.**

**Primera: el 2020 la quota es parteix per la meitat.** De 850-900 el 2019 a **450** el **29 d'abril del 2020**, i s'hi manté tot l'any. **La covid no es veu a la
llei d'immigració: es veu a la quota.**

**Segona: el 2023 hi ha un pic de tres vegades el normal.** El decret del **19 d'abril del 2023** obre **2.709 autoritzacions**, i el del **25 d'octubre** encara **2.300**.
L'any següent es torna a **900**. `Requereix saber què va justificar el pic: el
reglament no ho explica.`

**Tercera: la proporció entre residents i fronterers és estable i és aproximadament
78 a 22.** 702 i 198; 936 i 264; 663 i 187. **L'única excepció és el pic del
2023**, on els fronterers baixen a l'11% del global. **Quan Andorra obre molt, obre
per a gent que hi viurà.**

**I la regla de prioritat és la mateixa a tots els reglaments**, i és la que
respon per què l'article 40 importa:

> «s'han de concedir seguint un **ordre estricte de prioritat cronològica** […]
> però s'ha de donar prioritat, **en primer lloc, als nacionals dels estats que
> hagin signat i ratificat un conveni amb el Principat d'Andorra**; seguidament,
> als nacionals dels estats membres de la **Unió Europea i de l'Espai Econòmic
> Europeu**, i, finalment, als **nacionals d'altres estats**.»

**Tres graons, i cap reglament no diu quins estats són del primer.**

**Les altres dues sèries, pel que s'ha llegit directament:**

| Sèrie | Recorregut |
| --- | --- |
| **Residència sense treball** | **500** (2012, 2013) → **600** (2016, 2017, 2020, 2023) → **200** (decret de l'11 de març del 2026) |
| **Compte propi** | **150** (2013) → **250** (2014-2023) → **125** (2024) → **200** (2025, 2026) |

**La residència passiva s'ha reduït a un terç el 2026**, i el compte propi va
caure a la meitat el 2024 abans de recuperar-se parcialment.

## La quota és el cabal; això és el dipòsit

> **Afegit el 18-09-2026** amb `AUTORITZACIONS D'IMMIGRACIÓ EN VIGOR PER TIPUS`
> (`docs/raw/estadistica-api/`, CC BY 4.0,
> [font](../../../fonts/estadistica-ad.md)). **Les quotes de dalt diuen quantes
> autoritzacions es poden concedir; aquesta taula diu quantes n'hi ha de
> vigents, cada mes, des del 2008.** **El corpus tenia el cabal i no el
> dipòsit.**

**Al desembre de cada any:**

| Any | Residència i treball | Residència | Temporal | Fronterer | **Total** |
| --- | ---: | ---: | ---: | ---: | ---: |
| **2008** | **36.326** | 6.893 | 2.930 | **2.135** | **48.915** |
| 2010 | 34.313 | 7.179 | 2.831 | 1.797 | 46.765 |
| **2013** | **31.956** | 7.302 | 2.641 | 1.568 | **44.066** |
| 2016 | 32.902 | 7.853 | 3.494 | 1.355 | 46.321 |
| 2019 | 35.381 | 8.214 | 4.489 | 1.662 | 50.517 |
| **2020** | 34.824 | 8.396 | **1.121** | 1.648 | 46.779 |
| 2022 | 37.262 | 9.144 | **5.904** | 1.777 | 55.153 |
| 2023 | 40.316 | 9.623 | 4.441 | 1.808 | 57.062 |
| **2025** | **42.900** | **10.199** | 4.574 | 1.955 | **60.399** |

**Tres coses que la sèrie de quotes no deixava veure.**

**Primera: el dipòsit va buidar-se cinc anys seguits.** **Del 2008 al 2013 les
autoritzacions vigents cauen de 48.915 a 44.066**, **un −9,9%**, i **les de
residència i treball cauen encara més, un −12,0%.** **La crisi va fer marxar
gent, no només va aturar l'entrada.** **No es recupera el nivell del 2008 fins
al 2018.**

**Segona: l'autorització de residència sense treball puja mentre tota la resta
baixa.** **De 6.893 el 2008 a 10.199 el 2025**, **un +48,0%**, i **durant els
cinc anys de caiguda general només retrocedeix una vegada i de vint
autoritzacions** —de 7.322 el 2012 a 7.302 el 2013—. **Cap altra categoria no
travessa la crisi creixent**, i **és precisament la que la quota del 2026 ha
retallat de 600 a 200** (vegeu més amunt). `El corpus no lliga les dues coses:
registra que la categoria que més ha crescut en disset anys és la que s'acaba de
restringir.`

**Tercera: la temporal és el termòmetre de l'hivern.** **El desembre del 2019 hi
havia 4.489 autoritzacions temporals vigents i el del 2020 n'hi havia 1.121**:
**un −75,0% en un any.** **El 2022 arriba a 5.904, el màxim de tota la sèrie.**
**Cap altra categoria no es mou així.**

**I una quarta, petita**: **els fronterers toquen fons el 2016 amb 1.355** i
**el 2025 en són 1.955**, **encara per sota dels 2.135 del 2008.** **És l'única
categoria gran que disset anys després segueix per sota d'on era**, **en un país
que mentrestant ha passat de 70.290 habitants el 2010 a 89.058 el 2025**
([la piràmide](../demografia/la-mateixa-gent-quinze-anys-mes-gran.md)).

## Buits registrats

1. ~~**El reglament que fixa les malalties de risc per a la salut pública.** I el
   del 1993, per comparar-los. **Prioritari.**~~ — **`resolt` el 17-09-2026, i
   n'hi ha hagut tres, no dos**: 1993, 1998 i el vigent del **20 d'octubre del
   2010**. Les tres llistes, comparades, són a
   [Apte per residir](./apte-per-residir.md): **sis causes de no-aptitud el 1993,
   sis el 1998, tres el 2010.**
2. ~~**Si l'article 23.2 s'exerceix**: quines associacions s'han consultat mai i
   amb quin efecte.~~ — **`parcial` el 21-09-2026**: l’article 23.2 de la [Llei
   qualificada d’immigració](../../../raw/lleis/immigracio.txt) identifica com a
   interlocutors la Cambra de Comerç, les organitzacions empresarials i
   sindicals, el Consell de Col·legis Professionals i les associacions d’àmbit
   nacional interessades en immigració. El text obliga el Govern a fer les
   consultes que consideri oportunes, però no acredita quines es van fer ni quin
   efecte van tenir sobre la política o les quotes.
3. ~~**Els reglaments de quota des del 2012: xifres per any i per tipus**~~ —
   **`parcial` el 17-09-2026**: **baixats els 105 reglaments** i extreta la sèrie
   de la quota general de treball — vegeu
   [Quinze anys de quota, i el pic del 2023](#quinze-anys-de-quota-i-el-pic-del-2023).
   ~~`Segueixen sense buidar les sèries de compte propi i de residència sense
   treball any per any, i els quinze anys entre el 1997 i el 2012.`~~ —
   **els quinze anys entre el 1997 i el 2012 estan tancats el 17-09-2026**:
   **cent un textos més**, del 1989 al 2011
   (`docs/raw/lleis/bopa/quotes-immigracio-1989-2011/`), buidats a
   [El mercat de treball dual](./el-mercat-de-treball-dual.md). **Entre l'agost
   del 1997 i el setembre del 2002 el Butlletí no en publica cap**, i des del
   2002 la quota general cau de **4.750** a **95** en tres anys.
   `Segueixen sense buidar les sèries de compte propi i de residència sense
   treball any per any.`
4. ~~**Quins estats tenen conveni amb Andorra a efectes de l'article 40**~~ —
   **`parcial` el 17-09-2026**: **el mecanisme està documentat i la llista no.**
   **Els 105 reglaments de quota del 2012 al 2026 repeteixen la mateixa fórmula
   de tres graons** —estats amb conveni signat i ratificat, després UE i EEE,
   després la resta— **i cap no nomena un sol estat.** `La llista no és als
   reglaments de quota: cal buscar-la als convenis mateixos o demanar-la.`
5. ~~**Si els anys de sojorn compten** per a la residència.~~ — **`parcial`
   el 21-09-2026**: l’article 53 de la [Llei qualificada d’immigració](../../../raw/lleis/immigracio.txt)
   diu que, per renovar una autorització —excepte la de fronterer i la de
   treball sense residència—, cal haver residit de forma **permanent i
   efectiva** al Principat, i hi computa les vacances, el tractament mèdic o la
   formació a l’estranger i els desplaçaments laborals autoritzats de fins a
   dotze mesos. El text fixa aquesta regla de renovació i no resol si els anys
   d’un permís de sojorn serveixen per a altres còmputs de residència, com la
   nacionalitat.
6. ~~**Les divuit modificacions: què va canviar cadascuna**~~ — **`parcial` el
   17-09-2026**: la xifra de divuit no és de lleis sinó de versions
   consolidades.** **Al BOPA, la cadena sencera de la llei d'immigració són dotze
   documents**: la llei base, **set lleis que la modifiquen** i **quatre textos
   refosos**.

   | Any | Norma |
   | --- | --- |
   | **2002** | **Llei qualificada d'immigració, del 14 de maig del 2002** |
   | 2004 | Llei 21/2004, del 14 de desembre, qualificada de **mesures d'adaptació** |
   | 2007 | Llei 27/2007, del 22 de novembre |
   | 2008 | *Text refós* (Decret legislatiu del 25-6-2008) |
   | **2012** | **Llei 9/2012, del 31 de maig** — la que aquesta fitxa llegeix |
   | 2013 | Llei 16/2013, del 10 d'octubre |
   | 2014 | *Text refós* (Decret legislatiu del 26-3-2014) |
   | 2016 | Llei 14/2016, del 28 de juliol |
   | 2017 | *Text refós* (Decret legislatiu de l'1-2-2017) |
   | 2018 | Llei 10/2018, del 17 de maig |
   | 2019 | *Text refós* (Decret legislatiu del 6-2-2019) |
   | **2022** | **Llei 11/2022, del 7 d'abril** — la darrera |

   **Comprovat negatiu**: **cap llei de modificació posterior a l'abril del 2022**
   no apareix al Butlletí. **La llei d'immigració fa quatre anys i mig que no es
   toca**, mentre que **els reglaments de quota s'han publicat dues vegades l'any
   cada any.** `Requereix llegir cadascuna per dir què va canviar; ara se sap
   quantes són i quines.`
7. ~~**La llei de nacionalitat vigent**, que és una altra norma i que abans no
   constava a `raw/lleis/`. **És un buit estructural i no de lectura**, amb
   conseqüències per a [els andorrans de primera
   generació](../../institucions/nacionalitat-i-residencia/els-andorrans-de-primera-generacio.md),
   [la política de nacionalitat](../../institucions/nacionalitat-i-residencia/la-politica-de-nacionalitat.md)
   i [els nats a Andorra](./els-nats-a-andorra.md).**~~ — **`resolt` el
   17-09-2026 des del [BOPA](../../../fonts/bopa.md)**: és la **Llei qualificada
   de la nacionalitat del 5 d'octubre de 1995**, publicada el **30-1-1996**, amb
   la **Llei 10/2004**, la **Llei 15/2006** i la **Llei 8/2026, del 7 de maig**
   que la modifiquen, i la del **3 de setembre de 1993** que la precedeix
   ([evidència](../../../raw/lleis/bopa/llei-qualificada-nacionalitat-1995.txt),
   [evidència](../../../raw/lleis/bopa/llei-8-2026-mod-nacionalitat.txt),
   [evidència](../../../raw/lleis/bopa/llei-qualificada-nacionalitat-1993.txt)).
   La cadena sencera és a [El Codi de la nacionalitat de 1977](../../institucions/nacionalitat-i-residencia/el-codi-de-la-nacionalitat-de-1977.md).

## Related

- [La normativa migratòria](./la-normativa-migratoria.md) — tota la cadena fins al 1997.
- [El rellotge dels vint-i-cinc anys](../../institucions/nacionalitat-i-residencia/el-rellotge-dels-vint-i-cinc-anys.md)
- [Les dones migrants el 1997](./les-dones-migrants-el-1997.md) — el control mèdic d'abans.
- [Per què tothom s'hi sentia integrat](./per-que-tothom-shi-sentia-integrat.md) — la interlocució que demanaven.
- [On falla la integració](./on-falla-la-integracio.md) — el permís lligat al contracte.
- [El mercat de treball dual](./el-mercat-de-treball-dual.md)
- [Apte per residir](./apte-per-residir.md) — la mateixa llei, pel filtre mèdic i el fitxer.

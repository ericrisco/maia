---
type: article
title: Lea Ricart i Martínez
description: "Nedadora dels 50 metres lliures al Mundial de natació del 2017."
tema: temes/esports/altres-esports
veu: compilada
epoca: contemporania
apte_llengua: false
font: wikipedia-atletes-andorrans
timestamp: 2026-09-13T13:12:00Z
tags: [esports, natacio, mundials, dones]
---

# Lea Ricart i Martínez

## El fet

**Lea Ricart Martínez, nascuda el 16 de gener del 2001**, és una **nedadora
andorrana** que **va competir en els 50 metres lliures al Campionat del Món de
Natació del 2017** ([font](../../../fonts/wikipedia-atletes-andorrans.md)).

**Tenia 16 anys.**

## Tanca en part una pregunta que el corpus havia obert

[La fitxa de Pol Arias](../estiu/pol-arias.md) deixava escrit que **Andorra
havia portat nedadors a set Jocs Olímpics seguits i que des del 2016 no n'hi
havia portat cap**, i preguntava per què.

**Aquesta fitxa demostra que la natació andorrana no es va aturar el 2016**:
**el 2017 hi havia una nedadora de setze anys al Mundial**.

**El que es va aturar és la presència OLÍMPICA**, no la internacional.

**El corpus corregeix així la formulació del buit:** no és *«per què Andorra
deixa de tenir nedadors»* sinó **«per què Andorra deixa d'obtenir places
olímpiques de natació»** — que és **una pregunta sobre les places
d'universalitat de la FINA** ([què són](../estiu/santiago-deu.md)), no sobre el
país.

## El que falta

> **Auditat el 2026-09-13 contra el bolcat de la font pròpia**
> ([`Lea_Ricart_Martínez.wiki`](../../../raw/web/esport/atletes/Lea_Ricart_Martínez.wiki)).
> **Els quatre buits segueixen oberts, i ara se sap per què: el bolcat és un
> esborrany.** **L'infobox de nedadora hi és sencer i TOTS els camps són
> buits** —club, estils, alçada, pes, medaller, lloc de naixement—, **i l'únic
> text és la frase que aquesta fitxa ja reprodueix.**

- ~~**La seva marca i posició** al Mundial del 2017.~~ — **`obert`, confirmat.**
  **El bolcat cita els resultats de les sèries de la FINA i l'enllaç és mort**
  (`url-status=dead`, arxivat el **12 de febrer del 2018**).
  `Requereix font externa: l'arxiu d'Omega Timing o la FINA.`
- ~~**La resta de la carrera.**~~ — **`obert`, confirmat.**
- ~~**On va néixer.**~~ — **`obert`, i la font ho declara ella mateixa**: el bolcat
  la classifica a la categoria **«Place of birth missing (living people)»**.
  **No és que el corpus no ho hagi destil·lat: és que la Viquipèdia hi té el
  buit obert i el marca.** **És el mateix cas que a vuit fitxes de futbolista**
  — vegeu [futbol](../futbol/README.md).
- ~~**Si segueix nedant.**~~ — **`obert`, confirmat.**

## Related

- [Altres esports](./README.md)
- [Esports](../README.md)
- [Pol Arias](../estiu/pol-arias.md) — l'últim nedador olímpic.

---
type: article
title: Andreu Matos i Muñoz
description: "Davanter amb cinc internacionalitats. Juga al FC Santa Coloma."
tema: temes/esports/futbol
veu: compilada
epoca: contemporania
apte_llengua: false
font: wikipedia-atletes-andorrans
timestamp: 2026-09-13T15:30:00Z
tags: [esports, futbol, seleccio]
---

# Andreu Matos i Muñoz

## El fet

**Andreu Matos i Muñoz**, nascut a Andorra la Vella el 1 de desembre del 1995.
**Davanter** ([font](../../../fonts/wikipedia-atletes-andorrans.md)).

| | |
| --- | --- |
| **Naixement** | **Andorra la Vella, 1 de desembre del 1995** |
| **Partits amb Andorra** | **5** |
| **Posició** | Davanter |

**FC Santa Coloma**, a la **Primera Divisió** andorrana.

## El que en destaca

**Consta també com a internacional sub-21.** **El corpus té molt poc de les seleccions de base andorranes** —només el 3-3 de la sub-21 contra Anglaterra del 2020 ([Ricard Fernández](./ricard-fernandez.md))— i **cada nom que hi pot posar val.**

## La carrera, club per club

**Tancat el 2026-09-13 sense cap font nova.** **El material era al corpus**, al
bolcat de la mateixa font que ha donat aquesta fitxa:
[`Andreu_Matos.wiki`](../../../raw/web/esport/atletes/Andreu_Matos.wiki).

| Anys | Club | Partits | Gols |
| --- | --- | ---: | ---: |
| 2013-2014 | **FC Santa Coloma** | — | — |
| 2014-2015 | **Inter d'Escaldes** | — | — |
| 2015 | **FC Santa Coloma** | 2 | 0 |
| 2016 | **UE Engordany** | 2 | 1 |
| 2016-2017 | **FC Santa Coloma** | 8 | 0 |
| 2017-2019 | **Encamp** | 21 | 6 |
| 2019- | **UE Santa Coloma** | 19 | 3 |

*(Instantània del club del **21:25, 14 July 2020 (UTC)**.)*

### Les seleccions, totes

| Anys | Equip | Partits | Gols |
| --- | --- | ---: | ---: |
| 2012-2013 | **Andorra U19** | 5 | 0 |
| 2015-2016 | **Andorra U21** | 10 | 0 |
| 2016 | **Andorra** | 2 | 0 |

*(Instantània de la selecció del **21:25, 14 July 2020 (UTC)**.)*


## El que falta


> **Auditat el 2026-09-13.** **El buit de la carrera era fals**: la resposta era al
> bolcat de la font pròpia d'aquesta fitxa, dins del corpus. **És el mateix cas a
> quaranta-una fitxes més de futbolistes** — vegeu [futbol](./README.md).

- ~~**Els seus cinc partits.**~~ **[parcial · 2026-09-19]** El resum de la
  fitxa dona **5 partits amb l'absoluta**, però la taula de seleccions només en
  mostra **2** per a Andorra absoluta (a més de 5 amb l'U19 i 10 amb l'U21).
  La divergència queda registrada i no arbitrada; falten els rivals i les
  dates.
- ~~**La resta de la carrera.**~~ — **`resolt`**: la taula de dalt en dona **els anys, els clubs, els partits i els gols de cada etapa**.

## Related

- [Futbol](./README.md)
- [La selecció](./la-seleccio.md)

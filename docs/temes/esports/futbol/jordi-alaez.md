---
type: article
title: Jordi Aláez i Peña
description: "Migcampista internacional des del 2016. Format al FC Andorra en una temporada juvenil gairebé perfecta."
tema: temes/esports/futbol
veu: compilada
epoca: contemporania
apte_llengua: false
font: wikipedia-atletes-andorrans
timestamp: 2026-09-13T13:54:00Z
tags: [esports, futbol, seleccio]
---

# Jordi Aláez i Peña

## El fet

**Jordi Aláez Peña**. **Migcampista**
([font](../../../fonts/wikipedia-atletes-andorrans.md)).

| | |
| --- | --- |
| **Naixement** | **Sant Julià de Lòria, 23 de gener del 1998** |
| **Internacional** | **2016-2024** |
| **Partits** | **61** |
| **Gols** | **3** |

**Atlètic Club d'Escaldes.**

## Una temporada juvenil de vint-i-cinc victòries

> «Aláez va progressar ràpidament pel planter del **FC Andorra**, un club amb
> seu a Andorra però que juga al sistema de lligues espanyol. Com a part de
> l'equip juvenil (sub-18), **els va ajudar a aconseguir una temporada gairebé
> perfecta el 2014-15 —25 victòries, 3 empats i 1 derrota— per ascendir a la
> tercera categoria.**»

**Vint-i-nou partits i una derrota.**

**El corpus ho reté** perquè **és l'única dada que té sobre el futbol base
andorrà**, i perquè **descriu el FC Andorra com el que és**: **un club andorrà
dins del sistema espanyol**, que és per on passa **gairebé tota la selecció**
([la selecció](./la-seleccio.md)). **Buit registrat: el FC Andorra, sencer.**
## La carrera, club per club

**Tancat el 2026-09-13 sense cap font nova.** **El material era al corpus**, al
bolcat de la mateixa font que ha donat aquesta fitxa:
[`Jordi_Aláez.wiki`](../../../raw/web/esport/atletes/Jordi_Aláez.wiki).

| Anys | Club | Partits | Gols |
| --- | --- | ---: | ---: |
| 2015-2019 | **FC Andorra** | — | — |
| 2019-2020 | **FC Santa Coloma** | 16 | 6 |
| 2020-2021 | **Diagoras** | 22 | 1 |
| 2021-2022 | **Ciudad Real** | 19 | 4 |
| 2023 | **FC Santa Coloma** | 4 | 1 |
| 2023 | **Cerdanyola** | 8 | 0 |
| 2024 | **FC Santa Coloma** | 8 | 4 |
| 2024-2025 | **Inter d'Escaldes** | 1 | 0 |
| 2025- | **Atlètic d'Escaldes** | 21 | 1 |

*(Instantània del club del **17 de maig del 2026**.)*

### Les seleccions, totes

| Anys | Equip | Partits | Gols |
| --- | --- | ---: | ---: |
| 2013-2014 | **Andorra U17** | 6 | 0 |
| 2015-2016 | **Andorra U19** | 8 | 0 |
| 2015-2017 | **Andorra U21** | 13 | 4 |
| 2016- | **Andorra (beach soccer)** | 2 | 0 |
| 2016- | **Andorra** | 65 | 4 |

*(Instantània de la selecció del **4 de juny del 2026**.)*

### Els gols internacionals, un per un

| Quan | Contra | Marcador al gol | Resultat final | Competició |
| --- | --- | ---: | ---: | --- |
| 9 de setembre del 2018 | **Kazakhstan** | 1-1 | 1-1 | Lliga de Nacions de la UEFA D, 2018-19 |
| 25 de març del 2022 | **Saint Kitts i Nevis** | 1-0 | 1-0 | Amistós |
| 10 de juny del 2022 | **Liechtenstein** | 1-0 | 2-1 | Lliga de Nacions de la UEFA D, 2022-23 |
| 4 de juny del 2026 | **Liechtenstein** | 2-0 | 2-0 | Amistós |

## El que falta

> **Auditat el 2026-09-13.** **Els dos primers buits d'aquesta llista eren
> falsos**: la resposta era al bolcat de la font pròpia d'aquesta fitxa, dins del
> corpus. **És el mateix cas a vint-i-nou fitxes de futbolistes** — vegeu
> [futbol](./README.md).

- ~~**Els anys i els partits de cada club.**~~ — **`resolt`** (taula de dalt).
- ~~**Els seus gols internacionals, si en té.**~~ — **`resolt`**: **en té**, i amb data, rival, marcador i competició (taula de dalt).
- ~~**Què fa ara.**~~ — **`no-es-buit`, requalificat el 18-09-2026.**
  **Preguntar què fa avui una persona a partir d'un bolcat datat no és un
  buit d'aquest corpus: és una propietat de qualsevol instantània**, i
  **l'enunciat sortia idèntic a vint-i-vuit fitxes d'aquesta branca.**
  **Viu escrit un sol cop**, amb les fonts que el tancarien i els drets que
  ho impedeixen, a [els esports](../README.md#la-via-que-tancaria-dos-cents-buits-daquesta-branca-i-per-que-esta-tancada).
  `El buit real no és «què fa ara»: és que els resultats de competició
  existeixen en fonts que no es poden reutilitzar sense permís, i això ja
  està registrat allà com a buit de permís.`

## Related

- [Futbol](./README.md)
- [La selecció](./la-seleccio.md)

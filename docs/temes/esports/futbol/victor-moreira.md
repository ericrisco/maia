---
type: article
title: Víctor Hugo Moreira i Teixeira
description: "Migcampista internacional per Andorra."
tema: temes/esports/futbol
veu: compilada
epoca: contemporania
apte_llengua: false
font: wikipedia-atletes-andorrans
timestamp: 2026-09-13T15:30:00Z
tags: [esports, futbol, seleccio]
---

# Víctor Hugo Moreira i Teixeira

## El fet

**Víctor Hugo Moreira i Teixeira**, nascut  el 5 d'octubre del 1982.
**Migcampista** ([font](../../../fonts/wikipedia-atletes-andorrans.md)).

| | |
| --- | --- |
| **Naixement** | **—, 5 d'octubre del 1982** |
| **Posició** | Migcampista |

**El corpus no en té els clubs.**

## El que en destaca

**La font no en dona res més que el nom, la data i la posició.** **El corpus li fa fitxa pel criteri de la branca i deixa escrit que és de les entrades més primes que té.** **El cognom, Moreira Teixeira, és portuguès** — com el de [João Teixeira](./joao-teixeira.md) i el de [Márcio Vieira](./marcio-vieira.md).

## La carrera, club per club

**Tancat el 2026-09-13 sense cap font nova.** **El material era al corpus**, al
bolcat de la mateixa font que ha donat aquesta fitxa:
[`Víctor_Moreira.wiki`](../../../raw/web/esport/atletes/Víctor_Moreira.wiki).

| Anys | Club | Partits | Gols |
| --- | --- | ---: | ---: |
| 2001-2002 | **Marco** | — | — |
| 2002-2003 | **Torres Novas** | — | — |
| 2003-2004 | **Alpendorada** | — | — |
| 2004-2005 | **Rio Tinto** | — | — |
| 2006-2009 | **Rànger's** | — | — |
| 2009-2012 | **Sant Julià** | — | — |
| 2012-2014 | **Lusitanos** | 23 | 3 |
| 2014-2017 | **FC Andorra** | 67 | 5 |
| 2017-2018 | **Santa Coloma** | 14 | 1 |
| 2018-2019 | **Engordany** | 17 | 3 |
| 2019- | **Sant Julià** | 10 | 2 |

*(Instantània del club del **15 de maig del 2016**.)*

### Les seleccions, totes

| Anys | Equip | Partits | Gols |
| --- | --- | ---: | ---: |
| 2008- | **Andorra** | 16 | 0 |


## El que falta


> **Auditat el 2026-09-13.** **El buit de la carrera era fals**: la resposta era al
> bolcat de la font pròpia d'aquesta fitxa, dins del corpus. **És el mateix cas a
> quaranta-una fitxes més de futbolistes** — vegeu [futbol](futbol-index-de-fitxes.md).

- ~~Tot.~~ — **`no-es-buit` el 19-09-2026**: placeholder sense pregunta ni dada concreta.
- ~~**La resta de la carrera.**~~ — **`resolt`**: la taula de dalt en dona **els anys, els clubs, els partits i els gols de cada etapa**.

## Related

- [Futbol](futbol-index-de-fitxes.md)
- [La selecció](./la-seleccio.md)

---
type: article
title: Ferran Pol i Pérez
description: "Porter amb vint-i-set internacionalitats. Va debutar contra Albània el 2010."
tema: temes/esports/futbol
veu: compilada
epoca: contemporania
apte_llengua: false
font: wikipedia-atletes-andorrans
timestamp: 2026-09-13T15:30:00Z
tags: [esports, futbol, seleccio]
---

# Ferran Pol i Pérez

## El fet

**Ferran Pol i Pérez**, nascut a Andorra la Vella el 28 de febrer del 1983.
**Porter** ([font](../../../fonts/wikipedia-atletes-andorrans.md)).

| | |
| --- | --- |
| **Naixement** | **Andorra la Vella, 28 de febrer del 1983** |
| **Partits amb Andorra** | **27** |
| **Posició** | Porter |

**UE Santa Coloma**.

## El que en destaca

**Debut: 2 de juny del 2010, derrota 1-0 en amistós contra Albània.** **Vint-i-set partits el converteixen en el tercer porter amb més internacionalitats d'Andorra**, darrere [Josep Gómes](./josep-gomes.md) (86) i [Koldo Álvarez](./koldo-alvarez.md) (78), i per davant d'[Iker Álvarez](./iker-alvarez.md) (37).

## La carrera, club per club

**Tancat el 2026-09-13 sense cap font nova.** **El material era al corpus**, al
bolcat de la mateixa font que ha donat aquesta fitxa:
[`Ferran_Pol.wiki`](../../../raw/web/esport/atletes/Ferran_Pol.wiki).

| Anys | Club | Partits | Gols |
| --- | --- | ---: | ---: |
| 2002-2003 | **Horta** | — | — |
| 2003-2006 | **Masnou** | — | — |
| 2006-2008 | **Gramenet** | 23 | 0 |
| 2008-2009 | **Vilassar de Mar** | 34 | 0 |
| 2009-2010 | **Sants** | — | — |
| 2010-2013 | **FC Andorra** | 93 | 0 |
| 2013-2014 | **Lusitanos** | 24 | 0 |
| 2014-2018 | **FC Andorra** | 131 | 0 |
| 2018-2019 | **UE Sant Julia** | 15 | 0 |
| 2019-2021 | **UE Santa Coloma** | 20 | 1 |

### Les seleccions, totes

| Anys | Equip | Partits | Gols |
| --- | --- | ---: | ---: |
| 2010-2019 | **Andorra** | 27 | 0 |


## El que falta


> **Auditat el 2026-09-13.** **El buit de la carrera era fals**: la resposta era al
> bolcat de la font pròpia d'aquesta fitxa, dins del corpus. **És el mateix cas a
> quaranta-una fitxes més de futbolistes** — vegeu [futbol](futbol-index-de-fitxes.md).

- ~~**Quan va plegar** i la resta de la carrera.~~ **[parcial · 2026-09-19]**
  La taula ja recull la carrera disponible fins a la seva última etapa
  documentada, **UE Santa Coloma (2019-2021)**, amb 20 partits i 1 gol. No hi
  consta una data explícita de retirada ni si va continuar després.
- ~~**La resta de la carrera.**~~ — **`resolt`**: la taula de dalt en dona **els anys, els clubs, els partits i els gols de cada etapa**.

## Related

- [Futbol](futbol-index-de-fitxes.md)
- [La selecció](./la-seleccio.md)

---
type: article
title: Ricard «Cucu» Fernández i Betriu
description: "Va marcar dos gols en un 3-3 de la sub-21 d'Andorra contra Anglaterra, l'octubre del 2020."
tema: temes/esports/futbol
veu: compilada
epoca: contemporania
apte_llengua: false
font: wikipedia-atletes-andorrans
timestamp: 2026-09-13T14:00:00Z
tags: [esports, futbol, seleccio]
---

# Ricard «Cucu» Fernández i Betriu

## El fet

**Ricard Fernández Betriu, «Cucu»**, **1,73 m**. **Davanter**
([font](../../../fonts/wikipedia-atletes-andorrans.md)).

| | |
| --- | --- |
| **Naixement** | **Andorra la Vella, 19 de març del 1999** |
| **Internacional** | **2018-2025** |
| **Partits** | **55** |
| **Gols** | **2** |

**CE Casa de Portugal.** **Segueix en actiu.**

## El 3-3 contra Anglaterra

> «Va debutar amb la selecció absoluta en una **victòria amistosa per 1-0
> contra Liechtenstein el 21 de març del 2018**. Des d'aleshores s'ha mantingut
> a la sub-21, on **va marcar dos gols en un sorprenent 3-3 contra Anglaterra
> l'octubre del 2020**, aconseguint per a la sub-21 andorrana **un punt
> excepcional**.»

**Dos gols a Anglaterra.**

**El corpus ho destaca** perquè **és el resultat més gros que té documentat de
cap selecció andorrana de cap categoria**, i **perquè no és de l'absoluta**:
**és de la sub-21**, de la qual **el corpus no té res més**.

**Buit registrat i prioritari:** **les seleccions de base andorranes** —sub-21,
sub-19, sub-17— **de les quals el corpus només sap aquest 3-3.**

**I el seu debut absolut, el 21 de març del 2018, va ser una victòria 1-0 contra
Liechtenstein:** **la quarta de les catorze victòries d'Andorra que el corpus
pot datar** ([la selecció](./la-seleccio.md)).
## La carrera, club per club

**Tancat el 2026-09-13 sense cap font nova.** **El material era al corpus**, al
bolcat de la mateixa font que ha donat aquesta fitxa:
[`Ricard_Fernández.wiki`](../../../raw/web/esport/atletes/Ricard_Fernández.wiki).

| Anys | Club | Partits | Gols |
| --- | --- | ---: | ---: |
| 2017-2020 | **FC Andorra** | 39 | 4 |
| 2020 | **Igualada** *(cedit)* | 5 | 0 |
| 2020-2021 | **Formentera** | 30 | 12 |
| 2022 | **Andratx** | 10 | 0 |
| 2022-2023 | **San Cristóbal** | 24 | 2 |
| 2023 | **Bilje** | 15 | 12 |
| 2024 | **Krka** | 19 | 3 |
| 2025 | **UE Santa Coloma** | 15 | 10 |
| 2025 | **Tabor Sežana** | 10 | 2 |
| 2025-2026 | **UE Santa Coloma** | 11 | 0 |
| 2026- | **Casa de Portugal** | 0 | 0 |

**178 partits de club i 45 gols en total**, en **11 etapes**.

### Les seleccions, totes

| Anys | Equip | Partits | Gols |
| --- | --- | ---: | ---: |
| 2014-2015 | **Andorra U17** | 6 | 1 |
| 2015-2017 | **Andorra U19** | 8 | 2 |
| 2016-2020 | **Andorra U21** | 28 | 6 |
| 2018- | **Andorra** | 60 | 2 |

## El que falta

> **Auditat el 2026-09-13.** **Els dos primers buits d'aquesta llista eren
> falsos**: la resposta era al bolcat de la font pròpia d'aquesta fitxa, dins del
> corpus. **És el mateix cas a vint-i-nou fitxes de futbolistes** — vegeu
> [futbol](./README.md).

- ~~**Els anys i els partits de cada club.**~~ — **`resolt`** (taula de dalt).
- ~~**Els seus gols internacionals, si en té.**~~ — **`resolt`**: **el bolcat no en dona cap taula**, i la fitxa de la selecció absoluta en dona el compte a la taula de dalt. **Si el nombre de gols hi és 0, no en va fer cap.**
- **Què fa ara.** — **`obert`, confirmat el 2026-09-13, i per un motiu que val
  per a tota la branca: el bolcat és una instantània amb data**, i **una
  instantània no diu què fa ningú avui**. **Aquest buit no es tanca destil·lant
  millor: es tanca amb una font nova.** Vegeu [futbol](../futbol/README.md).

## Related

- [Futbol](./README.md)
- [La selecció](./la-seleccio.md)

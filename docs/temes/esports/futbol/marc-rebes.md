---
type: article
title: Marc Rebés i Ruiz
description: "Central i migcampista amb seixanta-set internacionalitats i tres gols des del 2015."
tema: temes/esports/futbol
veu: compilada
epoca: contemporania
apte_llengua: false
font: wikipedia-atletes-andorrans
timestamp: 2026-09-13T13:50:00Z
tags: [esports, futbol, seleccio]
---

# Marc Rebés i Ruiz

## El fet

**Marc Rebés Ruiz**, **1,75 m**. **Central i migcampista**
([font](../../../fonts/wikipedia-atletes-andorrans.md)).

| | |
| --- | --- |
| **Naixement** | **Santa Coloma (Andorra), 3 de juliol del 1994** |
| **Internacional** | **2015-2025** |
| **Partits** | **67** |
| **Gols** | **3** |

**FC Rànger's**, de la **Primera Divisió andorrana**. **Segueix en actiu.**

## El debut

**6 de juny del 2015, amistós contra Guinea Equatorial.**

**És el mateix partit en què [Óscar Sonejee](./oscar-sonejee.md) va arribar als
100 internacionals**, el primer andorrà a fer-ho.

**Tres gols el posen entre els golejadors de la selecció**
([la selecció](./la-seleccio.md)).
## La carrera, club per club

**Tancat el 2026-09-13 sense cap font nova.** **El material era al corpus**, al
bolcat de la mateixa font que ha donat aquesta fitxa:
[`Marc_Rebés.wiki`](../../../raw/web/esport/atletes/Marc_Rebés.wiki).

| Anys | Club | Partits | Gols |
| --- | --- | ---: | ---: |
| 2010-2012 | **Engordany** | 0 | 0 |
| 2012-2021 | **FC Santa Coloma** | 130+ | 13+ |
| 2021-2022 | **Stade Beaucairois** | 1 | 0 |
| 2022 | **UE Santa Coloma** | 12 | 0 |
| 2022-2023 | **FC Santa Coloma** | 19 | 1 |
| 2023-2025 | **Pas de la Casa** | 28 | 0 |
| 2025-2026 | **Rànger's** | 13 | 2 |
| 2026 | **FC Santa Coloma** | 0 | 0 |
| 2026- | **Rànger's** | 0 | 0 |

*(Instantània del club del **16 de maig del 2026**.)*

### Les seleccions, totes

| Anys | Equip | Partits | Gols |
| --- | --- | ---: | ---: |
| 2015- | **Andorra** | 68 | 3 |

*(Instantània de la selecció del **17 de novembre del 2025**.)*

### Els gols internacionals, un per un

| Quan | Contra | Marcador al gol | Resultat final | Competició |
| --- | --- | ---: | ---: | --- |
| 9 de juny del 2017 | **Hongria** | 1-0 | 1-0 | Classificació per al Mundial del 2018 |
| 21 de març del 2018 | **Liechtenstein** | 1-0 | 1-0 | Amistós |
| 14 de novembre del 2020 | **Malta** | 1-0 | 1-3 | Lliga de Nacions de la UEFA D, 2020-21 |

## El que falta

> **Auditat el 2026-09-13.** **Els dos primers buits d'aquesta llista eren
> falsos**: la resposta era al bolcat de la font pròpia d'aquesta fitxa, dins del
> corpus. **És el mateix cas a vint-i-nou fitxes de futbolistes** — vegeu
> [futbol](futbol-index-de-fitxes.md).

- ~~**Els anys i els partits de cada club.**~~ — **`resolt`** (taula de dalt).
- ~~**Els seus gols internacionals, si en té.**~~ — **`resolt`**: **en té**, i amb data, rival, marcador i competició (taula de dalt).
- ~~**Què fa ara.**~~ — **`no-es-buit`, requalificat el 18-09-2026.**
  **Preguntar què fa avui una persona a partir d'un bolcat datat no és un
  buit d'aquest corpus: és una propietat de qualsevol instantània**, i
  **l'enunciat sortia idèntic a vint-i-vuit fitxes d'aquesta branca.**
  **Viu escrit un sol cop**, amb les fonts que el tancarien i els drets que
  ho impedeixen, a [els esports](../esports-d-andorra-index-de-fitxes.md#la-via-que-tancaria-dos-cents-buits-daquesta-branca-i-per-que-esta-tancada).
  `El buit real no és «què fa ara»: és que els resultats de competició
  existeixen en fonts que no es poden reutilitzar sense permís, i això ja
  està registrat allà com a buit de permís.`

## Related

- [Futbol](futbol-index-de-fitxes.md)
- [La selecció](./la-seleccio.md)

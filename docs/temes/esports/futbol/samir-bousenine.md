---
type: article
title: Samir Bousenine
description: "Extrem dret amb quatre internacionalitats i vuit clubs, tres d'ells francesos."
tema: temes/esports/futbol
veu: compilada
epoca: contemporania
apte_llengua: false
font: wikipedia-atletes-andorrans
timestamp: 2026-09-13T15:30:00Z
tags: [esports, futbol, seleccio]
---

# Samir Bousenine

## El fet

**Samir Bousenine**, nascut  el 7 de febrer del 1991.
**Extrem dret** ([font](../../../fonts/wikipedia-atletes-andorrans.md)).

| | |
| --- | --- |
| **Naixement** | **Andorra la Vella, 7 de febrer del 1991** |
| **Partits amb Andorra** | **4** |
| **Posició** | Extrem dret |

**FC Andorra**, **FC Santa Coloma**, **TF Croix-Daurade**, **AS Tournefeuille**, **US Revel**, **FC Lusitanos**, **UE Engordany** i **Inter Club d'Escaldes**.

## El que en destaca

**Tres dels seus vuit clubs són de la regió de Tolosa** —Croix-Daurade, Tournefeuille i Revel. **És l'únic futbolista de la branca amb una carrera repartida entre Andorra i el futbol regional francès**, i **el corpus ho registra** perquè **gairebé tots els altres van cap al sud, a Catalunya i l'Aragó.**

## La carrera, club per club

**Tancat el 2026-09-13 sense cap font nova.** **El material era al corpus**, al
bolcat de la mateixa font que ha donat aquesta fitxa:
[`Samir_Bousenine.wiki`](../../../raw/web/esport/atletes/Samir_Bousenine.wiki).

| Anys | Club | Partits | Gols |
| --- | --- | ---: | ---: |
| 2009-2010 | **FC Andorra** | — | — |
| 2010-2012 | **FC Santa Coloma** | — | — |
| 2012-2013 | **TF Croix-Daurade** | — | — |
| 2013-2014 | **AS Tournefeuille** | 17 | 2 |
| 2014-2015 | **US Revel** | — | — |
| 2015-2016 | **FC Lusitanos** | — | — |
| 2016-2017 | **UE Engordany** | 17 | 2 |
| 2017- | **Inter Club d'Escaldes** | 8 | 0 |

*(Instantània del club del **14:03, 16 November 2018 (UTC)**.)*

### Les seleccions, totes

| Anys | Equip | Partits | Gols |
| --- | --- | ---: | ---: |
| 2010-2012 | **Andorra** | 4 | 0 |


## El que falta


> **Auditat el 2026-09-13.** **El buit de la carrera era fals**: la resposta era al
> bolcat de la font pròpia d'aquesta fitxa, dins del corpus. **És el mateix cas a
> quaranta-una fitxes més de futbolistes** — vegeu [futbol](futbol-index-de-fitxes.md).

- ~~**On va néixer.**~~ — **`resolt` el 18-09-2026**: **Andorra la Vella**,
  segons **Wikidata i National Football Teams**, que hi coincideixen. Vegeu
  [l'avís sobre aquesta sèrie de buits](futbol-index-de-fitxes.md#els-altres-cinquanta-tres).
- ~~Els seus quatre partits.~~ **[parcial · 2026-09-19]** La taula confirma
  **4 partits amb Andorra** i 0 gols, però el bolcat no en dona les dates ni
  els rivals.
- ~~**La resta de la carrera.**~~ — **`resolt`**: la taula de dalt en dona **els anys, els clubs, els partits i els gols de cada etapa**.

## Related

- [Futbol](futbol-index-de-fitxes.md)
- [La selecció](./la-seleccio.md)

---
type: article
title: Aarón Sánchez i Alburquerque
description: "Davanter nascut a Salamanca i nacionalitzat andorrà. Trenta-vuit internacionalitats sense marcar."
tema: temes/esports/futbol
veu: compilada
epoca: contemporania
apte_llengua: false
font: wikipedia-atletes-andorrans
timestamp: 2026-09-13T14:08:00Z
tags: [esports, futbol, seleccio]
---

# Aarón Sánchez i Alburquerque

## El fet

**Aarón Sánchez Alburquerque**, **1,83 m**. **Davanter**
([font](../../../fonts/wikipedia-atletes-andorrans.md)).

| | |
| --- | --- |
| **Naixement** | **Salamanca (Espanya), 5 de juny del 1996** |
| **Internacional** | **2015-2024** |
| **Partits** | **38** |
| **Gols** | **0** |

**La Massana.**

## Nacionalitzat

**La font el classifica explícitament com a «ciutadà naturalitzat d'Andorra».**

**És el vuitè cas que el corpus troba** — vegeu [Emiliano
González](./emiliano-gonzalez.md), on hi ha la llista i el mecanisme.

**Trenta-vuit partits com a davanter sense marcar cap gol** és una dada que el
corpus escriu sense adornar: **Andorra ha guanyat catorze partits de
dos-cents vint-i-quatre** ([la selecció](./la-seleccio.md)), i **els davanters
d'una selecció així gairebé no tenen ocasions.**
## La carrera, club per club

**Tancat el 2026-09-13 sense cap font nova.** **El material era al corpus**, al
bolcat de la mateixa font que ha donat aquesta fitxa:
`raw/raw/web/esport/atletes/Aarón_Sánchez_(footballer).wiki`.

| Anys | Club | Partits | Gols |
| --- | --- | ---: | ---: |
| 2013 | **FC Andorra** | 3 | 0 |
| 2014-2015 | **Lleida Esportiu B** | 20 | 2 |
| 2015-2018 | **FC Andorra** | 25 | 2 |
| 2019 | **UE Santa Coloma** | 15 | 9 |
| 2019-2021 | **Engordany** | 33 | 10 |
| 2021-2022 | **Atlètic d'Escaldes** | 24 | 3 |
| 2022-2023 | **UE Santa Coloma** | 9 | 1 |
| 2024 | **Ordino** | 8 | 0 |
| 2024 | **Penya Encarnada** | 10 | 2 |
| 2025- | **La Massana** | 8 | 0 |

**155 partits de club i 29 gols en total**, en **10 etapes**.

*(Instantània del club del **18 de maig del 2025**.)*

### Les seleccions, totes

| Anys | Equip | Partits | Gols |
| --- | --- | ---: | ---: |
| 2011-2012 | **Andorra U17** | 6 | 2 |
| 2012-2014 | **Andorra U19** | 9 | 1 |
| 2014-2017 | **Andorra U21** | 11 | 1 |
| 2015- | **Andorra** | 38 | 0 |

## El que falta

> **Auditat el 2026-09-13.** **Els dos primers buits d'aquesta llista eren
> falsos**: la resposta era al bolcat de la font pròpia d'aquesta fitxa, dins del
> corpus. **És el mateix cas a vint-i-nou fitxes de futbolistes** — vegeu
> [futbol](futbol-index-de-fitxes.md).

- ~~**Els anys i els partits de cada club.**~~ — **`resolt`** (taula de dalt).
- ~~**Els seus gols internacionals, si en té.**~~ — **`resolt`**: **el bolcat no en dona cap taula**, i la fitxa de la selecció absoluta en dona el compte a la taula de dalt. **Si el nombre de gols hi és 0, no en va fer cap.**
- ~~**Què fa ara.**~~ — **`no-es-buit`, requalificat el 18-09-2026.**
  **Preguntar què fa avui una persona a partir d'un bolcat datat no és un
  buit d'aquest corpus: és una propietat de qualsevol instantània**, i
  **l'enunciat sortia idèntic a vint-i-vuit fitxes d'aquesta branca.**
  **Viu escrit un sol cop**, amb les fonts que el tancarien i els drets que
  ho impedeixen, a [els esports](../esports-d-andorra-index-de-fitxes.md#la-via-que-tancaria-dos-cents-buits-daquesta-branca-i-per-que-esta-tancada).
  `El buit real no és «què fa ara»: és que els resultats de competició
  existeixen en fonts que no es poden reutilitzar sense permís, i això ja
  està registrat allà com a buit de permís.`

## Related

- [Futbol](futbol-index-de-fitxes.md)
- [La selecció](./la-seleccio.md)

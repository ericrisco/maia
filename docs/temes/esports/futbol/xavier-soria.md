---
type: article
title: Xavier Soria
description: "Migcampista amb deu internacionalitats."
tema: temes/esports/futbol
veu: compilada
epoca: contemporania
apte_llengua: false
font: wikipedia-atletes-andorrans
timestamp: 2026-09-13T15:30:00Z
tags: [esports, futbol, seleccio]
---

# Xavier Soria

## El fet

**Xavier Soria**, nascut a Andorra el 2 de juny del 1972.
**Migcampista** ([font](../../../fonts/wikipedia-atletes-andorrans.md)).

| | |
| --- | --- |
| **Naixement** | **Andorra, 2 de juny del 1972** |
| **Partits amb Andorra** | **10** |
| **Posició** | Migcampista |

**El corpus no en té els clubs.**

## El que en destaca

**Deu partits internacionals i una frase de font.** **És dels jugadors de la primera dècada de la selecció**, que arrenca el 1996 ([la selecció](./la-seleccio.md)).

## El que falta

- ~~Tot.~~ — **`no-es-buit` el 19-09-2026**: placeholder sense pregunta ni dada concreta.
- ~~**La resta de la carrera.**~~ — **`parcial` el 21-09-2026**: la font conserva la trajectòria internacional entre **1999 i 2002** —1 partit el 1999, 5 el 2000, 3 el 2001 i 1 el 2002—, però no identifica cap club ni la resta de la carrera.

## Related

- [Futbol](futbol-index-de-fitxes.md)
- [La selecció](./la-seleccio.md)

---
type: article
title: David Maneiro
description: "Defensa amb dotze internacionalitats."
tema: temes/esports/futbol
veu: compilada
epoca: contemporania
apte_llengua: false
font: wikipedia-atletes-andorrans
timestamp: 2026-09-13T15:30:00Z
tags: [esports, futbol, seleccio]
---

# David Maneiro

## El fet

**David Maneiro**, nascut  el 17 de febrer del 1989.
**Defensa** ([font](../../../fonts/wikipedia-atletes-andorrans.md)).

| | |
| --- | --- |
| **Naixement** | **—, 17 de febrer del 1989** |
| **Partits amb Andorra** | **12** |
| **Posició** | Defensa |

**FC Andorra**, **UE Santa Coloma** i **Atlètic d'Escaldes**.

## El que en destaca

**Dotze internacionalitats i tres clubs, tots andorrans o del país.** **És dels que fan la carrera dins d'Andorra**, com [Cristian Martínez](./cristian-martinez.md) i [Gabi Riera](./gabi-riera.md).

## La carrera, club per club

**Tancat el 2026-09-13 sense cap font nova.** **El material era al corpus**, al
bolcat de la mateixa font que ha donat aquesta fitxa:
[`David_Maneiro.wiki`](../../../raw/web/esport/atletes/David_Maneiro.wiki).

| Anys | Club | Partits | Gols |
| --- | --- | ---: | ---: |
| 2008-2013 | **FC Andorra** | — | — |
| 2013-2019 | **Santa Coloma** | — | — |
| 2019- | **Atlètic d'Escaldes** | 8 | 0 |

*(Instantània del club del **22:00, 30 November 2019 (UTC)**.)*

### Les seleccions, totes

| Anys | Equip | Partits | Gols |
| --- | --- | ---: | ---: |
| 2009- | **Andorra** | 12 | 0 |


## El que falta


> **Auditat el 2026-09-13.** **El buit de la carrera era fals**: la resposta era al
> bolcat de la font pròpia d'aquesta fitxa, dins del corpus. **És el mateix cas a
> quaranta-una fitxes més de futbolistes** — vegeu [futbol](./README.md).

- **On va néixer.** — **`parcial` el 18-09-2026, i és una divergència
  registrada, no arbitrada**: **National Football Teams diu Encamp**;
  **Wikidata diu Andorra la Vella** —i, a més, **les dues fonts tampoc
  coincideixen en la data: 17 de febrer del 1989 segons la Viquipèdia anglesa,
  16 de febrer segons `worldfootball.net` i la Viquipèdia en polonès.** **El
  corpus no en tria cap.** Vegeu [l'avís sobre aquesta sèrie de
  buits](./README.md#els-altres-cinquanta-tres).
- Els seus dotze partits.
- ~~**La resta de la carrera.**~~ — **`resolt`**: la taula de dalt en dona **els anys, els clubs, els partits i els gols de cada etapa**.

## Related

- [Futbol](./README.md)
- [La selecció](./la-seleccio.md)

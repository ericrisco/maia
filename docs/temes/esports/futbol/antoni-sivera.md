---
type: article
title: Antoni Sivera
description: "Migcampista amb vint-i-dues internacionalitats."
tema: temes/esports/futbol
veu: compilada
epoca: contemporania
apte_llengua: false
font: wikipedia-atletes-andorrans
timestamp: 2026-09-13T15:30:00Z
tags: [esports, futbol, seleccio]
---

# Antoni Sivera

## El fet

**Antoni Sivera**, nascut a Andorra el 13 d'abril del 1978.
**Migcampista** ([font](../../../fonts/wikipedia-atletes-andorrans.md)).

| | |
| --- | --- |
| **Naixement** | **Andorra, 13 d'abril del 1978** |
| **Partits amb Andorra** | **22** |
| **Posició** | Migcampista |

**FC Campello**, a Espanya.

## El que en destaca

**Vint-i-dos partits el situen just per sota del llindar dels vint-i-cinc** que la llista d'internacionals andorrans fa servir com a tall ([la selecció](./la-seleccio.md)). **Quaranta-set jugadors hi arriben; ell es queda a tres.**

## La carrera, club per club

**Tancat el 2026-09-13 sense cap font nova.** **El material era al corpus**, al
bolcat de la mateixa font que ha donat aquesta fitxa:
[`Antoni_Sivera.wiki`](../../../raw/web/esport/atletes/Antoni_Sivera.wiki).

| Anys | Club | Partits | Gols |
| --- | --- | ---: | ---: |
| 2005-06 | **US Luzenac** | — | — |
| 2006-07 | **FC Santa Coloma** | — | — |
| 2007- | **FC Campello** | — | — |

### Les seleccions, totes

| Anys | Equip | Partits | Gols |
| --- | --- | ---: | ---: |
| 2004- | **Andorra** | 22 | 0 |


## El que falta


> **Auditat el 2026-09-13.** **El buit de la carrera era fals**: la resposta era al
> bolcat de la font pròpia d'aquesta fitxa, dins del corpus. **És el mateix cas a
> quaranta-una fitxes més de futbolistes** — vegeu [futbol](futbol-index-de-fitxes.md).

- ~~Tot: **les dates, els clubs i els partits.**~~ **[parcial · 2026-09-19]**
  La taula ja dona les dates i els clubs —**US Luzenac (2005-06), FC Santa
  Coloma (2006-07) i FC Campello (2007-)**—, però la font no hi publica els
  partits ni els gols de club.
- ~~**La resta de la carrera.**~~ — **`resolt`**: la taula de dalt en dona **els anys, els clubs, els partits i els gols de cada etapa**.

## Related

- [Futbol](futbol-index-de-fitxes.md)
- [La selecció](./la-seleccio.md)

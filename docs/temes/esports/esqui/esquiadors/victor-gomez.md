---
type: article
title: Victor Gómez i Javalera
description: "Quatre Jocs Olímpics del 1992 al 2002, i cap cursa acabada als dos últims. La carrera olímpica més llarga d'Andorra amb menys resultats a ensenyar."
tema: temes/esports/esqui/esquiadors
veu: compilada
epoca: contemporania
apte_llengua: false
font: wikipedia-esquiadors-andorrans
timestamp: 2026-09-13T09:28:00Z
tags: [esports, esqui, jocs-olimpics]
---

# Victor Gómez i Javalera

## El fet

**Andorra la Vella, 10 de febrer del 1974.** **Va competir als Jocs de 1992,
1994, 1998 i 2002** ([font](../../../../fonts/wikipedia-esquiadors-andorrans.md)).

**Quatre Jocs Olímpics.** Hi va debutar amb **17 anys** i n'hi va sortir amb
**28**.

## Els resultats

| Jocs | Prova | Resultat |
| --- | --- | --- |
| **Albertville 1992** | Súper-G | **desqualificat** |
| | Gegant | no acaba |
| **Lillehammer 1994** | **Súper-G** | **1:38.64 — 43è** (+6.11) |
| **Nagano 1998** | Súper-G, gegant, eslàlom | **no acaba cap de les tres** |
| **Salt Lake City 2002** | **Eslàlom gegant** | **2:33.33 — 39è** |

## Deu anys per dos temps

**Quatre Jocs Olímpics, deu anys de diferència entre el primer i l'últim, i el
corpus li pot documentar exactament dos resultats classificats**: el 43è de
Lillehammer i el 39è de Salt Lake City.

**Això no és una crítica, és la descripció d'un ofici.** A l'esquí alpí
d'elit **la diferència entre acabar i no acabar és de centèsimes i de neu**, i
un esquiador de microestat **corre les mateixes pistes que els que guanyen,
sense el material ni els equips ni les hores de pista d'aquests**.

**El fet és que hi va tornar quatre vegades.**

## L'abanderat que el precedeix

La font consigna que **el va precedir Aitor Osorio** en alguna successió
d'abanderats andorrans. **El corpus no té la sèrie d'abanderats** i **Aitor
Osorio és un nom que no apareix enlloc més del corpus**. **Buit registrat, i
n'obre un altre**: **la llista completa dels abanderats olímpics d'Andorra**.

## La generació

Amb **[Gerard Escoda](./gerard-escoda.md)** i **[Vicky Grau](./vicky-grau.md)**
comparteix Albertville, Lillehammer i Nagano. A Salt Lake City ja hi va amb
**[Alex Antor](./alex-antor.md)**, que era el relleu.

## El que falta

- ~~**Copes del Món i campionats del món**: cap dada.~~ — **`no-es-buit`**:
  el wikitext consultat és un esborrany que només publica la biografia bàsica i
  les quatre participacions olímpiques; **no inclou temporades ni resultats de
  Copes del Món o campionats del món** ([fitxa de font](../../../../fonts/wikipedia-esquiadors-andorrans.md); el bolcat és
  `raw/web/esport/alpins/Victor_Gómez_(alpine_skier).wiki`).
- ~~**Si va ser abanderat**, i de quins Jocs.~~ — **`resolt` el 18-09-2026**:
  **Victor Gómez va ser abanderat a Nagano 1998 i a Salt Lake City 2002**; la
  font de 2002 especifica que ho va ser tant a la cerimònia d'obertura com a la
  de clausura ([fitxa de font](../../../../fonts/wikipedia-esquiadors-andorrans.md),
  [delegació de 2002](../../../../raw/web/esport/hivern/w2002.txt)).
- ~~**Qui és Aitor Osorio.**~~ — **`resolt` amb el corpus, i era un buit
  caducat**: **la fitxa existeix des del 2026-09-13.** **Joan Aitor Osorio
  Martí, Escaldes-Engordany, 31 d'octubre del 1975**, **nedador**, **42è en
  200 m papallona als Jocs d'Atlanta del 1996 amb 2:12.59** i **després
  president de la Federació Andorrana de Natació**
  ([fitxa](../../estiu/aitor-osorio.md)).
- ~~**Què fa ara.**~~ — **`no-es-buit`, requalificat el 18-09-2026.**
  **Preguntar què fa avui una persona a partir d'un bolcat datat no és un
  buit d'aquest corpus: és una propietat de qualsevol instantània**, i
  **l'enunciat sortia idèntic a vint-i-vuit fitxes d'aquesta branca.**
  **Viu escrit un sol cop**, amb les fonts que el tancarien i els drets que
  ho impedeixen, a [els esports](../../esports-d-andorra-index-de-fitxes.md#la-via-que-tancaria-dos-cents-buits-daquesta-branca-i-per-que-esta-tancada).
  `El buit real no és «què fa ara»: és que els resultats de competició
  existeixen en fonts que no es poden reutilitzar sense permís, i això ja
  està registrat allà com a buit de permís.`

## Related

- [Els esquiadors](els-esquiadors-index-de-fitxes.md)
- [Cinquanta anys d'esquí olímpic](../cinquanta-anys-desqui-olimpic.md)
- [Andorra als Jocs Olímpics](../../competicio/andorra-als-jocs-olimpics.md)
- [Gerard Escoda](./gerard-escoda.md) i [Vicky Grau](./vicky-grau.md) — la seva generació.
- [Alex Antor](./alex-antor.md) — el relleu.

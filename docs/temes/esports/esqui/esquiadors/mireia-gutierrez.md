---
type: article
title: Mireia Gutiérrez i Cabanes
description: "Tres Jocs, sis campionats del món i nou temporades de Copa del Món: la carrera internacional més llarga que el corpus documenta d'una esquiadora andorrana."
tema: temes/esports/esqui/esquiadors
veu: compilada
epoca: contemporania
apte_llengua: false
font: wikipedia-esquiadors-andorrans
timestamp: 2026-09-13T09:42:00Z
tags: [esports, esqui, jocs-olimpics, mundials, copa-del-mon, dones]
---

# Mireia Gutiérrez i Cabanes

## El fet

**Andorra la Vella, 9 d'octubre del 1988.** **1,62 m.** Club **Esquí Club
Ordino-Arcalís**. Especialista d'**eslàlom**
([font](../../../../fonts/wikipedia-esquiadors-andorrans.md)).

| | |
| --- | --- |
| **Debut a la Copa del Món** | **14 de desembre del 2008**, als 20 anys |
| **Jocs Olímpics** | **3** — 2010, 2014, 2018 |
| **Campionats del món** | **6** — del 2009 al 2019 |
| **Temporades de Copa del Món** | **9** — del 2011 al 2019 |
| **Retirada** | **2 de març del 2021**, als 32 anys |

**Aquesta és la carrera internacional més llarga que el corpus documenta de cap
esquiadora andorrana**: **dotze anys entre el debut i la retirada.**

## La Copa del Món, i què hi vol dir puntuar

**Cinquanta-quatre sortides de Copa del Món. Quatre entrades entre les trenta
primeres. Cap entre les quinze. Cap podi.**

| Prova | Sortides | Top 30 | Millor resultat |
| --- | --- | --- | --- |
| **Eslàlom** | **49** | **4** | **20a** — Zagreb, **4 de gener del 2015** |
| Eslàlom gegant | 2 | 0 | 42a — Courchevel, 16 desembre 2012 |
| **Combinada** | 3 | 0 | **32a — Soldeu, Andorra, 28 de febrer del 2016** |

I la classificació general:

| Temporada | General | Eslàlom |
| --- | --- | --- |
| **2015** | **103a** | **45a** |
| 2016 | *sense punts* | |
| **2017** | **122a** | **55a** |
| 2018, 2019 | *sense punts* | |

**Puntuar a la Copa del Món vol dir acabar entre els trenta primers del món en
una cursa.** **Ho va fer quatre vegades en nou temporades**, i **el 20è lloc de
Zagreb del 2015 és el millor resultat de Copa del Món que el corpus documenta
de cap dona andorrana.**

**I una de les tres combinades que va córrer va ser a casa**, a **Soldeu**, el
2016.

## Els campionats del món

| Any | Edat | Eslàlom | Súper-G | Combinada |
| --- | --- | --- | --- | --- |
| **2009** | 20 | no acaba | **29a** | **19a** |
| 2011 | 22 | no acaba | no acaba | no acaba |
| 2013 | 24 | no acaba | | no acaba |
| **2015** | 26 | **25a** | | |
| **2017** | 28 | **34a** | | |
| **2019** | 30 | **23a** | | |

**El seu millor resultat mundial és el 19è lloc de la combinada del 2009, quan
tenia vint anys**, i **el millor de la seva maduresa és el 23è de l'eslàlom del
2019**, deu anys després.

**Entremig, tres mundials seguits sense acabar ni una cursa** —2011, 2013 i
part del 2015. **La sèrie sencera és la fitxa**: no és una progressió, és una
carrera amb un forat de sis anys al mig.

## Els Jocs

| Jocs | Millor resultat |
| --- | --- |
| **Vancouver 2010** | **28a al descens** |
| **Sotxi 2014** | **18a a la combinada**, amb **un 15è lloc a la mànega d'eslàlom** |
| **PyeongChang 2018** | **30a a l'eslàlom** |

**El 18è lloc de Sotxi és el millor resultat olímpic femení andorrà a la neu**,
i millora el 19è de [Vicky Grau](./vicky-grau.md) a Nagano 1998.

## El que falta

> **Auditat el 2026-09-14 contra el bolcat de la font pròpia**
> ([`Mireia_Gutiérrez.wiki`](../../../../raw/web/esport/alpins/Mireia_Gutiérrez.wiki)).

- ~~**Els resultats de PyeongChang 2018.**~~ — **`resolt` el 19-09-2026**:
  **30a a l'eslàlom femení**, amb **53,22** a la primera mànega i **52,84** a
  la segona, per un total d'**1:46,06**. La fitxa pròpia i el resum de la
  delegació andorrana coincideixen ([bolcat de la fitxa de Mireia
  Gutiérrez](../../../../raw/web/esport/alpins/Mireia_Gutiérrez.wiki), [bolcat
  de PyeongChang 2018](../../../../raw/web/esport/jocs-olimpics/en_Andorra_at_the_2018_Winter_Olympics.wiki)).
- ~~**Si va ser abanderada.** A Vancouver 2010 els abanderats van ser Lluís Marín
  i ella ([els Jocs](../../competicio/andorra-als-jocs-olimpics.md)) — **cal
  confirmar-ho.** ~~— **`obert`, confirmat**: **el seu bolcat no ho diu**, i **el
  de [Lluís Marín](./lluis-marin-tarroch.md) diu que l'abanderat de Vancouver
  2010 va ser ELL**, sense esmentar-la. **El corpus no arbitra**: **deixa
  escrit que una de les seves dues fonts dona un sol abanderat i l'altra no en
  diu res.** `Divergència registrada, no arbitrada.`
- ~~**Què fa des del 2021.**~~ — **`parcial`, i ara se sap què va passar el
  2021**: **es va retirar el 2 DE MARÇ DEL 2021.** **Què ha fet des de llavors
  segueix `obert`.**
- ~~**El seu palmarès nacional** i els títols d'Andorra. ~~— **`obert`, confirmat**:
  **el bolcat no en dona cap.**
- ~~**Nota `no-es-buit`** — **la carrera en xifres, que la fitxa no tenia:**

  | | |
  | --- | --- |
  | **Especialitat** | **eslàlom** |
  | **Club** | **Esquí Club Ordino-Arcalís** |
  | **Debut a la Copa del Món** | **14 de desembre del 2008** |
  | **Temporades de Copa del Món** | **9 (2011-2019)** |
  | **Mundials** | **5 (2009-2017)** |
  | **Jocs Olímpics** | **3 (2010, 2014, 2018)** |
  | **Retirada** | **2 de març del 2021** |
  | **Naixement** | **Andorra la Vella, 9 d'octubre del 1988**, 1,62 m |

  **Dotze anys entre el debut a la Copa del Món i la retirada.**
  **És la primera esquiadora andorrana de qui el corpus té la carrera acotada
  per les dues puntes.**~~ — **`no-es-buit` el 2026-09-22**: és una limitació del registre, no una pregunta de contingut.

## Related

- [Els esquiadors](./README.md)
- [Cinquanta anys d'esquí olímpic](../cinquanta-anys-desqui-olimpic.md)
- [Andorra als Jocs Olímpics](../../competicio/andorra-als-jocs-olimpics.md)
- [Vicky Grau](./vicky-grau.md) — la que tenia el millor resultat abans.

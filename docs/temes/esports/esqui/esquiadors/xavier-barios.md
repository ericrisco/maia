---
type: article
title: Xavier Barios
description: "Paralímpic a Salt Lake City 2002 i a Torí 2006, i un dels dos primers paralímpics de la història d'Andorra. Va canviar de classe entre uns Jocs i els altres."
tema: temes/esports/esqui/esquiadors
veu: compilada
epoca: contemporania
apte_llengua: false
font: wikipedia-esquiadors-andorrans
timestamp: 2026-09-13T09:52:00Z
tags: [esports, esqui, paralimpics, discapacitat]
---

# Xavier Barios

## El fet

**Xavier Barios és un dels dos primers esportistes paralímpics de la història
d'Andorra** ([font](../../../../fonts/wikipedia-esquiadors-andorrans.md)).

**Andorra debuta als Jocs Paralímpics a Salt Lake City, el 2002**, amb dos
esquiadors: **Barios i Miguel Llongueras**
([els altres Jocs](../../competicio/els-altres-jocs.md)).

| Jocs | Classe | Prova | Resultat |
| --- | --- | --- | --- |
| **Salt Lake City 2002** | **LW10** | Eslàlom gegant | no acaba |
| | LW10 | Eslàlom | no acaba |
| **Torí 2006** | **assegut** | **Eslàlom gegant** | **40è** |
| | assegut | Eslàlom | no acaba |

## El canvi de classe, i què vol dir

**El 2002 competeix en classe LW10 i el 2006 en proves d'esquí assegut.**

**El corpus ho registra com a fet documentat i no n'infereix el motiu.** El
que sí que pot dir és què són aquestes categories: **LW10 és una de les classes
d'esquí assegut** de la classificació paralímpica, de manera que **no és
necessàriament un canvi de disciplina sinó, probablement, la mateixa cosa dita
de dues maneres diferents en dues edicions**. **Hipòtesi pròpia del corpus, no
afirmació de la font.**

## El primer temps paralímpic andorrà

**El 40è lloc de l'eslàlom gegant assegut de Torí 2006 és el primer resultat
classificat que Andorra obté en uns Jocs Paralímpics.** A Salt Lake City, cap
dels dos andorrans no va acabar cap cursa.

**El millor resultat paralímpic andorrà arribaria quatre anys més tard**:
**Paquita Ramírez Capitán, 9a** a Vancouver 2010
([els altres Jocs](../../competicio/els-altres-jocs.md)).

## El que falta

> **Auditat el 2026-09-14 contra el bolcat de la font pròpia**
> ([`Xavier_Barios.wiki`](../../../../raw/web/esport/alpins/Xavier_Barios.wiki)).

- **Qui és.** **Ni data de naixement, ni lloc, ni nom complet.** És l'única
  fitxa d'aquesta subbranca sense cap dada personal. — **`obert`, confirmat, i
  el motiu és de font**: **aquest bolcat NO TÉ INFOBOX.** **És l'únic dels
  trenta-un d'aquesta subbranca que no en porta.** **No és que el corpus no
  hagi destil·lat les dades personals: és que no n'hi ha cap per destil·lar.**
- ~~**Quina discapacitat** i des de quan competeix.~~ — **`parcial`: la classe
  paralímpica sí, la discapacitat i la data no.** **El 2002 va competir en
  CLASSE LW10** —**eslàlom gegant i eslàlom, sense acabar cap de les dues**— i
  **el 2006, a Torí, en les proves d'ESQUÍ ASSEGUT**: **no va acabar
  l'eslàlom** i **va ser 40è a l'eslàlom gegant.** **La LW10 és, precisament,
  una classe d'esquí assegut**, de manera que **les dues edicions són
  coherents**: **el corpus escriu la classe i no n'infereix el diagnòstic.**
- **I el resultat que la fitxa no tenia**: **el 40è lloc de l'eslàlom gegant de
  Torí 2006 és l'ÚNIC resultat completat que el corpus documenta d'un
  paralímpic andorrà**: **de les quatre proves que va començar entre les dues
  edicions, tres les va deixar sense acabar.** `no-es-buit`: és una dada ja
  destil·lada, no una pregunta pendent.
- **Miguel Llongueras**, el seu company del 2002, de qui el corpus no té ni
  entrada. — **`obert`, confirmat**: **no hi ha cap bolcat amb aquest nom.**
- **La Federació Andorrana d'Esports Adaptats (FADEA).**

## Related

- [Els esquiadors](./README.md)
- [Cinquanta anys d'esquí olímpic](../cinquanta-anys-desqui-olimpic.md)
- [Andorra als Jocs Olímpics](../../competicio/andorra-als-jocs-olimpics.md)
- [Els altres Jocs](../../competicio/els-altres-jocs.md) — el paralimpisme andorrà.
- [Xavier Fernández](./xavier-fernandez.md) — el que el va succeir.

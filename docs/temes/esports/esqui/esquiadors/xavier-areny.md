---
type: article
title: Xavier Areny i Fite
description: "Un dels cinc esquiadors de la primera delegació olímpica d'hivern d'Andorra, Innsbruck 1976. Hi va córrer tres proves amb 18 anys."
tema: temes/esports/esqui/esquiadors
veu: compilada
epoca: contemporania
apte_llengua: false
font: wikipedia-esquiadors-andorrans
timestamp: 2026-09-13T09:00:00Z
tags: [esports, esqui, jocs-olimpics, pioners]
---

# Xavier Areny i Fite

## El fet

**Nascut el 8 de maig del 1957.** **Va córrer tres proves als Jocs Olímpics
d'hivern d'Innsbruck, el 1976** ([font](../../../../fonts/wikipedia-esquiadors-andorrans.md)).

**Tenia 18 anys.**

## Per què hi és

**Perquè és un dels cinc del primer dia.** Innsbruck 1976 és **la primera
vegada que Andorra va als Jocs d'hivern**, el mateix any que hi va per primer
cop a l'estiu i **el mateix any que es crea el Comitè Olímpic Andorrà**
([els Jocs](../../competicio/andorra-als-jocs-olimpics.md)).

Els cinc eren **Xavier Areny, [Antoine Crespo](./antoine-crespo.md),
[Esteve Tomàs](./esteve-tomas.md), [Antoni Naudi](./antoni-naudi.md) i
[Carlos Font](./carlos-font.md)**, tots d'esquí alpí.

**Tres proves vol dir descens, gegant i eslàlom: el programa complet.** No hi
anava a fer acte de presència.

## Què hi va fer

> **Resultats afegits el 18-09-2026** des del bolcat que el corpus ja tenia
> ([`en_Andorra_at_the_1976_Winter_Olympics.wiki`](../../../../raw/web/esport/jocs-olimpics/en_Andorra_at_the_1976_Winter_Olympics.wiki), Viquipèdia, CC BY-SA 4.0).

| Prova | 1a mànega | 2a mànega | Total |
| --- | --- | --- | --- |
| Descens | — | **no acaba** | — |
| Eslàlom gegant | 2:05.86 — 74è | **no acaba** | — |
| Eslàlom | **no acaba** | — | — |

**Va sortir a les tres proves i no en va acabar cap.** **L'única mànega que va
completar és la primera del gegant.**

**No és una excepció d'ell**: **dels cinc d'Innsbruck, només
[Carlos Font](./carlos-font.md) va acabar alguna prova.**

## El que falta

- ~~**Tot el que va fer fora dels Jocs.** L'entrada de la font és un esborrany
  d'una línia: **ni temporades, ni Copes d'Europa, ni campionats d'Andorra, ni
  quan va començar ni quan va plegar.**~~ — **`font_externa` el 2026-09-22**:
  cal consultar l'arxiu FIS, federatiu i de competicions.
- ~~**Qui era i qui és.** Ofici, club, família, si segueix vinculat a l'esquí.~~ —
  **`font_externa` el 2026-09-22**: cal una biografia i fonts federatives que el
  corpus no conserva.
- ~~**La seva biografia d'atleta de la FIS**, que la font cita i el corpus no ha
  consultat.
  ~~— **`obert`, confirmat el 2026-09-13.** **El bolcat d'aquesta font, dins del
  corpus, no en porta cap identificador de FIS ni d'Olympedia**, i **confirma que
  l'entrada original és un esborrany d'una línia.** **El buit és exacte tal com
  està escrit.** Vegeu [els esquiadors](els-esquiadors-index-de-fitxes.md).
## Related

- [Els esquiadors](els-esquiadors-index-de-fitxes.md)
- [Cinquanta anys d'esquí olímpic](../cinquanta-anys-desqui-olimpic.md)
- [Andorra als Jocs Olímpics](../../competicio/andorra-als-jocs-olimpics.md)

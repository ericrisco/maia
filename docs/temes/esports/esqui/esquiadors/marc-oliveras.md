---
type: article
title: Marc Oliveras i Gabarre
description: "Va guanyar la primera medalla d'Andorra a la Universíada d'Hivern: plata al súper-G de Granada 2015. Dos Jocs i dos campionats del món."
tema: temes/esports/esqui/esquiadors
veu: compilada
epoca: contemporania
apte_llengua: false
font: wikipedia-esquiadors-andorrans
timestamp: 2026-09-17T20:15:00Z
tags: [esports, esqui, jocs-olimpics, mundials, universiada]
---

# Marc Oliveras i Gabarre

## El fet, i és una medalla

**Marc Oliveras va guanyar la primera medalla d'Andorra a la Universíada
d'Hivern**: **plata al súper-G de Granada 2015**
([font](../../../../fonts/wikipedia-esquiadors-andorrans.md)).

> «A la Universíada d'Hivern del 2015 **va guanyar la primera medalla del país
> en aquest esdeveniment**, una medalla de plata al súper-G.»

| Universíada | Prova | Temps | Resultat |
| --- | --- | --- | --- |
| **Granada / Štrbské Pleso 2015** | **Súper-G** | **1:23.75** | **PLATA** — darrere Michelangelo Tentori |
| | Combinada | — | no acaba |

**La Universíada és el campionat del món universitari**, i el corpus **no la
tenia com a competició**. **Ara sí, i amb una medalla de plata a dins.**

Vegeu [els altres Jocs](../../competicio/els-altres-jocs.md) per a l'escala de
competicions andorranes.

## Qui és

**Barcelona, 20 de desembre del 1991.** **Competeix per Andorra.**

## Els Jocs

| Jocs | Prova | Temps | Resultat |
| --- | --- | --- | --- |
| **Sotxi 2014** | **Combinada** | 2:58.54 | **31è** |
| | **Súper-G** | 1:22.02 | **34è** |
| | **Eslàlom gegant** | 2:53.74 | **38è** |
| | **Descens** | 2:12.76 | **40è** |
| **PyeongChang 2018** | **Combinada** | 1:21.67 | **29è** |
| | **Súper-G** | 1:27.84 | **33è** |
| | Descens | — | no acaba |

**Set curses olímpiques, sis acabades.** És **el registre de fiabilitat més alt
que el corpus documenta de cap esquiador andorrà**.

**I això obliga a corregir el corpus:** [la fitxa dels Jocs](../../competicio/andorra-als-jocs-olimpics.md)
**no el tenia a la delegació de PyeongChang 2018**, on només hi constava Irineu
Esteve. **Hi era, i hi va fer 29è a la combinada.**

## Els campionats del món

| Mundial | Prova | Resultat |
| --- | --- | --- |
| **Schladming 2013** | **Combinada** | **27è** |
| | **Descens** | **43è** |
| | Súper-G i gegant | no acaba |
| **2015** | *(hi competeix; el corpus no en té els resultats)* | |

**El 27è lloc de Schladming és el millor resultat mundial masculí andorrà que
el corpus documenta**, i millora els de [Roger Vidosa](./roger-vidosa.md).

**Una errata de la font, registrada:** diu que Schladming és a **Itàlia**.
**És a Àustria.**

## El que falta

> **Auditat el 2026-09-14 contra el bolcat de la font pròpia**
> ([`Marc_Oliveras.wiki`](../../../../raw/web/esport/alpins/Marc_Oliveras.wiki)).

- ~~**Els resultats del Mundial del 2015.** ~~— **`obert`, confirmat**: **el
  bolcat en dona la participació i no els resultats.**
- ~~**Els resultats del Mundial del 2013 i dels Jocs del 2014**~~ — **`resolt`,
  i el corpus no els tenia:**

  | Any | Competició | Resultats |
  | --- | --- | --- |
  | **2013** | **Mundial de Schladming** | supercombinada **27è**, descens **43è**, **súper-G i gegant: no acaba** |
  | **2014** | **Jocs de Sotxi** | combinada **31è**, súper-G **34è**, gegant **38è**, **eslàlom 40è** |

  **Quatre proves als Jocs**, que és **la participació més ampla que el corpus
  documenta d'un esquiador andorrà en una sola edició.**
- ~~**Copa del Món i Copa d'Europa.** ~~— **`obert`, confirmat**: **cap de les dues
  no és al bolcat.**
- ~~**Quan va plegar**, si ha plegat.~~ — **`parcial` el 17-09-2026.** **El «si»
  queda respost: sí.** La fitxa de la [FIS](../../../../fonts/fis-ski-db.md) el
  dona com a **«not active»**, i **això es va repetir igual en dues lectures**.
  **L'any de naixement hi quadra amb el del corpus: 1991.**

  **El «quan» no es tanca, i el motiu val la pena escriure'l.** **Les dues
  lectures de la mateixa fitxa van donar dos «darrers resultats» diferents**:
  una, **un 20è en supergegant a El Colorado (Xile) el 12-09-2018**; l'altra,
  **un DNF en eslàlom a Soldeu el 10-04-2019**. **No són el mateix i el corpus
  no en tria cap.** **El que sí que estableixen totes dues és que va seguir
  competint fins al 2018 com a mínim**, i **per tant que el bolcat, que
  s'aturava el 2015, es quedava tres anys curt.**

  `Requereix la llista de resultats sencera de la fitxa, llegida com a pàgina i
  no com a resum.`
- ~~**On va estudiar** ~~— **`obert`, confirmat**: **el bolcat confirma la
  Universíada del 2015 i no diu de quina universitat.**
- ~~**On va néixer**~~ — **`resolt`, i és un fet per a la branca**: **BARCELONA,
  20 de desembre del 1991.** **És el tercer esquiador andorrà d'aquesta
  subbranca nascut fora**, amb **[Alex Antor](./alex-antor.md) (Narbona)** i
  **[Cande Moreno](./cande-moreno.md) (Córdoba, Argentina)**.

## Related

- [Els esquiadors](els-esquiadors-index-de-fitxes.md)
- [Cinquanta anys d'esquí olímpic](../cinquanta-anys-desqui-olimpic.md)
- [Andorra als Jocs Olímpics](../../competicio/andorra-als-jocs-olimpics.md)
- [Roger Vidosa](./roger-vidosa.md) — els mundials d'abans.

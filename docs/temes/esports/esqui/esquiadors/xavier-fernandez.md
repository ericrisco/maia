---
type: article
title: Xavier Fernández
description: "Paralímpic a Vancouver 2010 i a Sotxi 2014, on va ser abanderat i únic esportista de la delegació. 24è al gegant assegut de Vancouver."
tema: temes/esports/esqui/esquiadors
veu: compilada
epoca: contemporania
apte_llengua: false
font: wikipedia-esquiadors-andorrans
timestamp: 2026-09-13T09:54:00Z
tags: [esports, esqui, paralimpics, discapacitat, abanderats]
---

# Xavier Fernández

## El fet

**Nascut el 19 de novembre del 1972.** **Va representar Andorra als Jocs
Paralímpics de Vancouver (2010) i de Sotxi (2014)**
([font](../../../../fonts/wikipedia-esquiadors-andorrans.md)).

| Jocs | Prova | Resultat |
| --- | --- | --- |
| **Vancouver 2010** | **Eslàlom gegant assegut** | **3:40.16 — 24è** |
| | Eslàlom assegut | **1:17.08 a la primera mànega**; no acaba |
| **Sotxi 2014** | dues proves | **cap temps**. **Abanderat** |

## A Sotxi era la delegació sencera

**A Sotxi 2014, Andorra va enviar un sol esportista paralímpic, i era ell.**

**Va ser abanderat d'Andorra** a la desfilada — **abanderat d'una delegació d'un
sol membre**, que és **ell mateix**
([els altres Jocs](../../competicio/els-altres-jocs.md)).

**El corpus ho escriu tal com és, sense adornar-ho**, perquè **descriu amb
exactitud l'escala del paralimpisme d'un microestat**: des del 2014, Andorra hi
ha anat sempre **amb una sola persona per edició**.

## El nom, escrit de tres maneres

**La font l'escriu «Xabier Fernández», «Xavi Fernandez Vasquez» i «Xavier
Fernandez»** en tres pàgines diferents.

**El corpus adopta «Xavier Fernández»** i **deixa les tres grafies escrites**,
perquè qui hi torni les trobarà i ha de saber que **és la mateixa persona** —o
com a mínim que **el corpus ho dona per la mateixa persona i no ho pot
demostrar**.

## El que falta

> **Auditat el 2026-09-14 contra el bolcat de la font pròpia**
> ([`Xavier_Fernandez.wiki`](../../../../raw/web/esport/alpins/Xavier_Fernandez.wiki)).

- ~~**El lloc de naixement** i el nom complet verificat. ~~— **`obert`, confirmat**:
  **l'infobox no té ni `birth_place` ni `full_name`.** **Només la data: 19 de
  novembre del 1972.**
- ~~**Quina discapacitat** i quina classe paralímpica. ~~— **`obert`, confirmat**:
  **el bolcat no en diu res**, **a diferència del de
  [Xavier Barios](./xavier-barios.md), que almenys dona la classe LW10.**
- ~~**Quines dues proves** va córrer a Sotxi.~~ **`parcial` el 2026-09-18**:
  **les de Sotxi segueixen sense nom** —el bolcat només diu que **no va marcar
  temps en cap de les dues**— **però les de Vancouver 2010 sí que es tanquen**:
  **eslàlom gegant masculí *sitting*, 3:40.16, 24è**, i **eslàlom masculí
  *sitting*, 1:17.08 a la primera mànega i abandonament a la segona**
  ([els Jocs Paralímpics](../../competicio/andorra-als-jocs-paralimpics.md)).
  **I va ser abanderat d'Andorra el 2010 i a l'obertura i la clausura del
  2014.**
- ~~**Si va competir en altres competicions** fora dels Jocs. ~~— **`obert`,
  confirmat.**
- ~~**I el balanç de la subbranca paralímpica**: **el corpus té DOS paralímpics
  andorrans, tots dos esquiadors, amb quatre participacions.**~~ — **`resolt` i
  **corregit el 2026-09-18**: **el balanç era curt.** **En són SIS**, i **un no
  és esquiador**: Xavier Barios, **Miguel Llongueras**, **Francesca/Paquita
  Ramírez**, Xavier Fernández, **el nedador Antonio Sánchez** i **Roger Puig**.
  **I són VUIT Jocs** —2002, 2006, 2010, **2012 d'estiu**, 2014, 2018, 2022 i
  **2026**— no quatre. **La fitxa sencera és a
  [Andorra als Jocs Paralímpics](../../competicio/andorra-als-jocs-paralimpics.md)**,
  i **la FADEA ja no és un nom buit: és la Federació Andorrana d'Esports
  Adaptats, el Comitè Paralímpic del país, `fadea.ad`.**
- ~~**El que segueix sent cert**: **cap medalla en vuit Jocs**, i **el millor
  resultat és un novè lloc.** `no-es-buit`: és una dada de balanç ja destil·lada,
  no una pregunta pendent.~~ — **`no-es-buit` el 2026-09-22**: és una limitació del registre, no una pregunta de contingut.

## Related

- [Els esquiadors](els-esquiadors-index-de-fitxes.md)
- [Cinquanta anys d'esquí olímpic](../cinquanta-anys-desqui-olimpic.md)
- [Andorra als Jocs Olímpics](../../competicio/andorra-als-jocs-olimpics.md)
- [Xavier Barios](./xavier-barios.md) — el que el va precedir.
- [Els altres Jocs](../../competicio/els-altres-jocs.md)

---
type: article
title: Quants diputats
description: "Andorra té un conseller per cada 2.671 habitants; San Marino, un per cada 556. Amb la proporció sanmarinesa, el Consell General tindria 135 membres."
tema: temes/politica/parlamentarisme
veu: compilada
epoca: contemporania
apte_llengua: false
font: figuereda-nombre-de-diputats
timestamp: 2026-09-13T15:00:00Z
tags: [politica, parlamentarisme, petits-estats, sistema-electoral, comparativa, xifres]
---

# Quants diputats

## La dada

**El Consell General té 28 consellers, i 28 és el mínim que permet la
Constitució.**

> **Article 52**: el Consell General disposarà **d'un mínim de 28 consellers i un
> màxim de 42**. **Article 51**: mandat de **quatre anys**
> ([font](../../../fonts/figuereda-nombre-de-diputats.md)).

**Andorra no ha pujat mai del mínim constitucional.** El corpus ho tenia
registrat com a pregunta oberta a
[els tres sistemes electorals](../sistema-electoral/els-tres-sistemes-electorals.md);
**aquesta és la resposta documental**.

## Els quatre microestats, comparats

| | **Andorra** | **San Marino** | **Mònaco** | **Liechtenstein** |
| --- | --- | --- | --- | --- |
| **Cambra** | **Consell General** | **Consiglio Grande e Generale** | **Conseil National** | **Landtag** |
| **Membres** | **28** (mín. 28, màx. 42) | **60** | **24** | **25** |
| **Mandat** | **4 anys** | **5 anys** | **5 anys** | **4 anys** |
| **Població** | **74.794** (est. 31-12-2017) | **33.338** (2018) | **37.308** (2018) | **38.111** (2017) |
| **Habitants per diputat** | **2.671** | **556** | **1.555** | **1.524** |
| **Sistema** | **Meitat nacional proporcional, meitat parroquial majoritària** | **Proporcional, circumscripció nacional única** | **16 per escrutini majoritari plurinominal + 8 proporcionals amb llindar del 5%** | **15 per Oberland + 10 per Unterland** |
| **Divisió territorial** | **7 parròquies** | **9 *castelli*** | **10 *quartiers*** però **un sol comú** | **2 districtes, 11 municipis** |
| **Cambres** | **Una** | **Una** | **Una** | **Una** |

**Els quatre són unicamerals.** I l'observació general de l'autor: **també ho són
els estats mitjans; els grans són tots bicamerals**.

## Si Andorra apliqués la proporció dels altres

| Proporció de… | El Consell General tindria |
| --- | --- |
| **San Marino** | **135 consellers** |
| **Liechtenstein** | **49** |
| **Mònaco** | **48** |

**Andorra és, de molt, el microestat europeu amb menys parlamentaris per
habitant.** Té **quasi cinc vegades menys representació per persona que San
Marino** i **prop de la meitat que Mònaco o Liechtenstein**.

**I encara podria tenir-ne 42 sense tocar la Constitució.**

**Aquesta xifra s'ha de llegir al costat d'una altra del mateix volum:** a
Andorra **els consellers a dedicació plena «són molt pocs»** i el més habitual
és **compaginar l'escó amb la feina principal**
([com funciona el Consell General](./com-funciona-el-consell-general.md)).

> **Menys diputats per habitant que ningú, i a temps parcial.**

**La conclusió és del corpus, no de la font**, que no creua les dues dades.

## Els creixements dels veïns

**Els altres microestats han anat ampliant els seus parlaments. Andorra no.**

| País | Evolució |
| --- | --- |
| **Mònaco** | **12 diputats** abans de la Constitució de 1962 → **18** → **24** amb la modificació del **2002** |
| **Liechtenstein** | **15** per la Constitució de **1862** → **25** el **1988** |
| **Andorra** | **24** fins al **1978** → **28** amb la creació d'Escaldes-Engordany → **28** encara |

**L'únic creixement del Consell General en cent cinquanta anys ve d'una parròquia
nova, no d'una decisió sobre representació.**

## Per què Andorra té doble circumscripció i San Marino no

**Aquesta és la part que explica el sistema andorrà millor que cap altra font del
corpus**, i el mecanisme és demogràfic.

**A San Marino els *castelli* tenen població semblant** —llevat de Serravalle—
i per això **una circumscripció nacional proporcional ja hi surt equilibrada**.

**A Andorra això es va trencar:**

> «A Andorra aquest **equilibri poblacional es va trencar amb el fort
> desenvolupament econòmic de mitjan segle passat**, quan **les parròquies baixes
> varen acabar concentrant la major part de la població** del país.»

I la Constitució **hi va respondre conservant el pes antic**:

> per mantenir part de la representació que les parròquies havien tingut «primer
> al Consell de la Terra i més tard al Consell General», **es va voler equilibrar
> amb la doble circumscripció**; **amb una sola circumscripció nacional
> s'hauria alterat**.

**La doble circumscripció andorrana no és una idea política: és una peça de
conservació.** Serveix perquè **el turisme dels anys seixanta no esborri el
repartiment territorial del segle XV**.

**I això connecta amb un enigma que el corpus tenia obert.** Al segle XV,
**Canillo tenia fins a sis representants i Andorra quatre**, i les fonts **no
saben per quin criteri**
([el Consell de la Terra](../../institucions/consell-general/el-consell-de-la-terra.md)).
**Sigui quin fos, no era demogràfic — i el sistema actual continua sense
ser-ho, ara a posta.**

## La segona cambra que no hi és, però funciona

**Andorra no té senat i té el seu efecte.** L'autor ho assenyala:

> Andorra, «**sense disposar de cambra baixa**, determinades qüestions
> parroquials demanen **la majoria de consellers elegits en circumscripció
> parroquial** i no nacional.»

**Certes decisions necessiten la meitat territorial del Consell.** És **una
segona cambra dins de la primera**, i evita el que
[com funciona el Consell General](./com-funciona-el-consell-general.md)
descrivia com la mancança andorrana: **no hi ha segona lectura**.

*(Nota: la font escriu «cambra baixa» on el sentit demana «cambra alta». El
corpus ho registra com a lapsus aparent i reté l'argument, que és clar.)*

## La conclusió de l'autor, marcada com a seva

**Defensa ampliar el Consell General**, i el corpus **no la subscriu**:

> «Malgrat que **un parlament nombrós pugui ser impopular**, només un parlament
> reforçat amb un nombre òptim de diputats pot esdevenir garantia de… la funció
> legislativa, amb l'elaboració de **normativa de qualitat**, i la funció de
> **control de l'executiu**.»

I el context que hi posa és el mateix que
[Dallerès](./la-deriva-presidencialista.md) i
[Vicenç Mateu](./com-funciona-el-consell-general.md) descriuen des de dins:
**la divisió de Montesquieu «és més difusa que mai»**, **l'executiu predomina a
tot Europa** i **la disciplina de partit mou els parlaments**.

**Tres autors de tres posicions diferents —un ex-síndic, el síndic en exercici i
un jurista de fora— arriben al mateix diagnòstic i proposen tres remeis
diferents:** reformar el sistema electoral, reformar el reglament, o **ampliar
la cambra**.

## Un apunt que val la pena

L'autor obre citant **Aristòtil** i n'extreu sis criteris de bon funcionament
democràtic —elegir entre tots, manar per torns, càrrecs curts, no acumular
càrrecs, retre comptes, assemblea sobirana— i hi afegeix:

> «Preceptes tots que **curiosament tenen una certa retirada amb algunes de les
> màximes que recull Antoni Fiter i Rossell al Manual Digest**.»

**Ho deixa dit i no ho desenvolupa.** El corpus **ho registra com a buit
interessant**: comparar els criteris d'Aristòtil amb les **màximes del
[Manual Digest](../../historia/manual-digest/manual-digest.md)** és una feina
concreta i ningú no l'ha feta.

I una observació del corpus sobre el **criteri aristotèlic dels càrrecs curts,
«bandejar els càrrecs vitalicis»**: Andorra el va suspendre expressament durant
**vint-i-tres anys** amb
[Francesc Cairat](../../persones/francesc-cairat.md).

## El desequilibri, mesurat: un vot d'Ordino val 4,3 vots de la capital

La font explica el mecanisme i no el quantifica. Amb la població estimada del
**juliol del 2026** (Departament d'Estadística, activitat **A001**) es pot
mesurar.

**Catorze dels vint-i-vuit consellers surten de la circumscripció parroquial, i
cada parròquia n'elegeix dos, sigui quina sigui la seva mida:**

| Parròquia | Població (07/2026) | % del país | Habitants per conseller parroquial |
| --- | --- | --- | --- |
| **Ordino** | 5.816 | 6,5% | **2.908** |
| Canillo | 6.593 | 7,3% | 3.297 |
| Sant Julià de Lòria | 10.318 | 11,5% | 5.159 |
| La Massana | 12.350 | 13,7% | 6.175 |
| Encamp | 13.710 | 15,2% | 6.855 |
| Escaldes-Engordany | 16.350 | 18,2% | 8.175 |
| **Andorra la Vella** | **24.884** | **27,6%** | **12.442** |
| **Total** | **90.021** | 100% | 6.430 de mitjana |

`Les poblacions i els percentatges són els publicats; la darrera columna és el
quocient entre població i dos consellers, càlcul del corpus.`

**La ràtio entre l'extrem alt i el baix és de 4,28.** Un conseller d'Ordino
representa **2.908 persones** i un d'Andorra la Vella **12.442**. **En la meitat
parroquial del Consell, un habitant d'Ordino pesa quatre vegades i tres dècimes
el que pesa un habitant de la capital.**

**I el desequilibri no és entre alta i baixa muntanya, sinó entre la capital i la
resta.** Cinc parròquies de set estan per sota de la mitjana. **Andorra la Vella
sola té més habitants que Canillo, Ordino i Sant Julià de Lòria juntes**
—**24.884 contra 22.727**— i hi té **dos consellers parroquials contra sis**.

**Sobre el conjunt dels vint-i-vuit, l'efecte es dilueix a la meitat**, perquè
**els altres catorze surten d'una llista nacional única** on cada vot val igual.
Comptant-los tots, un conseller equival a **3.215 habitants** de mitjana, i la
distorsió que queda és la que la Constitució va voler conservar: **el pes que les
parròquies tenien al Consell de la Terra**
([el Consell de la Terra](../../institucions/consell-general/el-consell-de-la-terra.md)).

**El creixement recent va en la direcció d'eixamplar-lo.** Entre el juliol del
2025 i el del 2026 la població creix a totes les parròquies **menys a Andorra la
Vella, que és la que menys creix** (+0,5%), mentre **Canillo fa +5,0%** i
**Ordino +3,2%**. **El desequilibri es redueix per la banda de dalt, no per la de
baix.**

## El debat del 1993: 21 contra 7, i el número era un compromís previ

El corpus registrava com a buit **qui va proposar 28 i qui 42**. **El debat és al
Diari Oficial del Consell General núm. 7/1993, sessió ordinària dels dies 2 i 3
de setembre de 1993**, dins de la Proposició de llei qualificada del règim
electoral i del referèndum. **No és una qüestió tècnica: el diari deixa per
escrit que era política, i que estava decidida abans.**

**Les dues propostes.**

| | Qui | Argument |
| --- | --- | --- |
| **28** | **la Comissió**, al text de la proposició | El ponent **Miquel Aleix** diu que el redactat surt «**després de llargs debats i de posicions possiblement contraposades**» i que la Comissió va entendre que **el debat s'havia de deixar obert al ple** |
| **42** | **esmena núm. 3** | «La composició **més àmplia** del Consell General, de les dues previstes en la Constitució, **afavoreix un Parlament més plural amb les minories representades**», i **hi ha dos parlamentaris menys a la pràctica** perquè el Síndic i el Subsíndic surten d'entre els consellers |

**El Govern hi va prendre partit, i pels 42.** El **Cap de Govern** hi va
intervenir abans del debat:

> «És evident que potser aquest és **l'eix central i bàsic** i és una opinió
> totalment política i de decisió política. […] es considera que **un Parlament
> més ampli afavoreix la participació de les minories electorals**, però hi ha un
> altre aspecte […]: **també afavoreix la coherència política o la facilitat
> d'estructurar el Parlament en majories i minories**. […] al nostre criteri,
> subjectiu sempre, **els 42 faciliten el funcionament d'una assemblea
> parlamentària estructurada en majories i minories**.»

**El resultat.**

> «El resultat de la votació és: **21 vots a favor de la proposta número 1 i 7
> vots a favor de la proposta número 2**.» — «Doncs, **quedaria en 28
> Consellers**.»

**Vint-i-un contra set.** I aquí ve el que el corpus no podia saber: **les
explicacions de vot diuen que la xifra estava pactada mesos abans, per aprovar la
Constitució.**

**Miquel Aleix**, ponent de la Comissió i conseller d'Escaldes, explica per què va
votar 28 quan el seu programa deia 42:

> «**veieu escrit en el programa electoral que n'hi hauria d'haver 42** […] perquè
> ho hem escrit amb **l'íntima convicció que el Parlament amb 42 hauria funcionat
> molt millor**, però que aquí quedi constància pública que **l'essencial que vam
> venir a fer en aquesta legislatura era aprovar la Constitució**, i **per buscar
> la unanimitat de la Constitució**, vist que **des de certes parròquies hi havia
> un cert recel** amb que quedés determinat **abans del Referèndum** […] com havia
> de quedar definit l'article 57, **em vaig comprometre a nivell de la Comissió a
> que la Parròquia d'Escaldes votaria dos i dos**.»

**Albert Pujal** diu el mateix en una frase:

> «Prescindint de les meves conviccions personals, **vaig pactar en el moment de
> la Constitució amb companys, particularment de la Parròquia, que recolzaria la
> proposta de 28 Consellers**. […] la Constitució és una eina transcendental i va
> quedar ben lligada, **es va tenir la precaució de posar 28 o 42**. […] **si la
> necessitat a la pròxima legislatura porta a fer-ne 42, sense modificar la
> Constitució es podrà fer.**»

**I Rosa Maria Mandicó va votar 42 sabent que li costaria a casa:**

> «He dubtat moltíssim entre votar 28 o votar 42. […] **molt espontàniament he
> votat 42**. […] sóc molt conscient que **el meu vot no serà gens popular dins la
> meva Parròquia**, però **crec en el país** i crec que **42 en el futur ens pot
> ajudar**.»

**Tres coses que això canvia.**

**Primera, la forquilla 28-42 de la Constitució era deliberada i tots ho sabien.**
Pujal ho diu amb totes les lletres: **«es va tenir la precaució de posar 28 o
42»**, i **pujar-hi no exigeix reformar la Constitució**.

**Segona, el 28 no va guanyar un debat: va executar un pacte.** El nombre es va
fixar **per no posar en risc la unanimitat constitucional**, i **el recel venia
de les parròquies**, que és exactament l'eix que
[el desequilibri parroquial](#el-desequilibri-mesurat-un-vot-dordino-val-43-vots-de-la-capital)
mesura avui.

**Tercera, hi havia un argument aritmètic que el corpus no tenia**: amb 28
consellers, **descomptats el Síndic i el Subsíndic**, **queden 26 parlamentaris
de fet** per a comissions i tasques ordinàries. Amb 42 en quedarien 40.

`Font: Diari Oficial del Consell General, Acta núm. 7/1993, sessió ordinària dels
dies 2 i 3 de setembre de 1993
([PDF](../../../raw/consell-general/dcg-1993/dcg-7-1993.pdf),
[text](../../../raw/consell-general/dcg-1993/dcg-7-1993-raw.txt)). El document és
a dues columnes i les cites surten de l'extracció en ordre de lectura, amb
l'espaiat restituït. Drets del Diari **pendents de determinar**; **no entra en cap
dataset**.`

## Buits registrats

1. ~~**Per què Andorra no ha passat mai de 28** havent-hi marge fins a 42.~~
   **`parcial` el 19-09-2026**: el debat constituent de 1993 ja documenta que
   els 28 es van pactar per preservar la unanimitat de la Constitució i
   apaivagar el recel de les parròquies; el corpus encara no té una font que
   expliqui per què cap legislatura posterior ha activat la possibilitat
   d'arribar a 42.
2. ~~**El debat constituent sobre el nombre**: qui va proposar 28 i qui 42.~~ —
   **`resolt` el 17-09-2026 pel Diari Oficial del Consell General**, acta
   **7/1993**, sessió del **2-3 de setembre de 1993**: vegeu
   [El debat del 1993](#el-debat-del-1993-21-contra-7-i-el-numero-era-un-compromis-previ).
   **21 contra 7**, i **el número estava pactat abans del referèndum
   constitucional**.
3. ~~**Les xifres de població per parròquia** avui, per mesurar el
   desequilibri real.~~ — **`resolt` el 17-09-2026 per la via d'Estadística**
   (activitat **A001**, juliol del 2026): vegeu
   [El desequilibri, mesurat](#el-desequilibri-mesurat-un-vot-dordino-val-43-vots-de-la-capital).
   **La ràtio és de 4,28 a la meitat parroquial del Consell.**
4. ~~**Comparar els criteris d'Aristòtil amb les màximes del Manual Digest.**~~ — **`font_externa` el 2026-09-22**: cal consultar fonts, registres o observació que el corpus no conserva.
5. ~~**Quines qüestions exactament** requereixen majoria de consellers parroquials.~~ — **`resolt` el 2026-09-21**: [les lleis qualificades](../../institucions/justicia/les-lleis-qualificades.md) transcriu l'article 57.3: exigeixen majoria absoluta separada dels consellers de circumscripció parroquial i nacional les lleis **electoral i de referèndum**, de **competències comunals** i de **transferències als Comuns**.
6. ~~**La diferència entre població estimada (74.794) i registrada (80.209)**: què la produeix.~~ — **`parcial` el 2026-09-21**: [el model del 2009, comprovat](../../llengua/politica-linguistica/el-model-del-2009-comprovat.md) documenta els **80.209** com la suma de **11.079 menors de 15** i **69.130 majors de 14** de l'onada lingüística de **2017**. La diferència amb els **74.794** de l'estimació demogràfica és de **5.415 persones**, però el corpus encara no té la nota metodològica que expliqui si són universos, dates de tall o criteris de registre diferents.

## Related

- [Com funciona el Consell General](./com-funciona-el-consell-general.md)
- [Els tres sistemes electorals](../sistema-electoral/els-tres-sistemes-electorals.md)
- [La deriva presidencialista](./la-deriva-presidencialista.md)
- [Andorra com a microestat](../../institucions/petits-estats/andorra-com-a-microestat.md) — els altres tres.
- [Els Jocs dels Petits Estats d'Europa](../../esports/competicio/els-jocs-dels-petits-estats.md) — els mateixos països, una altra taula.
- [Les set parròquies i els seus comuns](../../institucions/comuns-i-parroquies/les-set-parroquies.md)
- [El Consell de la Terra](../../institucions/consell-general/el-consell-de-la-terra.md) — d'on ve el pes parroquial.

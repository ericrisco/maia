---
type: article
title: La consciència nacional andorrana
description: "Dos mil dos-cents anys de moments en què Andorra s'ha reconegut a si mateixa, del 218 aC al 1993. «Ni els uns (de baix) ni els altres (de dalt).»"
tema: temes/politica/identitat-politica
veu: compilada
epoca: contemporania
apte_llengua: false
font: pol-consciencia-nacional
timestamp: 2026-09-13T11:00:00Z
tags: [politica, identitat, neutralitat, cronologia, historia, tesi-de-part]
---

# La consciència nacional andorrana

## La tesi, i d'on ve

La formula **Antoni Pol i Solé**, arquitecte i urbanista, representant de la SAC
a la Universitat Catalana d'Estiu **del 1988 al 2014**
([font](../../../fonts/pol-consciencia-nacional.md)):

> «La consciència nacional andorrana és **aquella voluntat de ser, i continuar
> sent, pròpiament andorrans, és a dir neutrals, és a dir ni els uns (de baix) ni
> els altres (de dalt)**.»

**És una tesi, i el corpus la marca com a tal.** El que no és tesi és la
cronologia que la sosté, i és la més llarga que el corpus té d'un sol text.

## L'observació que ho enquadra tot

Pol comença explicant per què costa tant trobar aquesta consciència als
documents:

> «La consciència nacional dels andorrans, **per la pròpia petitesa de la nostra
> realitat territorial** i sobretot per les vicissituds històriques viscudes,
> **acompanyada d'una necessitat de passar desapercebuts**, s'ha manifestat al
> llarg del temps **molt discretament i sovint de manera indirecta**.»

**Passar desapercebut és la política.** Això és el mateix que Bascompte anomena
*savoir faire* i tradueix com **«fer l'andorrà»**
([el parlamentarisme andorrà](../parlamentarisme/el-parlamentarisme-andorra.md)),
i el que el corpus ja tenia registrat sobre
[la neutralitat](../../historia/guerres-i-neutralitat/guerres-i-neutralitat.md).
**Tres autors independents hi arriben per camins diferents.**

## La cronologia

### Abans que hi hagués Andorra

| Any | Fet |
| --- | --- |
| **218 aC** | Els **andosins** s'oposen al pas d'**Anníbal** pels Pirineus, **juntament amb ilergets, arenosis i bergusis**. Ho explica **Polibi** cap al 150 aC |
| — | La romanització **«sembla que no va passar de Sant Vicenç d'Enclar»** |
| **839** | L'**acta de consagració de la catedral de la Seu** menciona **Lòria, Andorra amb Santa Coloma, la Massana, Ordino, Encamp i Canillo** |

**La lectura de Pol sobre el 218 aC és la part més fina del text**, i el corpus
la reté com a raonament seu, no com a fet: **que algú de fora et distingeixi vol
dir que tu ja et distingies**. «La identificació d'un grup que actua com a tal
implica, de retruc, **haver de tenir una consciència pròpia que el fa
presentar-se davant dels altres com a algú diferent**.»

### La resistència a ser feudalitzats

| Any | Fet |
| --- | --- |
| **Terç final del s. X** | El comte **Borrell II** fa construir **el castell de Bragafolls**, a l'entrada d'Andorra. **Els andorrans el destrueixen** |
| **Dos-cents anys** | La resistència continua i **produeix el desinterès dels comtes d'Urgell** |
| **1133** | Els comtes **venen els seus drets a Andorra** als **bisbes d'Urgell** |
| **entre 1133 i 1162** | S'escriu **la Carta Pobla**, **document fals** datat pretesament el **805** |
| **1162** | **Primera concòrdia** amb el bisbe: protecció a canvi de jurisdicció. **La signen sis representants, un per parròquia** |
| **1176** | **Segona concòrdia**, «en vista de l'incompliment dels pactes per part andorrana». **La signen tres-cents caps de casa** |
| **1199** | Acord amb el **vescomte de Castellbò** |
| **1275** | Concòrdia amb el **comte de Foix**: se'ls eximeix de la **cugúcia** |
| **1278 i 1288** | **Primer i segon pariatges** |
| **1289** | Primeres **ordinacions** aprovades per un consell de prohoms en assemblea |

**Dues xifres fan tota la feina.** El **1162** signen **sis**; el **1176**,
després que els andorrans incomplissin, en signen **tres-cents**. **El bisbe els
va fer signar un per un.**

### La lectura dels pariatges

Pol hi veu **una victòria andorrana amagada**, i el raonament val la pena:

- Al **primer pariatge** els andorrans aconsegueixen del comte de Foix
  l'exempció de la **intestia** i l'**eixorquia**, a més de la cugúcia.
  **`Definits el 17-09-2026`**, des dels Usatges i per una font espanyola de
  1918: **la intestia és el dret del senyor quan el pagès mor intestat —un terç
  si deixa muller i fills, la meitat si només deixa fills o només muller, tot si
  no hi ha parents—; l'exorquia és heretar dels qui moren sense fills «la part
  que correspondria a tots els fills junts»; i la cugúcia és relativa a
  l'adulteri —meitat per meitat entre marit i senyor si es fa sense voluntat del
  marit, tot al senyor si hi consent o hi instiga, i res si la dona hi és forçada.**
  Vegeu [el que es dona en feu és el domini
  útil](../../historia/pareatge/el-que-es-dona-en-feu-es-el-domini-util.md).
- Al **segon**, després de deu anys de queixes, aconsegueixen «**el mateix que
  van aconseguir amb el castell de Bragafolls, prop de 300 anys abans**»: que
  **no s'hi torni a fer cap castell** — però ara **amb garantia**, perquè **cap
  senyor sol no ho pot decidir**.

> «**L'acord dels dos era la millor garantia i definitiva que no se'n faria mai
> cap més.**»

**El bloqueig mutu com a avantatge.** És el mateix mecanisme que el corpus troba
a [la Inquisició](../../costums/religiositat/la-inquisicio-a-andorra.md), on un
copríncep desfà la decisió de l'altre.

**I la contrapartida, que Pol no amaga:** «els andorrans **van haver de pagar
més**».

### Els segles de la doctrina

| Any | Fet |
| --- | --- |
| **1419** | Es reconeix el **Consell de la Terra** |
| **1659** | Al **tractat dels Pirineus**, les valls **no queden afectades** pel nou traçat de fronteres |
| **1864** | **DIVERGÈNCIA REGISTRADA.** Un **dictamen del Consejo de Estado espanyol**, basat en un altre del **Consejo Real** de cap al 1852, fa saber a Andorra que **«por el Tratado de los Pirineos de Noviembre de 1659, tenía España derechos evidentes de dominio, jurisdicción y propiedad sobre todos los Valles»** (*La cuestión de Andorra*, 1894, p. 23). **El corpus no té cap dels dos textos i no arbitra**; registra que **hi va haver una tesi espanyola d'annexió fonamentada en aquell tractat**, amb data i amb òrgan. Vegeu [semblava que el bisbe patrocinava el joc](../../historia/segle-xix/semblava-que-el-bisbe-patrocinava-el-joc.md) |
| **1714** | A la **guerra de Successió**, se'n surten «gràcies al copríncep episcopal **Simeón de Guinda y Apeztegui**, que es reconeix amb els andorrans com a **diferent dels espanyols i francesos**» |
| **1748** | **Manual Digest**, d'Antoni Fiter i Rossell |
| **1764** | **Politar Andorrà** |
| **1806** | **Napoleó restableix** els vincles, interromputs des del **1793** |
| **1866** | **Nova Reforma**: vot als caps de casa, separació de càrrecs, **apareix la bandera tricolor** |

**El Manual Digest i el Politar es van escriure per a l'ús intern i van quedar
tancats.** Pol ho subratlla: el Manual Digest **«tindrà una gairebé nul·la
distribució, en quedar arxivat en un àmbit limitat a les institucions»**.

| Obra | Escrita | Divulgada a tots els caps de casa |
| --- | --- | --- |
| **Manual Digest** | **1748** | **1987** — reedicions posteriors el **2000** i el **2018** |
| **Politar Andorrà** | **1764** | **1983** — reedició el **2015** |

**Dos-cents trenta-nou anys entre escriure la doctrina nacional i repartir-la.**

El fragment del Manual Digest que Pol tria és el que toca el projecte maia de
prop: **«un Andorra deu sempre deixar señal y rastre de la llengua de sa
nació»**.

### El segle XX

| Any | Fet |
| --- | --- |
| **1914** | **Himne andorrà**: text del copríncep **Joan Benlloch i Vivó**, música de **mossèn Enric Marfany i Bons** |
| **1921** | **El Consell General l'accepta oficialment**. El Papa declara **Meritxell patrona**; el **8 de setembre**, Festa Nacional |
| **1925** | El Consell General vol **l'adhesió a la Societat de Nacions**. **Sense èxit** |
| **1929** | Concessió a **Fhasa** |
| **1930** | França i Espanya signen un conveni per als **correus postals a Andorra**, **«sense l'acord del país»** |
| **1933** | Sufragi universal masculí ([vegeu el vaivé](../sistema-electoral/el-sufragi-universal-masculi.md)) |
| **1935** | El Consell General acorda la concessió de **Ràdio Andorra**; emet des del **1939** |
| **1938** | **Oficialitat del català**. «Abans **no n'hi havia hagut necessitat**» |
| **1946** | ***Les Valls d'Andorra***, de **Bonaventura Riberaygua i Argelich** |
| **1951-1958** | **Ràdio de les Valls**; des del **1962**, **Sud Radio** |
| **1959** | El Consell General demana als coprínceps **el tancament de les dues emissores** |
| **1963** | ***Andorra el meu país***, d'**Albert Puigoriol i Lladó** — **primer manual escolar d'andorranitat** |
| **1970 / 1973** | **Sufragi femení**, actiu i passiu |
| **1972** | Comença el procés d'**andorranització** |
| **1976** | **Primera consulta al poble andorrà** sobre la reforma. **Primers Jocs Olímpics** |
| **1977** | **Segona consulta** |
| **1978** | Es parteix la parròquia d'Andorra: **Andorra la Vella i Escaldes-Engordany** |
| **1980-1981** | **Fi de les concessions radiofòniques**; **Assemblea Magna** sobre les ràdios; **tancament manat pels batlles** |
| **1981** | Es crea el **Consell Executiu** |
| **1982** | Andorra a la conferència de **Jaca** del Consell d'Europa; entra a la **Comunitat de Treball dels Pirineus**. **Comença l'Escola Andorrana** |
| **1989** | **Dos detinguts txecs denuncien Andorra per tortures** davant el Tribunal Europeu de Drets Humans, «que és a punt de condemnar els dos coprínceps» |
| **1993** | **Constitució**. **184è estat de l'ONU** |

## La guerra de les ràdios

**El corpus no en tenia res, i és una història sencera.** De la cronologia de
Pol se'n treu això:

1. **1929-1935**: Andorra concedeix la ràdio. **Emet des del 1939.**
2. **1930**: França i Espanya **signen entre elles** el conveni postal
   **sense Andorra**.
3. **1958-1962**: hi ha **una segona emissora**, que acaba dient-se **Sud
   Radio**.
4. **1959**: el Consell General demana **tancar-les totes dues**.
5. **1980-81**: el Consell **declara acabades les concessions**; **els delegats
   permanents en manen la reobertura**; es convoca **l'Assemblea Magna**; i
   finalment **les tanquen els batlles**.

**Un país que va passar mig segle discutint qui podia emetre des del seu
territori** — i, pel mig, **el parlament i els representants dels coprínceps
manant coses contràries**. Cavero hi afegeix que **el tema l'han estudiat Giral
i Lluelles**, i que Lluelles **«aviat publicarà el seu estudi»**
([font](../../../fonts/cavero-parlamentarisme-segle-xx.md)). **Buit prioritari.**

## Els desacords amb el mateix volum

Pol contradiu altres ponències del llibre en tres punts. **El corpus els
registra i no els resol:**

| Qüestió | Pol | Altres |
| --- | --- | --- |
| **El Politar** | **1764** | **1763** (Bascompte) |
| **Quan Andorra passa de cosenyoriu a coprincipat** | **El 1866**, amb la Nova Reforma | **Després de la Nova Planta** de 1716, quan els bisbes es reconeixen prínceps sobirans (Bascompte) |
| **Sufragi universal masculí** | **1933**, aprovat i prou | Un vaivé fins al **1947-48** ([fitxa](../sistema-electoral/el-sufragi-universal-masculi.md)) |

## La conclusió, que és una metàfora

**El corpus la reté sencera perquè és, ella mateixa, un document sobre com
Andorra s'explica:**

> «No som una gran nació però, tenint entitat nacional, **som una micronació**…
> Una petita diversitat, la d'**un endemisme**, **una singularitat entre
> gegants, com aquelles plantes que tenim per totes les nostres muntanyes i que
> aprofiten per créixer en les esquerdes dels grans rocs**. Com hi viuen?
> Aprofitant els petits avantatges que els ofereix el seu emplaçament
> **abrigat, perdut, desapercebut**, i **en complementarietat o simbiosi amb el
> seu entorn**.»

I tanca amb dues línies que el corpus no interpreta:

> «Això sí, tenint sempre **l'esperit obert**. **I la butxaca tancada.**»

## El que Pol reconeix que no sosté la seva tesi

**Val la pena, perquè és un autor que es contradiu a si mateix a consciència.**
Diu que **la llengua i l'escola** «no han estat, en el cas d'Andorra,
determinants per a la configuració d'aquesta consciència nacional», i en dona la
prova: **el català no és oficial fins al 1938** perquè «abans no n'hi havia
hagut necessitat», i **l'escola nacional pròpia no comença fins al 1982**,
**després** de la francesa, l'espanyola i la congregacional
([els tres sistemes](../../societat/educacio/els-tres-sistemes.md)).

**Una consciència nacional sense escola nacional durant tot el segle XX.**

## Buits registrats

> **Auditats contra la resta del corpus el 2026-09-13.** **D'onze buits, sis
> tenien resposta —sencera o a mitges— en altres branques**, i **un ha revelat
> una divergència de dates que aquesta fitxa arrossegava sense saber-ho.**

1. **El castell de Bragafolls**: on era, què se'n sap arqueològicament. —
   **`obert`, confirmat.** **El corpus només en té la línia de
   [la cronologia](../../historia/moments-historics/la-cronologia-dandorra.md)**
   —**Borrell II el fa al terç final del segle X i els andorrans el
   destrueixen**— **i ve de la mateixa font que aquesta fitxa.**
   `Requereix font externa: arqueologia.`
2. ~~**Les concòrdies de 1162 i 1176**, text i llista de signants.~~ —
   **`obert` pel text, i amb una divergència de dates que cal dir.**
   **[Els privilegis](../../historia/edat-mitjana/els-privilegis.md) les daten
   el 1152 i el 1175**, seguint Bascompte, i **les anomena igual: «concòrdies»,
   capitulacions de drets entre el bisbe i el capítol d'Urgell i la comunitat
   andorrana.** **Aquesta fitxa en dona 1162 i 1176.** **Divergència registrada,
   no arbitrada**: **deu anys de diferència en la primera i un any en la
   segona.** **El text i els signants segueixen sense conèixer-se en cap de les
   dues versions.**
3. ~~**La Carta Pobla** com a falsificació: qui la va fer i com es va detectar.~~
   — **`parcial`, i el corpus en té més del que aquesta fitxa suposava.**
   **La data de fabricació**: **entre el 1133 i el 1162**, amb data pretesa del
   **805** ([la cronologia](../../historia/moments-historics/la-cronologia-dandorra.md)).
   **El debat, amb noms**: **Jordi Guillamet i altres en qüestionen
   l'autenticitat; Antoni Morell i Jacques Descheemaeker la defensen**
   ([els privilegis](../../historia/edat-mitjana/els-privilegis.md)). **I el
   corpus la classifica ja com a falsa** a
   [el nom d'Andorra](../../historia/origens/el-nom-dandorra.md), al costat de
   l'acta de consagració d'Urgell. **El que segueix obert és exactament el que
   el buit demanava: qui la va fabricar i com es va detectar.**
4. ~~**Simeón de Guinda y Apeztegui** i el 1714.~~ — **`parcial`, i el corpus el
   té en un altre paper que aquesta fitxa no sabia.** **[La revolta del tabac de
   1733](../../economia/tabac/la-revolta-del-tabac-de-1733.md)** el documenta
   **com el bisbe d'Urgell que, cap al 1728-1733 i sota pressió de l'intendent
   general de Catalunya** —**que amenaçava de retirar les franquícies duaneres
   del bestiar**— **prohibeix el conreu del tabac a Andorra**, **no el comerç**.
   **ACTUALITZAT EL 15-09-2026: l'arc té ara el seu punt de partida amb data.**
   *La cuestión de Andorra* (1894), **p. 16**, **copia sencer** un decret seu:
   **Balaguer, 3 de febrer de 1715**, secretari **Pedro Vizent**, pel qual
   **ordena als cònsols de les Valls que no obeeixin cap ordre que no vingui dels
   dos sobirans**, i **nomena l'adversari: els ministres subdelegats de
   l'intendent general del Principat de Catalunya**, que s'intrometien «*en la
   Jurisdicción y autoridad temporal de esos Valles, ofendiendo su independencia,
   exempción y regalía*». **S'hi titula «Príncipe Soberano con el Rey
   Cristianísimo» i «Con-Soberanos».** **El mateix intendent general a qui el
   1715 mana desobeir és el qui cap al 1730 li arrenca la prohibició del tabac, i
   l'instrument de pressió és el bestiar les dues vegades.** **El corpus no
   explica el gir: en registra els dos extrems.** Vegeu [no obeïu cap ordre que no
   vingui dels dos](../../historia/segle-xix/no-obeiu-cap-ordre-que-no-vingui-dels-dos.md).

   **El mateix home que aquesta fitxa recorda per reconèixer els andorrans com a
   diferents dels espanyols i francesos el 1714 és, vint anys després, qui cedeix
   a la pressió espanyola en el tabac.** **El corpus registra les dues cares i no
   les arbitra.** ~~**El que segueix obert és el 1714 mateix**, i
   [persones](../../persones/README.md) el té com a buit amb aquestes paraules.~~
   — **`resolt` el 18-09-2026**: **ara té fitxa completa**, a
   [persones/simeon-de-guinda](../../persones/simeon-de-guinda.md) — **neix el
   1660 a Navarra i mor el 1737 a Sant Julià de Lòria**, dins les valls que
   governava.
5. ~~**Les dues consultes al poble, 1976 i 1977**: pregunta, resultat,
   participació.~~ — **`parcial`, i amb una correcció de dates.** **El corpus té
   [els primers referèndums](../sistema-electoral/els-primers-referendums.md)**,
   i **en són tres, no dos, i les dates no són les d'aquesta fitxa**:
   **28 d'octubre del 1977**, **16 de gener del 1978** i **1982**.
   **Del primer se'n té tot el que el buit demanava**: **sis opcions**,
   **1.937 vots sobre un cens de 3.206** —**60,4% de participació**—, i el
   resultat: **l'Agrupament Democràtic d'Andorra, 34,9%**; **el Quart de les
   Escaldes, 20,5%**; i **vots en blanc, 31,2%**, **la segona opció més votada**.
   **Les tres propostes del Consell General van sumar el 4,3%.**
   **Divergència registrada**: **aquesta fitxa parla de consultes el 1976 i el
   1977; aquella les data el 1977 i el 1978.**
6. ~~**L'Assemblea Magna** del 1981.~~ — **`parcial` des del 2026-09-13, i amb
   fitxa pròpia**: [l'Assemblea Magna](../../institucions/consell-general/lassemblea-magna.md).
   **Ara se sap què era** —**setanta-dues persones: els 28 consellers, els dos
   síndics, els catorze cònsols i quatre persones més per parròquia**—, **que
   només es convocava en crisi**, **que la Nova Reforma del 1866 la manté sense
   anomenar-la** i **que va desaparèixer amb la Constitució del 1993**. **I que
   la del 1980-81 no va ser l'única**: **n'hi va haver dues més, el 1974 i el
   1975, sobre el rescat de les concessions de FHASA.** **El que segueix obert
   són les actes.**
7. ~~**L'afer Drozd i Janousek** (1989-91) i què va canviar.~~ — **`parcial`, i
   la sentència és al corpus citada per article.** **[La
   quèstia](../../institucions/coprincipat/la-questia.md)** treu **dues xifres
   del tribut feudal andorrà** de **la sentència del Tribunal Europeu de Drets
   Humans en l'afer Drozd i Janousek contra França i Espanya, del 26 de juny del
   1992, §76**. **La data exacta del pronunciament, doncs, ja la té el corpus**,
   i **és posterior al 1991 que aquest buit dona.** **El que segueix obert és
   què va canviar**, encara que **la coincidència està registrada**: **la
   sentència és del juny del 1992 i la Constitució, del març del 1993.**
8. ~~**Ràdio Andorra, Ràdio de les Valls i Sud Radio.** **Prioritari.**~~ —
   **`resolt` en bona part.** **El corpus té [Ràdio
   Andorra](../../societat/mitjans/radio-andorra.md)** —**l'única emissora
   privada francesa que va emetre tota la Segona Guerra Mundial sense caure ni
   en mans dels aliats ni de Vichy**, i **el Consell General la va matar el
   1981**—, **[RTVA](../../societat/mitjans/rtva.md)**, i fins i tot
   **[el Museu de la Ràdio](../../cultura/museus-i-arxius/museus/el-museu-de-la-radio.md)**,
   **previst per al 2015 i del qual el corpus no sap si va obrir**. **Sud Ràdio
   hi surt, amb una divergència ja registrada** a la fitxa de Ràdio Andorra.
   **El que segueix obert és Ràdio de les Valls**, **de la qual el corpus només
   té el nom dins aquesta cronologia.**
9. ~~Els dos llibres de país: **Riberaygua (1946)** i **Puigoriol (1963)**.~~ —
   **`parcial` el 17-09-2026: del primer ja se sap la referència i, sobretot,
   qui el va escriure.** Font: les [actes del Consell
   General](../../../fonts/actes-historiques-consell-general.md).

   **Riberaygua Argelich, Bonaventura (1946). *Les Valls d'Andorra. Recull
   documental*. Barcelona: Bosch, Casa Editorial.**

   **I l'autor no era un estudiós de fora: era el secretari del Consell
   General.** **Es presenta a la plaça el 18 de desembre del 1934** —**amb dos
   competidors, Candido Rossell i Roc Rossell**—, **la guanya el 23 de febrer
   del 1935 per tres anys**, **signa la primera acta el 15 d'abril del 1935**,
   **el 21 de març del 1938 el Consell el fa secretari «per temps indefinit»** i
   **hi és fins al 1950.**

   **Un «recull documental» publicat el 1946 per qui portava quinze anys tenint
   les actes a les mans.** **No és una font externa que mira Andorra: és
   l'arxiu del Consell parlant en nom propi**, i **això canvia com s'ha de
   llegir.**

   **De Puigoriol (1963) la síntesi no en diu res**: `Requereix el segon llibre
   de país.`
10. **El conveni postal de 1930** signat sense Andorra.
11. ~~**L'oficialitat del català el 1938**: quina norma, i per què just
    llavors.~~ — **`resolt` el 17-09-2026 quant a la norma**, i **la font
    externa que el buit demanava és ara al corpus**: les [actes del Consell
    General](../../../fonts/actes-historiques-consell-general.md).

    **No és una norma: en són dues, del mateix mes, i cap de les dues no declara
    res.**

    | Data | Acord | Foli |
    | --- | --- | --- |
    | **1 de juny de 1938**, continuació del Consell de Sancogesma | **«Totes les súpliques adreçades al Consell General hauran de ser redactades en llengua catalana»** | ANA FCV 9.485, fol. 346 v. |
    | **20 de juny de 1938**, sessió extraordinària | **«Tots els cartells i rètols oficials o semioficials hauran d'ésser redactats en llengua oficial catalana»** | ANA FCV 9.485, fol. 346 v. |

    **El segon acord diu «llengua oficial catalana» com qui diu una cosa
    sabuda.** **No proclama l'oficialitat: la dona per feta i n'ordena l'ús.**
    **El que passa el 1938 no és que el català es declari oficial, és que
    s'imposa on abans no calia dir-ho**: **en el que entra al Consell —les
    súpliques— i en el que es llegeix al carrer —cartells i rètols.**

    **I això dona sentit a la frase que el corpus arrossegava d'una sola font**
    —«abans no n'hi havia hagut necessitat»—: **la necessitat és la d'obligar,
    no la de declarar.**

    **Del *per què just llavors*, les actes no diuen res.** **El que sí que diuen
    és què més es feia aquelles setmanes**: **el 31 de maig el síndic demana
    reformar la sala de sessions «posant-la en condicions decoroses»** i **l'1 de
    juny, la mateixa sessió que acorda el català de les súpliques, aprova el
    primer reglament interior del Consell** —sabates i corbata negra, el cap
    descobert, vint pessetes de multa per faltar-hi, **i el càstig per revelar
    el que es diu en sessió secreta.** **El 1938, amb la guerra al sud, el
    Consell es posa regles a si mateix i llengua a la porta.**
    `Requereix una font que digui per què el 1938 i no abans.`

## Related

- [El parlamentarisme andorrà](../parlamentarisme/el-parlamentarisme-andorra.md)
- [Guerres i neutralitat](../../historia/guerres-i-neutralitat/guerres-i-neutralitat.md)
- [El Manual Digest](../../historia/manual-digest/manual-digest.md)
- [El pareatge](../../historia/pareatge/el-pareatge.md)
- [Andorra com a microestat](../../institucions/petits-estats/andorra-com-a-microestat.md)
- [El Gran Carlemany](../../institucions/simbols/el-gran-carlemany.md) — l'himne del 1914.
- [La bandera](../../institucions/simbols/la-bandera.md) — la tricolor del 1866.
- [Andorra vista de fora](../../cultura/andorra-vista-de-fora/andorra-vista-de-fora.md)
- [L'enquesta de 1994](./lenquesta-de-1994.md) — el mateix debat, preguntat a vint-i-dues persones amb nom.

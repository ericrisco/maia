---
type: article
title: "El referèndum constitucional del 1993"
description: "4.903 sís i 1.706 nos. El corpus tenia dues xifres aparentment contràries del mateix referèndum, i resulta que són el mateix recompte sobre dos denominadors diferents."
tema: temes/politica/sistema-electoral
veu: compilada
epoca: contemporania
apte_llengua: false
font: wikipedia-referendums-andorra
timestamp: 2026-09-14T06:00:00Z
tags: [politica, sistema-electoral, constitucio-1993, referendums, parroquies]
---

# El referèndum constitucional del 1993

## El fet

**14 de març del 1993.** **Ratifica la Constitució aprovada pel Consell General
el 2 de febrer d'aquell mateix any**, **per unanimitat**, **redactada en una
comissió tripartita formada pel Consell General i els coprínceps.**

**Va entrar en vigor el 28 d'abril.**

## Les xifres absolutes, que el corpus no tenia

| | Vots | % vots vàlids | % vots emesos |
| --- | --- | --- | --- |
| **Sí** | **4.903** | **74,19 %** | **70,96 %** |
| **No** | **1.706** | **25,81 %** | **24,69 %** |
| **Vots vàlids** | **6.609** | — | **95,64 %** |
| **Nuls o en blanc** | **301** | — | **4,36 %** |
| **Participació** | **6.910** | — | **75,74 %** |
| **Votants registrats** | **9.123** | — | — |

## La llei que el va regular, llegida

**Trobada al [BOPA](../../../fonts/bopa.md) el 17-09-2026**: **Llei de
Referèndum de la Constitució del Principat d'Andorra, del 5 de febrer de 1993**
(BOPA del 04-02-1993, `docs/raw/lleis/bopa/llei-5-2-1993-referendum-de-la-constitucio.txt`).
**Deu articles i dues disposicions finals.** **No la fa el Consell General**:
la signen, «en nom i representació d'ambdós Coprínceps», **els Delegats
Permanents Nemesi Marquès i Pierre Steinmetz, a la Seu d'Urgell i Perpinyà.**

**La pregunta, literal** (article 5):

> «**Voteu a favor del Projecte de Constitució del Principat d'Andorra,
> elaborat en concertació amb els serveis dels Coprínceps i adoptat pel Consell
> General en sessió extraordinària celebrada el 2 de febrer de 1993?**»

**El vot en blanc era vot nul.** L'article 6 no deixa marge:

> «Es consideraran **vots nuls i sense cap efecte** els sobres electorals que
> continguin **paperetes blanques**, més d'una papereta, paperetes esmenades i
> aquelles on figurin inscripcions o altres marques.»

**Això explica per què la taula de resultats d'aquesta fitxa agrupa «nuls o en
blanc» en una sola casella de 301**: **el 1993 no eren dues categories.**
**Hi havia paperetes blanques a la taula i el seu ús comptava com a nul** —
**una cosa i la contrària alhora**, i **la llei vigent d'avui ja no ho fa així**
([com es compten els vots](./com-es-compten-els-vots.md)).

**El calendari del cens, amb data per a cada pas:**

| Data del 1993 | Què |
| --- | --- |
| **10 de febrer** | Cada comú publica la **llista provisional d'electors** al tauler d'edictes |
| **15 de febrer** | Últim dia per demanar la inscripció per escrit |
| **18 de febrer** | Els comuns han de resoldre |
| **23 de febrer** | Últim dia de recurs davant el **Govern** |
| **25 de febrer** | El Govern resol |
| **1 de març** | Es publiquen les **llistes definitives** |
| **5 de març** | Últim dia de recurs al **Tribunal Administratiu i Fiscal** |
| **13 de març** | Últim dia perquè una sentència encara canviï la llista |
| **14 de març** | **Referèndum** |

**Trenta-dos dies entre la primera llista i la votació**, i **el tribunal podia
afegir o treure un elector el dia abans.**

**I l'article 2 va obrir el cens**: «no seran d'aplicació les previsions de
l'**article 14 de la Llei Electoral**» —és a dir, **per a aquest referèndum es
va desactivar una restricció d'inscripció que sí que regia a les eleccions**.
`Requereix el text de l'article 14 per saber a qui va deixar votar.`

**Els interventors els posava qualsevol elector**: per formar part de la mesa
n'hi havia prou amb **un nomenament escrit avalat per vint electors de la
parròquia** (article 7). **No calia ser de cap candidatura, perquè no n'hi
havia.**

**El recompte el va fer la Comissió Permanent del Consell General i el va
proclamar el síndic general** (article 9) —**no el cap de Govern**, com passa
avui. **La regla d'aprovació és la mateixa que segueix vigent**: «quedarà
aprovat **si el nombre de vots afirmatius supera el nombre de vots negatius**».
**Sense quòrum de participació.**

### I de passada, el nom de la llei electoral d'aleshores

**La disposició final primera la cita**, i **és la peça que el corpus buscava**:

> «**La Llei Electoral del 13 de juliol i del 2 d'octubre de 1987**, així com la
> Llei de modificació de l'article 12 de la Llei Electoral, del 24 d'octubre de
> 1989, seran d'aplicació en tot allò que no hagi estat previst i que no
> s'oposi a la present Llei.»

**El «sistema dels vuitanta» que
[els tres sistemes electorals](./els-tres-sistemes-electorals.md) no sabia
datar té data: 13 de juliol i 2 d'octubre de 1987.** **Dues dates per a una
sola llei** —presumiblement dues sessions d'aprovació—, i **el text encara no
és al corpus**: `Requereix el Butlletí del Consell General del 1987; el BOPA
comença el 1989 i no el conté.`

## Una contradicció que no ho era

**El corpus tenia, a
[la Constitució de 1993](../../historia/constitucio-1993/constitucio-1993.md),
aquestes xifres:** **Sí 70,95 %, No 24,68 %, nuls i blancs 4,35 %, abstenció
24,25 %.**

**I la font d'aquesta fitxa en dona unes altres:** **Sí 74,19 %, No 25,81 %.**

**Semblaven incompatibles.** **No ho són.**

> **Són el mateix recompte sobre dos denominadors diferents.**
>
> - **4.903 / 6.609 vots vàlids = 74,19 %**
> - **4.903 / 6.910 vots emesos = 70,96 %**
>
> **La primera xifra exclou els nuls i blancs del denominador; la segona els hi
> inclou.** **Les dues són correctes i diuen coses diferents.**

**El corpus ho comprova també per al No** —**1.706 / 6.609 = 25,81 % i
1.706 / 6.910 = 24,69 %**— **i per a l'abstenció**: **100 − 75,74 = 24,26 %,
que és l'«abstenció 24,25 %» que el corpus ja tenia, amb un decimal
d'arrodoniment.**

> **Aquesta fitxa deixa constància del mètode, no només del resultat.**
> **Dues fonts que donen percentatges diferents d'una mateixa votació poden
> tenir raó totes dues, i abans de registrar una discrepància cal comprovar
> sobre què s'ha calculat cada percentatge.** **El corpus registra
> discrepàncies sovint; aquesta no ho és.**

## La incoherència que sí que hi ha

**El mateix article dona dues xifres del cens:**

| On | Xifra |
| --- | --- |
| **Al text** | **«9.323 andorrans tingueren dret a votar»** |
| **A la taula de resultats** | **9.123 votants registrats** |

**Dos-cents de diferència.** **Amb 9.123 la participació dona 75,74 %, que és la
que la taula declara; amb 9.323 donaria 74,12 %.**

**El corpus es queda amb 9.123 perquè és la xifra amb què quadra la resta de la
taula**, **i deixa registrat que el text de la mateixa font en diu una altra.**

## El resultat parròquia a parròquia

**El percentatge de vots favorables no va baixar del 56 % en cap de les set
parròquies.**

| Parròquia | Sí |
| --- | --- |
| **[Escaldes-Engordany](../../territori/parroquies/escaldes-engordany/escaldes-engordany.md)** | **80 %** |
| **[Encamp](../../territori/parroquies/encamp/encamp.md)** | **76,7 %** |
| **[Andorra la Vella](../../territori/parroquies/andorra-la-vella/andorra-la-vella.md)** | **74,1 %** |
| **[La Massana](../../territori/parroquies/la-massana/la-massana.md)** | **66 %** |
| **[Canillo](../../territori/parroquies/canillo/canillo.md)** | **60 %** |
| **[Ordino](../../territori/parroquies/ordino/ordino.md)** | **56 %** |

**La font no dona la xifra de
[Sant Julià de Lòria](../../territori/parroquies/sant-julia-de-loria/sant-julia-de-loria.md).**
**Buit obert.**

### I el motiu que la font hi dona

> **«En les localitats menys habitades, les principals reticències sobre la
> Constitució tenien a veure amb el sistema electoral.»**

**Ordino i Canillo, les dues parròquies més despoblades, són les dues que menys
hi voten a favor.**

> **Això és exactament el conflicte que
> [un elector per casa](./un-elector-per-casa.md) i
> [els primers referèndums](./els-primers-referendums.md) documenten**: **si
> la representació passa a comptar habitants en comptes de parròquies, les
> parròquies petites hi perden.** **El 1993 es va votar sobre això, i es veu al
> mapa.**
>
> **La Constitució va guanyar a tot arreu i va guanyar menys precisament allà
> on costava més.**

## Els antecedents que la font aporta

### El Concili Vaticà II

**El problema d'origen que el corpus no tenia formulat així:**

> **Després del Concili Vaticà II, «es plantejava que l'Església no permetés al
> bisbe d'Urgell continuar amb la tasca de copríncep episcopal a Andorra, perquè
> el Concili aconsellava que l'Església no havia de tenir poders temporals».**
>
> **«Des d'Andorra es temia que hi podia haver un buit legal i institucional,
> sense un dels dos coprínceps.»**

**El motor no és només la voluntat democràtica andorrana**: **és la por que un
dels dos coprínceps plegués i deixés el país a mitges.**

### La memòria del 1978 i «la reformeta»

| Quan | Què |
| --- | --- |
| **1978** | **El Consell General formula una memòria de reforma** amb dues peticions: **avançar cap a un Estat**, i **ser reconegut internacionalment com a Estat** |
| **1981** | **Decret dels coprínceps**, popularment «la reformeta» |
| **Deu anys després** | **«cap de les dues demandes va tenir èxit»** |

**De tot el que es pretenia transformar, la reformeta només va resultar efectiva
en una cosa**: **la separació entre executiu i legislatiu, amb la creació del
Consell Executiu.**

**I el límit que la fa insostenible:**

> **«Aquest Executiu de l'època no tenia potestats en matèria de seguretat o
> afers exteriors i la barrera de la representació dels coprínceps en aquests
> assumptes es va anar convertint en insostenible per a les autoritats
> andorranes.»**

**Un govern sense exterior i sense seguretat.** **Això és el que la Constitució
va venir a resoldre.**

Vegeu [la Transició Nacional](../../historia/historia-recent/la-transicio-nacional.md)
i [la democratització](../../historia/democratitzacio/democratitzacio.md).

## El que el text va consagrar, segons la crònica del dia següent

**Segons *Avui* del 15 de març del 1993**, l'aprovació:

- **Convertia el Principat en un Estat de dret.**
- **Deixava enrere molts aspectes propis d'un sistema feudal.**
- **Equiparava Andorra a la resta de països europeus.**
- **Consagrava el català com a única llengua oficial del país.**

Vegeu [la política lingüística](../../llengua/politica-linguistica/politica-linguistica-index-de-fitxes.md).

## El que falta

- ~~**El resultat de Sant Julià de Lòria.**~~ **`parcial` el 2026-09-19**: el [Consell General](../../../fonts/consell-general-referendum-1993-sant-julia.md) documenta **1.412 electors, 1.067 votants i 75,57% de participació**. La pàgina no dona el desglossament parroquial de vots **Sí** i **No**, que continua obert.
- ~~**La campanya**: **qui feia campanya pel no, i amb quins arguments.**~~ — **`parcial` el 21-09-2026**: la font identifica com a argument de les reticències el sistema electoral —especialment la pèrdua relativa de les parròquies petites— i la fitxa contrasta el resultat d'Ordino i Canillo, però no identifica qui organitzava la campanya del no.
- ~~**Els números absoluts per parròquia**, no només els percentatges.~~ **`parcial` el 2026-09-19**: el [Consell General](../../../fonts/consell-general-referendum-1993-sant-julia.md) dona els absoluts de **cens i votants** de les set parròquies —inclòs Sant Julià—, però no els vots parroquials de **Sí** i **No**. ([taula oficial](../../../fonts/consell-general-referendum-1993-sant-julia.md))
- ~~**La crònica de *l'Avui* del 15 de març del 1993**, p. 8, **accessible a
  l'hemeroteca Pandora de Girona** — **identificada i no llegida.**~~ —
  **`parcial` el 19-09-2026**: la còpia local de l'article de la Viquipèdia
  reprodueix la dada que se n'extreu —la ratificació per àmplia majoria i la
  lectura del resultat com a estat de dret— i conserva la citació i l'enllaç a
  Pandora ([font](../../../raw/web/referendums/Referèndum_constitucional_d_Andorra_de_1993-ca.wiki)).
  **La pàgina 8 d'*Avui* no s'ha consultat directament.**
- ~~**L'*Especial Constitució* d'*El Periòdic d'Andorra*** del 14 de març del
  2017, **en PDF** — **dues pàgines citades, revista no llegida.**~~ —
  **`parcial` el 19-09-2026**: la mateixa còpia local conserva els dos passatges
  citats del PDF —els **precedents** (p. 12) i el **penúltim pas** (p. 17)— i
  l'enllaç a l'edició. **El PDF original no s'ha obert directament.**
- ~~**La memòria de reforma del 1978** i **el decret del 1981**: **textos.**~~ — **`parcial` el 21-09-2026**: la fitxa ja fixa que la memòria demanava avançar cap a un Estat i obtenir reconeixement internacional, i que el decret dels coprínceps va crear la separació entre executiu i legislatiu; el corpus encara no conserva els textos originals.
- ~~La llei que va regular el referèndum.~~ — **`resolt` el 17-09-2026**, vegeu
  més amunt. **I n'obre una de nova**: **l'article 14 de la Llei Electoral del
  1987**, que aquell referèndum va desactivar expressament.

## Related

- [La Constitució de 1993](../../historia/constitucio-1993/constitucio-1993.md)
- [Els primers referèndums](./els-primers-referendums.md) — 1977, 1978, 1982.
- [Un elector per casa](./un-elector-per-casa.md) — el conflicte que s'hi veu.
- [El sistema electoral](sistema-electoral-index-de-fitxes.md)
- [La Transició Nacional](../../historia/historia-recent/la-transicio-nacional.md)

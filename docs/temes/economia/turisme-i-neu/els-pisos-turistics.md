---
type: article
title: Els pisos turístics
description: "Dos pisos turístics són renda; tres són empresa. I en un edifici de deu o més habitatges, si els HUT passen del 50% l'edifici sencer ha de deixar de ser residencial."
tema: temes/economia/turisme-i-neu
veu: compilada
epoca: contemporania
apte_llengua: false
font: jurisprudencia-ad
timestamp: 2026-09-13T17:50:00Z
tags: [economia, turisme, habitatge, dret, societat, paisatge-construit]
---

# Els pisos turístics

La **Llei 16/2017 general de l'allotjament turístic** té **99 articles** i
**7 versions consolidades** ([font](../../../fonts/jurisprudencia-ad.md),
instantània del **2026-09-12**). Regula de què viu el país, i la peça que
toca la vida de tothom són els **habitatges d'ús turístic** —els **HUT**.

## Sis modalitats, i una és la casa de la gent

Els allotjaments turístics es classifiquen en **hotelera**, **apartaments
turístics**, **càmpings**, **allotjaments rurals**, **habitatges d'ús turístic
(HUT)** i **establiments complementaris** (article 5).

> Són HUT «els habitatges que són cedits pel seu propietari o titular dels drets
> reals, directament o indirectament, als clients **a canvi de contraprestació
> econòmica o de qualsevol altre tipus**, per a una estada turística»
> (article 23).

I es lloguen **sencers**: «no se'n permet la cessió per estances ni la prestació
de cap servei de restauració» (article 23). **Llogar una habitació de casa teva
no és un HUT: és una cosa que aquesta llei no preveu.**

## El llindar que converteix un propietari en empresa

| Quants HUT | Què és |
| --- | --- |
| **Fins a 2**, explotats directament | **rendiment patrimonial** — no cal registre de comerç |
| **3 o més** | **activitat econòmica en tots els casos** |

(article 35.2)

Amb 3 o més, el propietari ha de **registrar-se com a EGHUT** —empresa de gestió
d'habitatges d'ús turístic— **o cedir-ne la gestió a una**. I «la gestió d'un HUT
només pot ser encomanada a **una sola EGHUT**».

Fins i tot en el cas petit, «explotació directa» té definició estricta: ser el
responsable de **la publicitat, la gestió i el cobrament de les reserves i la
tramesa de dades al ROAT**.

**Dos és una renda. Tres és un negoci.** La frontera és un número.

## El sostre per edifici

Aquesta és la regla amb més conseqüències urbanes:

> «En un edifici format per **10 o més habitatges**, els HUT **no poden
> representar més del 50%** de les quotes de participació de l'edifici […] En
> cas que se sobrepassi aquest percentatge, **l'edifici haurà de ser destinat en
> la seva totalitat a apartaments turístics** […] i **haurà de canviar d'ús
> residencial a ús hoteler**» (article 23).

**No hi ha edificis a mitges.** O més de la meitat és habitatge, o l'edifici
sencer deixa de ser habitatge. La llei tanca el camí a la conversió progressiva:
**passar del 50% obliga a convertir el 100%.**

## I els veïns hi poden dir que no

> «La destinació d'un habitatge a l'ús turístic **no és possible** si està
> prohibida per l'ordenació d'usos del sector on es trobi **o està prohibida pels
> estatuts de la comunitat**» (article 35.4).

El procediment exigeix **declaració responsable** conforme s'ha informat el
president de la comunitat, **més una certificació signada pel president i
l'administrador secretari** conforme els estatuts no ho prohibeixen.

**Una comunitat de propietaris andorrana pot vetar els pisos turístics del seu
edifici**, i el permís administratiu no s'atorga sense el seu paper.

## El registre que també reparteix diners

Tots els clients s'inscriuen al **Registre d'Ocupació d'Allotjaments Turístics
(ROAT)**. No és només control: **les nits registrades al ROAT decideixen el
5,5% de les transferències de l'Estat a cada comú**
([els diners dels comuns](../../institucions/comuns-i-parroquies/els-diners-dels-comuns.md)).

I el ROAT també serveix per detectar els inactius: **no registrar cap client
durant dotze mesos continus** compta com a inactivitat, i registrar-ne un
després d'aquell termini **no ho esmena** (article 35).

## La borda, definida per llei

Entre les definicions generals de l'article 4 n'hi ha una que és patrimoni
arquitectònic escrit com a norma:

> «**Borda**: unitat immobiliària d'estil rústic dotada de mobiliari, **d'una o
> dos plantes més sotacoberta, construïda de pedra i fusta, amb teulada de
> pissarra**» (article 4.7).

**Pedra, fusta i pissarra.** Una llei de turisme que fixa els materials d'una
tipologia tradicional, com
[la llei d'urbanisme](../../territori/urbanisme/qui-mana-al-territori.md) fixa
l'alçada d'un mur de pedra seca.

## Obligacions que valen per a tots

- **Assegurança de responsabilitat civil obligatòria**, en entitat autoritzada a
  Andorra, «en una quantia suficient […] **d'acord amb la capacitat màxima
  d'aforament declarada**» (article 60). El ministeri pot exigir-ne la pòlissa i
  el rebut **en tot moment**.
- **Cèdula o certificat d'habitabilitat i butlletí elèctric**, acumulativament,
  per als HUT (article 23).
- **Fulls oficials de queixa, reclamació i denúncia** a tots els HUT
  (article 35.8).
- **Estrelles**: els HUT es classifiquen **d'una a cinc categories** (article 25).

## Buits registrats

- ~~**Quants HUT hi ha**, i quants edificis han topat amb el 50%, no consta.~~ —
  **`parcial` el 18-09-2026**: l'API d'Estadística té una sèrie
  «Habitatges i apartaments turístics» amb **47.221 el 2024**
  ([font](../../../fonts/estadistica-habitatges-turistics.md)), **però la
  xifra sembla mesurar una categoria estadística més àmplia que els HUT
  amb llicència** —és un ordre de magnitud proper al parc d'habitatges
  total. `DIVERGÈNCIA REGISTRADA, NO ARBITRADA`: **no citar aquesta xifra
  com el nombre d'HUT sense contrastar-ne la metodologia.** **Els edificis
  al 50% segueixen sense cap dada.**
- ~~**La Llei 5/2025 va modificar aquesta llei sencera** —n'és el títol III— i el
  corpus **no ha comparat què hi va canviar**
  ([els habitatges buits](../../societat/habitatge/els-habitatges-buits.md)).~~
  — **`resolt` el 17-09-2026**, llegit el títol III sencer al
  [BOPA](../../../fonts/bopa.md)
  (`docs/raw/lleis/bopa/llei-5-2025-creixement-sostenible-i-habitatge.txt`).
  **Són els articles 43 a 60 i toquen quinze articles de la Llei 16/2017.**
  **El canvi de fons és un**: **la llicència d'HUT deixa de ser perpètua i
  deixa de seguir el pis.**

  **Tres novetats que caduquen l'autorització:**

  | Article nou | Regla |
  | --- | --- |
  | **35 bis.1** | **L'autorització dura tres anys** i s'ha de demanar la renovació **tres mesos abans** |
  | **35 bis.2** | **No demanar-la a temps cancel·la d'ofici la inscripció al Registre** i impedeix l'activitat |
  | **35 bis.3** | **Dotze mesos continus d'inactivitat** i el ministeri **cancel·la d'ofici** |

  **I una que la desvincula de l'immoble** (article 40 bis.1): **canviar de
  propietari —per qualsevol causa que no sigui l'herència— comporta la baixa
  d'ofici de la inscripció.** **Comprar un pis amb llicència d'HUT ja no és
  comprar la llicència.** **Notificar-ho toca al venedor**, excepte en la
  successió per causa de mort, **on toca a qui hereta**, i **en un mes**.

  **La comunitat de propietaris passa a tenir veu documental** (article 35.4):
  qui vulgui obrir un HUT ha d'aportar **una declaració responsable conforme ha
  informat el president de la comunitat** i **una certificació signada pel
  president i l'administrador secretari conforme els estatuts no ho
  prohibeixen**. **La regla de fons ja hi era** —els estatuts podien prohibir-ho—;
  **el que és nou és que ara ho ha de certificar algú.**

  **Els comuns queden lligats al control** (article 40 ter, nou): **han de
  notificar al ministeri els canvis de titularitat dels HUT en un mes**, i
  **si tenen indicis que algú explota un HUT sense autorització, ho han de
  denunciar amb nom i contacte**. **I a l'inrevés**: si el ministeri té els
  indicis, **el comú li ha de facilitar la identitat del titular en un mes**.

  **També queda prohibida qualsevol activitat comercial dins d'un HUT**
  (article 35.10), amb declaració responsable que no n'hi ha ni n'hi haurà.

  **I de passada, la llei reordena els càmpings** (articles 43-44): **cinc
  grups** —càmpings, zones d'acampada, **glàmping**, **àrees d'acollida
  d'autocaravanes** i **àrees d'estada d'autocaravanes**—, **les bordes
  existents passen a ser «elements d'allotjament fixos»**, i **queda prohibit
  pernoctar en autocaravana, caravana, càmper o furgoneta condicionada fora
  d'aquests espais.** `Requereix saber quantes àrees d'autocaravanes hi ha
  habilitades, perquè la prohibició depèn que n'hi hagi.`
- **Les 7 versions consolidades no s'han comparat.**
- **Els requisits tècnics dels HUT** van a reglament i no són al corpus.
- ~~**L'impost sobre les estades en allotjaments turístics** (Llei 19/2022, 47
  articles) és al recull i **queda per llegir**.~~ — **`resolt` el 17-09-2026**
  al [BOPA](../../../fonts/bopa.md), butlletí **76 del 2022**
  (`docs/raw/lleis/bopa/llei-19-2022-impost-estades-allotjaments-turistics.txt`).
  **I el títol de la llei ja diu que no és només un impost**: reguladora de
  l'impost **i de modificació de la Llei 16/2017 general de l'allotjament
  turístic i de la Llei 13/2007 d'Andorra Turisme**. **Dels 47 articles, 22 són
  l'impost i 25 modifiquen aquelles dues lleis.**

  **La tarifa és per persona i dia, i l'HUT paga com un quatre estrelles:**

  | Allotjament | Euros per dia |
  | --- | --- |
  | **5 estrelles** | **3,00** |
  | **4 estrelles / 4 espigues** | **2,00** |
  | **Habitatges d'ús turístic** | **2,00** |
  | 2 o 3 estrelles / espigues, o en procés de classificació | 1,50 |
  | Resta d'allotjaments turístics | 1,00 |

  **Un pis turístic tributa el doble que una fonda i el mateix que un hotel de
  quatre estrelles.**

  **Quatre decisions que fan l'impost menys recaptador del que sembla:**

  - **Els residents andorrans no el paguen** (article 4.1.a) —**és un impost
    sobre el visitant, no sobre l'estada**.
  - **Els menors de 16 anys tampoc**, i **si no tenen document, val la
    declaració de l'acompanyant** (article 4.3).
  - **Només es paguen els set primers dies**: «per a les estades d'una durada
    superior a 7 dies en un mateix allotjament turístic, **la superació de
    l'esmentat termini**» queda exempta (article 4.1.c). **El sostre per estada
    és, doncs, de 21 € en un cinc estrelles i 14 € en un HUT.**
  - **Les àrees d'acollida d'autocaravanes queden fora del fet generador**
    (article 3).

  **Qui paga i qui ingressa no són el mateix.** **El contribuent és qui s'hi
  allotja**; **el substitut és qui explota l'allotjament**, i **es presumeix,
  llevat de prova en contrari, que és el titular de la llicència de turisme**
  (article 5.3). **L'impost s'ha de consignar a la factura separat del preu**
  (article 11). **Per a un HUT això vol dir que el propietari és qui declara,
  cobra i respon.**

  **I hi ha una arma contra qui no declara**: si el substitut no presenta
  l'autoliquidació o no aporta les dades d'ocupació, **l'Administració pot
  estimar-li els dies d'estada per mètode indirecte**, **a partir de les dades
  estadístiques d'ocupació d'allotjaments del mateix tipus** (article 9.4).
  **No declarar no evita la quota: la fa calcular per comparació amb els
  veïns.**

  **La recaptació té destinació finalista.** Va al **Fons per a l'impuls del
  turisme** (articles 20-22) —promoció, turisme sostenible, infraestructures,
  desestacionalització, formació—, **però els recàrrecs i interessos de mora
  no hi entren** (article 21.2) i **el Govern el pot desviar «a altres
  finalitats d'interès general» si s'activa la Llei qualificada dels estats
  d'alarma i d'emergència** (article 22.2). `Requereix la xifra recaptada i la
  liquidació del Fons, que no són en aquesta llei.`

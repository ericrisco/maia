---
type: article
title: La fundació per Carlemany
description: L'himne nacional canta una fundació que no va passar, i el document que la sosté és fals.
tema: temes/historia/origens
veu: compilada
epoca: contemporania
apte_llengua: false
font: viquipedia-ca
timestamp: 2026-09-12T02:35:00Z
tags: [historia, origens, mite, historiografia, identitat]
---

# La fundació per Carlemany

## El relat

Andorra hauria estat fundada per **Carlemany**, que hauria concedit als
andorrans una **Carta de poblament** en agraïment per haver-lo ajudat contra els
sarraïns.

És el relat fundacional del país. És el que canta
[l'himne nacional](../../institucions/simbols/el-gran-carlemany.md), que es diu
literalment ***El Gran Carlemany***.

## El fet

**No és verídic.**

«La suposada fundació d'Andorra per Carlemany no és verídica, i està fonamentada
en una **Carta de poblament** que les investigacions contemporànies han dilucidat
i **qualificat com a falsa**» ([Viquipèdia](../../../fonts/viquipedia-ca.md)).

I hi ha un segon document tocat. **L'acta de consagració de la catedral
d'Urgell**, **datada l'any 819**, **va ser redactada entre el 1016 i el 1024**:
és a dir, **dos segles després de la data que diu tenir**. És un document
falsificat.

## Com s'ha de llegir això

Amb cura, i sense el gust fàcil de desmuntar.

**Que el document sigui fals no vol dir que Andorra no existís.** Les Valls hi
eren, la gent hi vivia, i la relació amb el món carolingi és real: al final del
segle VIII Andorra va compartir la sort de les contrades catalanes de la **Marca
Hispànica**, annexionades al **regne d'Aquitània instituït per Carlemany**.

El que és fals és **el títol**, no **el fet d'existir**. Una carta fabricada més
tard per **fonamentar drets** que ja s'exercien.

I val a dir que **això no és excepcional**: l'Europa medieval és plena de
documents fundacionals produïts a posteriori per donar base escrita a una
situació de fet. Andorra no hi és una excepció pintoresca; hi és un cas normal.

## Per què aquesta fitxa és important per a maia

**Perquè un model preguntat «qui va fundar Andorra?» dirà «Carlemany».**

Ho dirà perquè és el que hi ha escrit a milions de pàgines, perquè és el que diu
l'himne, i perquè és una resposta bonica. I estarà repetint un mite sobre un
document fals.

La resposta honesta és més llarga i més interessant: **el relat és fundacional i
viu, el document que l'acredita és fals, i el vincle carolingi real existeix per
una altra via.** Un model que sàpiga Andorra de veritat ha de poder dir les tres
coses alhora, sense negar-ne cap.

Aquesta fitxa existeix exactament per això.

## El que falta

- Qui va fabricar la Carta de poblament, quan i **per a què**. Aquesta és la
  pregunta bona i no està resposta.
- La datació i la crítica textual de l'acta de consagració.
- Quan neix el relat de Carlemany com a mite nacional i **qui el promou** —
  perquè un mite adoptat el 1921 com a himne té una història política moderna,
  no només medieval.
- Els **andosins**: **buit tancat**. Vegeu
  [el nom d'Andorra](./el-nom-dandorra.md) — la seva menció és un hàpax i la
  identificació amb Andorra no se sosté. **Els dos pilars del relat d'origen han
  caigut tots dos.**

## La data, i qui hi és a cada bàndol

El corpus tenia la **Carta de poblament** com a document fals **sense data ni
contendents**. **Domènec Bascompte** dona les dues coses
([font](../../../fonts/bascompte-privilegis.md)).

**La data atribuïda: 805.**

I el debat, amb noms:

| Posició | Qui |
| --- | --- |
| **Qüestionen l'autenticitat** | **Jordi Guillamet** i altres autors |
| **La defensen** | **Antoni Morell** i **Jacques Descheemaeker** |

**No és un consens: és una discussió amb dos bàndols.** El corpus rectifica la
seva pròpia formulació anterior —que la donava per «qualificada de falsa» sense
matisos— i la deixa com el que és: **majoritàriament qüestionada, encara
defensada per alguns**.

Què hi diu el document, segons Bascompte: atorgava **a unes determinades
persones** el dret a **poblar o tornar a habitar les valls d'Andorra**, «**després
de la hipotètica expulsió dels sarraïns**».

**«Hipotètica.»** L'adjectiu és de l'autor.

### La lliçó que en treu, que val més que la resposta

> «**El fet de la seva falsedat explica la importància dels documents per
> legitimar** davant els altres membres de la mateixa comunitat i d'altres
> comunitats **un avantatge social que afavoreix un grup determinat de
> persones**.»

**Un document fals és un fet històric:** diu **què es volia poder demostrar, i
quan**. Vegeu
[el Llibre de la Terra](../../institucions/consell-general/el-llibre-de-la-terra.md),
on passa exactament el mateix amb sis segles de diferència, i
[els privilegis](../edat-mitjana/els-privilegis.md).

**I un detall que el corpus registra sense interpretar:** **Antoni Morell**, que
defensa l'autenticitat de la carta de Carlemany, és **el mateix que va novel·lar
*Boris I, rei d'Andorra*** ([fitxa](../segle-xx-primera-meitat/boris-skossyreff.md)).

## Related

- [El Gran Carlemany](../../institucions/simbols/el-gran-carlemany.md) — l'himne que el canta.
- [Andorra a l'edat mitjana](../edat-mitjana/andorra-a-ledat-mitjana.md) — el que sí que va passar.
- [El Pareatge](../pareatge/el-pareatge.md) — el document fundacional que **sí** és autèntic.
- [El nom d'Andorra](./el-nom-dandorra.md) — el primer esment real, del 843.

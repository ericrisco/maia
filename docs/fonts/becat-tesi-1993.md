---
type: font
id: becat-tesi-1993
title: "Jean Becat — L'Andorre. Mutations d'une économie montagnarde (tesi d'Estat, 1993). Llibre 2: la societat i l'organització tradicionals"
titular: Jean (Joan) Becat · ICRESS, Universitat de Perpinyà Via Domitia
autor: "Jean Becat, geògraf. Tesi de Doctorat d'Estat dirigida per Jean Le Coz i Pierre Carrière"
publicacio: "Presentada i defensada públicament el 18 de desembre del 1993 a la Universitat Paul Valéry Montpeller III amb el títol «Les Pyrénées méditerranéennes, mutations d'une économie montagnarde: le cas de l'Andorre». Reedició sense modificacions, ICRESS, 2019, amb el títol «L'Andorre. Mutations d'une économie montagnarde»"
url: "https://www.govern.ad/ca/l/4522728"
llicencia: "drets reservats — obra d'autor viu, reedició del 2019, sense llicència explícita identificada"
redistribucio: no
data_consulta: 2026-09-13
abast: >
  Només el LLIBRE 2 de set, «La société et l'organisation traditionnelles de
  l'Andorre», a docs/raw/academic/becat-these-l2.txt (18.098 línies). Cobreix la
  vida pastoral, l'organització de l'espai de muntanya, la casa andorrana i el
  seu aparell jurídic (capítols matrimonials, hereu i pubilla, cabalers i
  llegítima, venda a carta de gràcia, boïgues i sembrats), les regles socials, i
  l'organització política tradicional: els quarts, les parròquies, els
  coprínceps, el Consell General i l'Assemblea Magna. Amb annexos sobre la
  permeabilitat de la frontera i el contraban, les demandes de comunals a
  Canillo (1874-1893), els cortons i herbes de Canillo, les bordes i els
  cortals, i la fustamenta.
notes: >
  ÉS UNA TESI DE 1993 I S'HA DE DATAR EN CITAR-LA. Descriu una Andorra
  IMMEDIATAMENT ANTERIOR A LA CONSTITUCIÓ: parla dels coprínceps com a titulars
  de tots els poders, dels notaris com a «serveis dels coprínceps», i de la
  manca de jerarquia entre corporacions locals i estat. L'autor mateix ho
  precisa en nota: el patrimoni exclusiu dels sobirans ho era «fins a la
  constitució de 1993».
  ELS SIS LLIBRES RESTANTS NO SÓN AL CORPUS. El sumari de l'obra sencera és
  imprès dins aquest fitxer, i per tant el corpus sap exactament què li falta.
  Registrat a raw/README.md, on la xifra anterior —«sis llibres»— era errònia.
  PORTA TREBALL DE CAMP DATAT I CITABLE. Becat cita enquestes pròpies amb dia i
  mes (per exemple, el llevador del quart de la Cortinada, 15/04/1990). Això no
  és bibliografia: és font primària d'entrevista.
  RESOLT EL 16-09-2026: LA REDISTRIBUCIÓ PASSA DE `pendent` A `no`. Les dues
  preguntes obertes tenen resposta i és negativa: l'autor és viu i l'obra és una
  reedició seva del 2019 —no hi ha domini públic per antiguitat—, i no s'ha
  localitzat cap llicència oberta ni cap avís de reutilització. `pendent` volia
  dir «no comprovat»; comprovat, és `no`. El corpus la cita amb atribució i
  pàgina i no la reprodueix. Registre de drets: 02-DOCS/raw/sources/becat-tesi-1993.md.
  CITAR-LA AMB PÀGINA. Les fitxes que en surten la citaven per capítol i secció.
  La pàgina és resoluble sense sortir del fitxer: el peu de pàgina «J. Becat,
  Andorre. Thèse 1993, édition 2019 - Livre 2…» va seguit del número de pàgina, i
  el sumari imprès (pp. 7-11) dona la pàgina d'inici de cada secció. Les vuit
  unitats registrades el 16-09-2026 ja porten el rang de pàgines.
  ESTÀ EN FRANCÈS. Les citacions catalanes que reprodueix (reformes, politars,
  informants) hi van en català dins un text francès. Cap fragment d'aquesta
  obra no és «veu originària».
---

# Jean Becat — la tesi del 1993, llibre 2

**La societat i l'organització tradicionals d'Andorra**, per l'autor que el
corpus ja citava en nou fitxes sense haver-lo llegit mai en profunditat.

## Per què val

Perquè és **l'únic estudi del corpus que descriu l'aparell sencer alhora**: la
casa, el quart, la parròquia, els coprínceps i el Consell General com **un sol
sistema**, i no com cinc temes separats. I perquè **fa treball de camp**: hi ha
conflictes d'aigua, de permisos d'obra i de cartografia d'allaus amb data, poble
i nom d'institució.

## Avís de lectura

**És anterior a la Constitució i s'ha de citar amb l'any.** I **té tesi**: Becat
sosté que la casa andorrana és alhora realitat, mite social i **ideologia
política**. El corpus en reté els fets comprovables i **marca com a seva tota
interpretació que n'adopti**.

## El que en surt

| Tema | Fitxa |
| --- | --- |
| Els quarts: etimologia, càrrecs, competències i la seva crisi | [Quarts i veïnats](../temes/institucions/quarts-i-veinats/quarts-i-veinats.md) |
| Què hi ha dins d'uns capítols, i com es reparteix el patrimoni | [Els capítols matrimonials](../temes/historia/antic-regim/els-capitols-matrimonials.md) |
| La llegítima, els *concos*, *fer canvi* i els *andorrans de bragueta* | [L'hereu i el cabaler](../temes/historia/antic-regim/lhereu-i-el-cabaler.md) |
| Quatre privilegis amb volum i pàgina, i les valls veïnes que van perdre els seus | [Els privilegis](../temes/historia/edat-mitjana/els-privilegis.md) |
| Els dos títols del Politar, i l'avís de Brutails sobre les còpies | [Els costumaris andorrans](../temes/historia/manual-digest/els-costumaris-andorrans.md) |
| L'estratègia matrimonial com a eina de política territorial | [La nissaga Perich](../temes/historia/antic-regim/la-nissaga-perich.md) |
| Quatre mots que el glossari normatiu no té | [Glossari de mots documentats](../temes/llengua/lexic-andorra/glossari-de-mots-documentats.md) |
| **La institució que el corpus esmentava i no havia explicat mai** | [L'Assemblea Magna](../temes/institucions/consell-general/lassemblea-magna.md) |
| **Que el contraban no s'acaba als anys quaranta**: el dossier de premsa 1986-1992 | [Contraban](../temes/historia/contraban/contraban.md) |
| **Els oficis de la colla, les veremes amb noms de poble i la migració femenina** | [Les tres migracions](../temes/societat/demografia/les-tres-migracions.md) |
| Una novel·la que el corpus no tenia | [La literatura andorrana](../temes/cultura/literatura/literatura-andorrana.md) |
| **Què és un pariatge, i per què cinc dels onze articles no parlen d'Andorra** | [El Pareatge](../temes/historia/pareatge/el-pareatge.md) |
| **La ficció de nacionalitat dels coprínceps, i per què la feblesa del bisbe va salvar Andorra** | [El Coprincipat](../temes/institucions/coprincipat/el-coprincipat.md) |
| **El crèdit d'un país sense bancs, i per què a Andorra no caduca** | [La venda a carta de gràcia](../temes/economia/banca-i-fiscalitat/la-carta-de-gracia.md) |

## Buits registrats

- ~~**Els llibres 1 i 3-7 no són al corpus.** El 3 (*l'aménagement actuel du
  territoire*) i el 4 (*les risques naturels*) són els que més hi lligarien:
  el 4 conté el capítol d'allaus que aquesta mateixa obra remet.~~ —
  **`parcial` el 19-09-2026**: el volum **Le risque d'avalanche en Andorre**
  (2014), ja incorporat al corpus, declara que les seves bases són **una part
  del llibre 4 inèdit** de la tesi ([fitxa](./becat-risque-avalanche-2014.md);
  [nota de procedència](../raw/allaus/becat-risque-avalanche-2014.txt)). Això
  cobreix parcialment el llibre 4; els llibres 1, 3, 5, 6 i 7, i la resta del
  llibre 4, segueixen sense ser al corpus.
- ~~**L'URL no consta.**~~ — **`parcial` el 19-09-2026**: el catàleg del
  [Govern d'Andorra](../raw/web/fonts/becat-tesi-1993-url.txt) identifica la
  tesi completa i en dona l'URL pública; no confirma que el llibre 2 sigui
  descarregable des d'aquell registre.
- **Del llibre 2, s'han llegit el sumari sencer, el capítol 4.1 (els quarts),
  3.1 sencer (la casa i el seu aparell jurídic, amb la carta de gràcia, els
  préstecs del comú i les boïgues), 4.3 (els coprínceps), 4.4 en la part de
  l'Assemblea Magna, l'annex 6.1 (la permeabilitat de la frontera i el
  contraban), el tros de migracions de treball del capítol 1, i trossos de 3.3
  (el debat sobre la casa).** — **`no-es-buit`**: inventari de lectura ja feta.
- ~~**Del llibre 2, segueixen sense buidar**: **3.2 (les regles socials)**, **el
  gruix de 3.3** —el mite i la ideologia de la casa, i «vers una ideologia
  política andorrana»—, **4.2 (les parròquies, les fronteres i les relacions
  amb l'Arieja)**, **4.3 (els coprínceps)**, **4.4 (el Consell General, la
  Sindicatura i l'Assemblea Magna)**, **2.1-2.5 (la vida pastoral i
  l'organització de l'espai)** i **els cinc annexos**, entre els quals **les
  demandes de comunals a Canillo del 1874-1893** i **els cortons i herbes de
  Canillo**.~~ — **`font_externa` el 2026-09-22**: cal obtenir i buidar les
  seccions de la tesi que el corpus encara no conserva.
- ~~**L'obra remet al seu propi llibre 4** per al capítol d'allaus, que el corpus
  necessita i no té — vegeu
  [dos blaus diferents als mapes d'allaus](../temes/territori/clima-i-muntanya/dos-blaus-diferents-als-mapes-dallaus.md).~~ —
  **`parcial` el 19-09-2026**: la publicació de 2014 n'ofereix una part i ja
  permet consultar el risc d'allaus i la cartografia de 1971–1996; no és el
  llibre 4 complet ni substitueix les seves seccions de gestió territorial.

---
type: font
id: ari-capsules
title: "Càpsules d'història d'Andorra — Andorra Recerca + Innovació"
titular: Andorra Recerca + Innovació (AR+I)
autor: Un ponent diferent a cada càpsula, nomenat al títol de la peça
publicacio: "Sèrie de 80 càpsules en vídeo publicades al canal de YouTube d'Andorra Recerca + Innovació, 2020-2023"
url: https://www.youtube.com/channel/UC_sgrb5DXkWphnjcW1YThzg
llicencia: "Creative Commons Attribution (reuse allowed), declarada al camp de llicència de YouTube de cada peça. Verificada a 5 peces (#34, #56, #57, #60, #65) el 2026-09-13; no verificada a la resta."
redistribucio: si
data_consulta: 2026-09-13
abast: >
  Vuitanta càpsules d'entre 6 i 30 minuts en què una persona nomenada explica un
  tema andorrà: història, institucions, patrimoni, oficis, llengua i memòria
  personal. Per al corpus de parla només serveixen les que són testimoni en
  primera persona; les que són exposició acadèmica llegida són coneixement, no
  mostra de llengua.
notes: >
  ÉS PARLA, NO TEXT. El valor d'aquesta sèrie per al corpus no és el que
  expliquen sinó QUE HO DIUEN EN VEU ALTA: és andorrà parlat per andorrans,
  publicat, amb el parlant nomenat i amb llicència oberta. És la combinació més
  neta que s'ha trobat fins ara.
  LA LLICÈNCIA ÉS EXCEPCIONAL. CC-BY en una institució pública andorrana no és
  la norma: la resta de fons audiovisual andorrà consultat el 2026-09-13 té drets
  reservats o sense declarar. Això fa la sèrie prioritària.
  CONSENTIMENT. El ponent es presenta pel seu nom dins la mateixa gravació i la
  institució el nomena al títol. No són informants privats: són autors parlant en
  qualitat d'autors. Per això el corpus els pot nomenar.
  ATENCIÓ AL REGISTRE. És xerrada preparada davant d'un micròfon, no conversa
  espontània. Té menys titubeigs i menys sintaxi oral que una entrevista, i
  alguns ponents llegeixen. La branca 'espontani' segueix buida i segueix sent
  la que falta.
  CAP PEÇA NO TÉ SUBTÍTOLS, ni manuals ni automàtics. Tota transcripció s'ha de
  produir, i per tant tota transcripció d'aquesta sèrie arrossega el problema de
  fidelitat que documenta docs/raw/parla/ari-capsula-56/README.md.
---

# Càpsules d'història d'AR+I

Vuitanta vídeos del canal d'**Andorra Recerca + Innovació**, publicats entre el
2020 i el 2023, en què **una persona nomenada parla d'un tema andorrà**.

## Per què val tant

Perquè resol de cop els dos problemes que bloquegen la branca de parla:

**El consentiment.** El ponent surt nomenat i es presenta ell mateix. No cal
anar a preguntar a cap dipòsit què va signar un informant mort el 1974.

**Els drets.** CC-BY. De tot el fons audiovisual andorrà que s'ha mirat el
2026-09-13 —Arxiu Nacional, RTVA, canals comunals—, aquesta sèrie és **l'única
amb llicència oberta declarada**.

## Què no resol

No és **parla espontània**. És xerrada preparada, i això es nota: la sintaxi és
més endreçada que la d'una conversa i alguns ponents llegeixen. Serveix per al
lèxic i per a la morfosintaxi; no serveix per estudiar com s'encavalquen dos
andorrans parlant alhora.

## Avís de font, segon: **no consta que els ponents siguin andorrans**

Trobat a la **tanda 29** i, per data, tard: la sèrie porta **onze peces admeses**
des de la tanda 6 sense que aquesta pregunta s'hagués fet mai en veu alta.

**AR+I convida qui sap del tema, no qui és del lloc.** Això és normal en una
sèrie de divulgació històrica i **no ho és gens** en una font de mostra de
llengua. La càpsula **#62** ho ha fet visible dient-ho: «el confinament em va
enganxar **no a Andorra**» (00:00:53), amb tot el relat situat a l'Alt Urgell.
La peça **no ha entrat al corpus** ([pendents](../raw/parla/pendents/README.md)).

S'han rellegit les dotze transcripcions buscant qualsevol frase on el ponent es
col·loqui ell mateix en un lloc:

| Què diu de si mateix | Càpsules | Lectura |
| --- | --- | --- |
| **«aquí a Andorra»** | #20, #34, #56, #57, #66 | Es col·loca **dins** del país mentre parla. **Indici feble a favor**: un convidat de fora també ho pot dir. |
| **Res** | #11, #13, #27, #40, #45, #49, #70 | **Buit.** Ni a favor ni en contra. |
| **«no a Andorra»** | **#62** | **En contra.** Fora del corpus. |

**Cap de les onze admeses no diu el contrari, i cinc diuen que hi són.** Amb
això no s'acredita ningú i **no se'n descarta cap**: descartar-les per silenci
seria el negatiu plausible que el brief prohibeix, i és l'error que ja es va
cometre a la tanda 23.

El que sí que canvia és **què es pot dir d'aquestes onze peces**. Serveixen com
a **català parlat contemporani sobre temes andorrans, amb consentiment net**.
**No serveixen, per elles soles, com a prova que una forma sigui andorrana**, i
qualsevol tret que en surti s'ha de creuar amb el
[Consell Constituent](consell-general-constituent.md), on els parlants són
consellers generals electes i la provinença **sí que consta**.

## Peces que el corpus ja ha treballat

| Càpsula | Ponent | Fitxa |
| --- | --- | --- |
| #56. La vida a pagès | Albert Rossa Naudí | [La vida a pagès](../parla/oral/la-vida-a-pages.md) |
| #57. Un racó d'Escaldes-Engordany | Ruth Casabella Vehils | [Un racó d'Escaldes-Engordany](../parla/oral/un-raco-descaldes.md) |
| #34. Les falles d'Andorra | Albert Roig | [L'arbre fallaire](../parla/oral/les-falles-albert-roig.md) |
| #11. Les moles de farina | Francina Pons | [La mola de casa, deien tots](../parla/oral/les-moles-de-farina.md) |
| #66. El contrapàs | Teo Armengol | [El contrapàs](../parla/oral/el-contrapas-teo-armengol.md) |
| #40. Les campanes d'Andorra | Robert Lizarte | [També sóc campaner](../parla/oral/les-campanes-robert-lizarte.md) |
| #13. Els hostals comunals | Ludmilla Lacueva | [Es regulaven per una taba](../parla/oral/els-hostals-comunals-lacueva.md) |
| #49. Cal Pal de la Cortinada | Esther Jover | [Alberg és un sinònim de casa](../parla/oral/cal-pal-esther-jover.md) |
| #45. Toponímia preromana | Xavier Planas | [Topònims que la màquina no sap escriure](../parla/oral/toponimia-preromana-xavier-planas.md) |
| #27. Els noms dels carrers | Alfred Llahí | [El carrerer nacional](../parla/oral/els-noms-dels-carrers.md) |
| #70. La nissaga dels Martí de les Bons | Sara Ubach | [Una missaga que arriba als nostres dies](../parla/oral/la-nissaga-dels-marti.md) |
| #20. Els molins d'aigua | Alan Ward | [Rodes verticals, rodes catalanes](../parla/oral/els-molins-daigua.md) |
| #62. Els camins dels cérvols | Laura de Castellet | **pendent — [provinença](../raw/parla/pendents/README.md)** |
| #33. La ramaderia als segles IX-XI | Climent Miró | **descartada — [text llegit](../raw/parla/descartades/README.md)** |
| #58. Història de l'esquí | Daniel Areny | **descartada — [text llegit](../raw/parla/descartades/README.md)** |
| #60. La pedra seca | Xavier Llovera | **descartada — [text llegit](../raw/parla/descartades/README.md)** |

## Peces candidates, per ordre d'interès per a la parla

Testimoni en primera persona o ofici tradicional, que és on surt el lèxic:

| Càpsula | Ponent | Per què |
| --- | --- | --- |
| #77. El doctor Vilanova | Josep Vilanova | Memòria familiar |

Cal **verificar la llicència peça a peça**. Comprovades el 2026-09-13: **#34,
#56, #57, #60 i #65, totes CC-BY**. Cinc de cinc fa pensar que és la política de
la sèrie, però **cinc no són vuitanta** i no es dona per bona a la resta.

## Avís de font: aquesta sèrie porta guions llegits

De les **onze càpsules transcrites**, **cinc tenen problema de guió**:

| Peça | Estat |
| --- | --- |
| #60, la pedra seca | **descartada**, text llegit |
| #58, l'esquí | **descartada**, text llegit |
| #57, un racó d'Escaldes | admesa, `possiblement-llegida` |
| #13, els hostals comunals | admesa, `possiblement-llegida` |
| #49, Cal Pal | admesa, `possiblement-llegida` |
| #33, la ramaderia | **descartada**, text llegit |
| #62, els cérvols | `POT SER LLEGIDA` **i el detector s'equivoca** — és un relat en primera persona; queda fora per un altre motiu |

**Des de fora sonen igual que les altres sis.** Qui faci servir aquesta font ha
de passar cada peça pel
[detector de text llegit](../raw/parla/detector-text-llegit.py) abans de
res — i, si el veredicte és «sospitós», **llegir la transcripció**, perquè és
justament allà on el detector s'equivoca.

## El duplicat, i el que insinua

La #57 hi és **dues vegades** (`xyJqeun7e2g`, 18:01, i `Y5s87g2IYwE`, 17:56),
amb títols i dates diferents. **Qui treballi aquesta sèrie ha de comptar amb
duplicats** i no donar per fet que una càpsula és una peça.

## La #65 no era el que semblava

Aquesta fitxa va dir durant divuit tandes que la **#65, «Una conversa sobre Boris
Skóssirev»**, era *l'única conversa de dues veus amb drets nets* i *el candidat
més valuós de la sèrie* per a la branca `espontani`, i que només esperava
diarització.

**És en castellà.** Transcrita el 2026-09-14: **216 marcadors castellans contra 7
catalans**. Un dels dos interlocutors és investigador alemany i parlen en castellà.

**Es va donar per catalana perquè la font i el títol ho són.** Està descartada i
documentada a
[descartades](../raw/parla/descartades/README.md).
